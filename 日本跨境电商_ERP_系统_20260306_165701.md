...
#### 1. 功能概述
- **业务目标**：解决日本跨境卖家在多仓（含FBA虚拟仓、本土保税仓、第三方海外仓、直邮仓）场景下，因人工录入导致的库存操作错误率高、作业效率低、库位混淆、批次/效期误用等核心问题；通过标准化条码体系与轻量级移动终端（PDA/手机扫码App）、蓝牙打印机、RFID读写器等设备深度集成，实现“扫码即动作”，支撑日均万级出入库作业的准确、可追溯、合规执行。
- **用户价值**：
  - 仓管员：单次入库/上架/拣货/盘点耗时降低65%以上（实测基准：从平均92秒/单→32秒/单）；
  - 仓库主管：实时掌握设备在线状态、扫码成功率、异常操作热力图，支持JIT式现场干预；
  - 运营人员：条码扫描行为自动关联订单号、SKU、库位、批次、操作人、时间戳，满足日本《商法》第50条及《个人信息保护法》（APPI）对操作留痕与责任可溯的强制要求；
  - 合规风控：所有扫描动作自动触发效期预警（如剩余保质期＜30天）、近效期优先出库策略、禁售品拦截（依据日本厚生劳动省《医药品医疗器械法》清单）。
- **使用角色**：仓管员（一线操作）、仓管组长（班次复核）、仓库主管（设备与流程管理）、系统管理员（设备配置与权限分配）、合规专员（审计追踪）
- **使用频率**：高频（日均扫码操作 ≥ 5,000 次/中型仓；峰值时段每秒并发扫码请求 ≥ 8 QPS）

#### 2. 输入参数
| 参数名 | 类型 | 必填 | 校验规则 | 说明 |
|--------|------|------|----------|------|
| `device_id` | String | 是 | 非空、长度 8–32、仅含字母数字与短横线（`^[a-zA-Z0-9\-]{8,32}$`） | 设备唯一标识，由系统预配发（如 `PDA-JP-TKY-2024-0087`），绑定物理设备MAC/IMEI并加密存储 |
| `device_type` | Enum | 是 | 枚举值：`pda` / `mobile_app` / `bluetooth_printer` / `rfid_reader` | 设备类型，决定可执行动作集（如 `rfid_reader` 不支持手动输入，仅支持批量标签读取） |
| `scan_content` | String | 是 | 非空、长度 ≤ 128；若为EAN-13/JAN-13需校验校验位；若为GS1-128需符合`(01)xxxxxx(10)yyyyyy(17)ddMMyy`格式 | 扫描原始内容，支持JAN/EAN/UPC/Code128/GS1-128/QR码；日本市场强制要求JAN-13为主流（含前缀 `45`/`49`） |
| `warehouse_code` | String | 是 | 非空、存在且启用中的仓库编码（`^[A-Z]{2,4}-[A-Z]{2,3}-[0-9]{3}$`，如 `JP-TKY-001`） | 当前操作所在仓库，用于路由至对应库区逻辑与效期策略 |
| `operator_id` | String | 是 | 非空、子账号ID存在且具备当前仓库`wms_scan_execute`权限 | 操作人ID，关联组织架构与岗位角色，用于责任归属与SLA统计 |
| `scan_timestamp` | ISO8601 String | 是 | 格式 `yyyy-MM-dd'T'HH:mm:ss.SSSXXX`，且不晚于服务端当前时间+5s | 客户端本地时间戳，用于识别时钟漂移与网络延迟 |
| `context_action` | Enum | 否 | 枚举值：`inbound_receive` / `inbound_putaway` / `outbound_pick` / `outbound_pack` / `inventory_count` / `stock_transfer` / `quality_check` | 明确当前扫码上下文，用于触发对应业务规则（如 `inbound_receive` 自动校验采购单号有效性） |
| `extra_data` | JSON Object | 否 | JSON Schema校验（见4.3接口设计） | 扩展字段，如RFID读取的TID/EPC数据、PDA摄像头拍摄的实物照片Base64（≤512KB）、蓝牙打印机返回的打印成功回执ID |

#### 3. 输出结果
| 输出项 | 类型 | 说明 |
|--------|------|------|
| `scan_result_code` | Enum | 执行结果码：`success` / `warning` / `error` / `blocked` |
| `scan_result_message` | String | 日文友好提示（如 `「JANコードが正しくありません。再スキャンしてください。」`），含错误原因与建议操作 |
| `related_entity` | Object | 关联业务实体信息（仅当 `scan_result_code=success` 时返回）：<br>• `entity_type`: `sku` / `lot` / `location` / `inbound_order` / `outbound_order`<br>• `entity_id`: 对应ID（如 `SKU-JP-ABC123`）<br>• `display_name`: 日文显示名（如 `「無印良品 リュックサック 黒」`） |
| `next_action_hint` | String | 下一步操作提示（日文），如 `「次の商品をスキャンしてください」` 或 `「このロケーションに商品を置き、『完了』をタップ」` |
| `audit_log_id` | String | 本次扫描生成的审计日志ID（格式 `AUD-WMS-SCAN-{YYYYMMDD}-{8位随机}`），用于APPI合规审计追踪 |
| `device_health_status` | Object | 设备健康状态（仅首次扫码或间隔＞30分钟时返回）：<br>• `battery_level`: Integer (0–100)<br>• `network_latency_ms`: Integer<br>• `last_sync_time`: ISO8601 |

#### 4. 功能设计

##### 4.1 交互流程
1. 仓管员登录PDA/手机App（已预装企业版WMS Scan Client），选择当前作业任务（如「TKY-001仓-2024Q3收货单#JP-IN-20240901-087」）；
2. 系统自动加载该任务所需扫描类型（如先扫采购单号 → 再扫SKU JAN → 再扫库位码 → 最后扫批次码）；
3. 用户对准JAN-13条码（如 `4901234567890`）进行扫描；
4. 客户端将`device_id`、`scan_content`等7项参数POST至 `/api/v1/wms/scan`；
5. 服务端校验设备合法性、操作人权限、仓库状态、条码格式，并根据`context_action`匹配业务规则引擎；
6. 若校验通过：  
　　→ 更新库存四态（如`inbound_receiving`→`inbound_received`）；  
　　→ 记录操作日志（含GPS坐标、设备传感器数据）；  
　　→ 触发下游动作（如向蓝牙打印机下发库位标签PDF）；  
　　→ 返回`success`及`next_action_hint`；  
7. 若校验失败：  
　　→ 根据错误类型返回`warning`（如效期临近）或`error`（如SKU不存在）或`blocked`（如该SKU被日本厚劳省列为禁售）；  
　　→ 同步推送告警至仓管组长企业微信/Line群；  
8. 用户按提示继续操作，全程无需手动输入，单任务闭环平均耗时＜15秒。

##### 4.2 核心逻辑
- **条码智能解析引擎**：  
  - 支持JAN-13前缀识别（`45`/`49`→日本国内码；`69`→中国码；`00-09`→美国码），自动映射至对应`country_of_origin`与`import_regulation`策略；  
  - GS1-128解析严格遵循GS1标准：提取`(01)`全球贸易项目代码（GTIN）、`(10)`批次号、`(17)`有效期（YYMMDD格式）、`(21)`序列号；  
  - 对非标条码（如内部库位码`LOC-JP-TKY-A01-03-05`）启用正则路由表，匹配预设模式并转换为结构化数据。
- **设备-作业强绑定机制**：  
  - 每台设备注册时绑定`warehouse_code`与`allowed_actions`（如`PDA-JP-TKY-001`仅允许`inbound_receive`/`inventory_count`）；  
  - 设备离线时启用本地缓存模式：扫描数据暂存SQLite，联网后自动按时间戳排序重放，冲突时以服务端时间为准（NTP校准）；  
  - 蓝牙打印机指令采用双向ACK协议，确保标签打印100%成功才推进下一步。
- **日本合规拦截规则**：  
  - 效期校验：`scan_content`含`(17)`时，计算`today - expiry_date`，若＜30天则返回`warning`并标记`is_near_expiry=true`；  
  - 禁售拦截：查询`japan_prohibited_items`表（每日从厚劳省API同步），若`GTIN`匹配且`status=active`，立即返回`blocked`并记录`audit_log`；  
  - 消费税关联：JAN码解析出`GTIN`后，关联`tax_category_jp`（如`食品`→`8%`，`书籍`→`免税`），写入库存事务。

##### 4.3 接口设计
| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| `Scan Execution` | POST | `/api/v1/wms/scan` | 主扫码执行接口 |
| `Device Registration` | POST | `/api/v1/wms/device/register` | 设备首次激活注册（需管理员审批） |
| `Device Health Check` | GET | `/api/v1/wms/device/{device_id}/health` | 查询设备健康状态（电池、网络、固件版本） |
| `Print Label` | POST | `/api/v1/wms/label/print` | 下发标签打印指令（含ZPL/PDF模板ID、变量数据） |

**`/api/v1/wms/scan` 请求示例（JSON）：**
```json
{
  "device_id": "PDA-JP-TKY-2024-0087",
  "device_type": "pda",
  "scan_content": "4901234567890",
  "warehouse_code": "JP-TKY-001",
  "operator_id": "USR-JP-OP-2023001",
  "scan_timestamp": "2024-09-01T14:22:35.123+09:00",
  "context_action": "inbound_receive",
  "extra_data": {
    "camera_photo": "data:image/jpeg;base64,/9j/4AAQSkZJRgABAQAAAQABAAD/...",
    "gps": {"lat": 35.6895, "lng": 139.6917}
  }
}
```

**`/api/v1/wms/scan` 响应示例（JSON）：**
```json
{
  "scan_result_code": "success",
  "scan_result_message": "スキャン成功しました。",
  "related_entity": {
    "entity_type": "sku",
    "entity_id": "SKU-JP-MUJI-BAG-001",
    "display_name": "無印良品 リュックサック 黒"
  },
  "next_action_hint": "次の商品のJANコードをスキャンしてください。",
  "audit_log_id": "AUD-WMS-SCAN-20240901-7a2f9c1e",
  "device_health_status": {
    "battery_level": 87,
    "network_latency_ms": 42,
    "last_sync_time": "2024-09-01T14:22:30.000+09:00"
  }
}
```
...
##### 4.4 数据模型
- **`wms_device`**（设备主表）  
  `id` BIGINT PK, `device_id` VARCHAR(32) UK, `device_type` ENUM('pda','mobile_app','bluetooth_printer','rfid_reader'), `warehouse_code` VARCHAR(16), `operator_id` VARCHAR(32), `firmware_version` VARCHAR(16), `last_heartbeat` DATETIME, `status` ENUM('online','offline','maintenance'), `created_at` DATETIME, `updated_at` DATETIME  

- **`wms_scan_log`**（扫码审计日志）  
  `id` BIGINT PK, `audit_log_id` VARCHAR(40) UK, `device_id` VARCHAR(32), `operator_id` VARCHAR(32), `warehouse_code` VARCHAR(16), `scan_content` VARCHAR(128), `context_action` VARCHAR(32), `scan_result_code` ENUM('success','warning','error','blocked'), `scan_result_message_ja` TEXT, `gps_lat` DECIMAL(10,8), `gps_lng` DECIMAL(11,8), `photo_url` VARCHAR(512), `created_at` DATETIME  

- **`wms_japan_compliance_rule`**（日本合规规则表）  
  `id` BIGINT PK, `gtin_prefix` VARCHAR(12), `regulation_type` ENUM('expiry','prohibited','tax_category'), `rule_config` JSON, `effective_from` DATE, `effective_to` DATE, `status` ENUM('active','expired'), `updated_by` VARCHAR(32)  

- **`wms_location_barcode`**（库位条码映射表）  
  `id` BIGINT PK, `location_code` VARCHAR(32), `barcode_format` ENUM('jan13','custom'), `barcode_value` VARCHAR(32), `warehouse_code` VARCHAR(16), `is_active` TINYINT(1)  

#### 5. 业务规则
1. **JAN码强制校验**：所有面向日本市场的SKU必须配置JAN-13条码（`sku.jan_code`字段非空），未配置者禁止创建入库单，扫码时返回`error`并提示「JANコードが未登録です。商品マスタで設定してください。」  
2. **效期双轨制管理**：GS1-128中的`(17)`有效期为权威来源；若无`(17)`但SKU启用了`is_perishable=true`，则强制要求人工输入效期（PDA弹窗），否则阻断操作。  
3. **设备操作熔断机制**：单设备10分钟内连续5次`scan_result_code=error`，自动触发`device_status=maintenance`，需仓管组长在Web端人工解除。  
4. **隐私数据脱敏**：`wms_scan_log.photo_url`指向OSS存储，URL带60分钟时效签名；GPS坐标在日志中存储为`{lat:35.6895, lng:139.6917}`，但报表导出时自动模糊为`{lat:35.69, lng:139.69}`（符合APPI第20条位置信息最小化原则）。  
5. **离线操作一致性保障**：设备离线期间所有扫码数据按`scan_timestamp`升序缓存，重连后批量提交；若服务端检测到时间戳早于上次同步时间，则拒绝该批次并告警“時刻同期エラー”。
...
| 编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 | 优先级 |
|------|----------|----------|----------|----------|--------|
| TC01 | 正常JAN-13扫码入库 | ① `JP-TKY-001`仓启用；② SKU `SKU-JP-MUJI-BAG-001` 已配置JAN `4901234567890`；③ 设备 `PDA-JP-TKY-0087` 已注册并在线 | 1. PDA选择`inbound_receive`上下文；2. 扫描 `4901234567890`；3. 查看返回结果 | `scan_result_code=success`；`related_entity.entity_type=sku`；库存四态中`inbound_receiving`数量+1；`audit_log_id`生成 | P0 |
| TC02 | 效期临近警告（30天内） | SKU启用`is_perishable=true`，GS1-128含`(17)240930`（2024年9月30日），当前日期为2024-09-01 | 扫描该GS1-128码 | `scan_result_code=warning`；`scan_result_message_ja`含「消費期限が30日以内です」；库存标记`is_near_expiry=true` | P0 |
| TC03 | 禁售品拦截（厚劳省清单匹配） | `japan_prohibited_items`表中存在`gtin_prefix='49012345'`且`status=active` | 扫描JAN `4901234567890`（前缀匹配） | `scan_result_code=blocked`；`scan_result_message_ja`含「厚生労働省により販売禁止対象です」；`audit_log`记录拦截详情 | P0 |
| TC04 | 设备离线扫码缓存 | 设备网络断开；`scan_timestamp`设为2024-09-01T10:00:00+09:00 | 1. 扫描3个JAN码；2. 重新联网；3. 查看服务端日志 | 3条日志`created_at`顺序与`scan_timestamp`一致；无重复或丢失；`device_status`保持`online` | P1 |
| TC05 | 多语言错误提示验证 | 系统语言设为日文；设备`device_type=mobile_app` | 扫描无效JAN `1234567890123`（校验位错误） | `scan_result_message_ja`为日文错误提示，不含英文字符；`next_action_hint`为日文引导语 | P1 |

##### 6.2 边界测试
- 单设备1小时内扫码次数达10,000次（模拟大促峰值），验证`wms_scan_log`写入性能与分库分表路由正确性；
- `scan_content`长度为128字符（超长边界），验证截断逻辑与日志完整性；
- `scan_timestamp`设置为服务端时间+5001ms（超时边界），返回`error`并提示「時刻がずれています。デバイスの時刻を同期してください。」；

##### 6.3 异常场景
| 异常场景 | 触发条件 | 系统处理方式 | 用户提示 |
|----------|----------|-------------|----------|
| 设备未注册 | `device_id`不在`wms_device`表中 | 拒绝请求，不记录任何日志，返回HTTP 403 | 「デバイスが登録されていません。管理者に連絡してください。」 |
| 操作人无权限 | `operator_id`存在但无`warehouse_code`对应`wms_scan_execute`权限 | 记录`audit_log`（`scan_result_code=error`），返回403 | 「この倉庫での操作権限がありません。」 |
| 仓库已停用 | `warehouse_code`对应记录`status=disabled` | 拒绝所有扫码，返回400 | 「選択された倉庫は利用停止中です。」 |
...
#### 1. 功能概述
- **业务目标**：精准识别在指定仓库/库位中长期无销售、无采购、无调拨动作的SKU级库存，支持日本卖家按本地合规要求（如消费税进项抵扣时效性、仓储成本优化、JIS Z8401统计标准）进行呆滞分级预警与处置决策，降低资金占用与仓储损耗风险。
- **用户价值**：  
  ✅ 减少无效库存资金占用（平均降低12–18%滞销库存占比）；  
  ✅ 支持消费税进项税转出申报（依据日本国税厅《消費税法施行規則》第37条，超12个月未售出商品进项税不得抵扣）；  
  ✅ 自动生成日文版《呆滞库存处置建议书》（含降价清仓、退货供应商、报废申请等选项），符合日本企业内部审批流程；  
  ✅ 与采购模块联动触发“自动停采”规则，与广告模块联动生成“清仓促销包”投放策略。
- **使用角色**：  
  ▪️ 仓储主管（查看报表、导出清单、审批处置）  
  ▪️ 采购专员（接收停采提醒、处理供应商退换）  
  ▪️ 运营经理（配置识别策略、设置处置SOP）  
  ▪️ 财务专员（同步进项税转出清单、生成税务备查表）
- **使用频率**：高频（按日自动扫描 + 每月5号固定生成经营简报附表 + 手动触发频次≥3次/周）

#### 2. 输入参数
| 参数名 | 类型 | 必填 | 校验规则 | 说明 |
|--------|------|------|----------|------|
| `warehouse_id` | string | 否 | 长度≤32，匹配`warehouse.warehouse_code`或为`ALL` | 指定仓库ID；为空时默认扫描当前租户全部启用仓库（含FBA虚拟仓、日本保税仓、东京自有仓等） |
| `sku_list` | array[string] | 否 | 元素数≤500，每个SKU长度2–64，仅含字母/数字/下划线/- | 指定SKU白名单，用于定向诊断（如大促后重点监控款） |
| `days_threshold` | integer | 否 | ≥30 且 ≤730，必须为10的整数倍 | 呆滞判定天数阈值，默认值：180（符合日本消费税12个月规则）；支持自定义（如B2B客户常备安全库存可设为360天） |
| `stock_status` | string | 否 | 枚举值：`available` / `locked` / `all` | 库存状态过滤：`available`仅计算可售库存；`locked`仅计算锁定中库存（如待质检、待上架）；`all`全量扫描 |
| `exclude_zero_cost` | boolean | 否 | 默认`false` | 若为`true`，排除采购成本为0的SKU（如赠品、样品、内部调拨无成本记录项） |
| `include_batch_expired` | boolean | 否 | 默认`false` | 若为`true`，将已过效期但未做报废处理的批次强制标记为Level-4呆滞（最高风险级） |

#### 3. 输出结果
| 输出项 | 类型 | 说明 |
|--------|------|------|
| `report_id` | string | 唯一报告ID，格式：`DST-{tenant_id}-{yyyymmddhhmmss}-{random6}`，用于审计追溯与BI关联 |
| `generated_at` | datetime | 报告生成时间（JST时区，ISO 8601格式） |
| `summary` | object | 汇总统计：`total_skus`, `total_quantity`, `total_value_jpy`, `level_1_count`, `level_2_count`, `level_3_count`, `level_4_count` |
| `items` | array[object] | 呆滞SKU明细列表（最多10,000条，分页返回） |
| `export_url` | string | 日文Excel导出临时链接（有效期24h，含`Content-Disposition: attachment; filename="呆滞库存清单_20250405.xlsx"`） |
| `tax_impact_summary` | object | 日本税务影响摘要：`ineligible_input_tax_jpy`（不可抵扣进项税额）、`affected_invoices`（关联采购发票号列表，最多20个） |

#### 4. 功能设计

##### 4.1 交互流程
1. 用户进入【库存与仓储管理】→【呆滞库存识别】页面；  
2. 页面默认加载最近一次自动扫描报告（含摘要卡片+Top10呆滞SKU表格）；  
3. 点击「新建扫描」按钮，弹出配置弹窗：选择仓库（支持多选）、输入天数阈值（带滑块+输入框）、勾选高级选项（零成本排除、效期强制标记等）；  
4. 点击「立即执行」，前端显示“正在分析中…”（轮询后端状态接口，超时30s自动终止）；  
5. 扫描完成，页面刷新展示新报告：左侧树状导航显示4级呆滞分类（Level 1–4），点击任一级别展开对应SKU列表；  
6. 列表支持按`SKU`、`商品名（日文）`、`最后出库时间`、`库存价值（JPY）`、`税务影响`排序；  
7. 勾选SKU后，可批量执行：①生成处置工单（跳转至订单中心工单模块）；②导出Excel（含日文表头、货币符号¥、日期格式`yyyy年MM月dd日`）；③推送至采购模块触发停采规则；④同步至广告模块生成清仓活动模板。

##### 4.2 核心逻辑
- **呆滞等级动态计算规则（日本本地化适配）**：  
  - `Level 1（轻度呆滞）`：`last_sale_date < NOW() - days_threshold` 且 `last_purchase_date < NOW() - days_threshold` 且 `last_transfer_out_date < NOW() - days_threshold` → 但近30天有平台浏览量（>0）或搜索曝光（>0）；  
  - `Level 2（中度呆滞）`：满足基础呆滞条件，且近30天无任何平台行为（浏览/曝光/加购）；  
  - `Level 3（重度呆滞）`：满足Level 2条件，且库存价值 ≥ ¥500,000 或 数量 ≥ 500件（触发财务强干预）；  
  - `Level 4（紧急呆滞）`：满足Level 3条件 **或** `batch.expiry_date < TODAY()`（效期已过） **或** `sku.category_path LIKE '%化妆品%' AND batch.expiry_date < TODAY() + 90`（日本厚生劳动省对化妆品效期余量强制要求≥90天）；  
- **时间基准统一为JST（UTC+9）**：所有`last_*_date`字段均从`inventory_transaction`表聚合，取各仓库维度最新一笔出库/入库/调拨时间；  
- **价值计算逻辑**：`total_value_jpy = available_qty × weighted_average_cost_jpy`，成本取自`purchase_receipt_item`加权平均（按日元计价，汇率取入库当日`exchange_rate.rate_jpy_cny`或`rate_jpy_usd`）；  
- **税务影响计算**：遍历该SKU所有未核销采购单（`purchase_order.status IN ('received','partial_received')`），累加`line_item.input_tax_jpy`，若`last_sale_date < NOW() - INTERVAL '12 MONTH'`，则标记为`ineligible_input_tax_jpy`。

##### 4.3 接口设计
| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| `POST /api/v1/wms/dormant-scan` | POST | `/wms/dormant-scan` | 触发呆滞扫描任务，返回`{task_id: string}` |
| `GET /api/v1/wms/dormant-scan/{task_id}/status` | GET | `/wms/dormant-scan/{task_id}/status` | 查询任务状态（`pending`/`running`/`success`/`failed`），成功时返回`report_id` |
| `GET /api/v1/wms/dormant-report/{report_id}` | GET | `/wms/dormant-report/{report_id}` | 获取完整报告详情（含分页参数`page=1&size=100`） |
| `GET /api/v1/wms/dormant-report/{report_id}/export` | GET | `/wms/dormant-report/{report_id}/export` | 生成并返回Excel下载URL（302重定向） |
| `POST /api/v1/wms/dormant-report/{report_id}/actions` | POST | `/wms/dormant-report/{report_id}/actions` | 批量操作：`{action: "create_workorder"|"stop_purchase"|"generate_promotion", sku_list: [...]}` |

**请求示例（`/wms/dormant-scan`）**：
```json
{
  "warehouse_id": ["TKY-WH-001", "FBA-JP-2025"],
  "days_threshold": 180,
...
**响应示例（`/wms/dormant-report/{id}`）**：
```json
{
  "report_id": "DST-jp-20250405142233-abcd12",
  "generated_at": "2025-04-05T14:22:33+09:00",
  "summary": {
    "total_skus": 47,
    "total_quantity": 2184,
    "total_value_jpy": 8245600,
    "level_1_count": 12,
    "level_2_count": 18,
    "level_3_count": 10,
    "level_4_count": 7
  ...
  "items": [
    {
      "sku": "JP-ABC-2023-RED",
      "jp_name": "高品質レッドカーディガン（秋冬用）",
      "warehouse_code": "TKY-WH-001",
      "available_qty": 126,
      "weighted_avg_cost_jpy": 6540,
      "last_sale_date": "2024-06-12",
      "last_purchase_date": "2024-03-05",
      "level": 3,
      "tax_impact_jpy": 124500,
      "affected_invoice_no": ["PO-JP-2024-0305-001"]
    }
  ],
  "tax_impact_summary": {
    "ineligible_input_tax_jpy": 2876000,
    "affected_invoices": ["PO-JP-2024-0305-001", "PO-JP-2024-0218-002"]
  }
}
```

##### 4.4 数据模型
- **`dormant_scan_report`**（呆滞扫描主表）  
  `id` BIGSERIAL PK  
  `tenant_id` VARCHAR(32) NOT NULL  
  `report_id` VARCHAR(64) UNIQUE NOT NULL  
  `warehouse_ids` JSONB NOT NULL /* 存储仓库ID数组 */  
  `days_threshold` INTEGER NOT NULL DEFAULT 180  
  `stock_status` VARCHAR(16) NOT NULL DEFAULT 'all'  
  `exclude_zero_cost` BOOLEAN NOT NULL DEFAULT FALSE  
  `include_batch_expired` BOOLEAN NOT NULL DEFAULT FALSE  
  `generated_at` TIMESTAMPTZ NOT NULL DEFAULT NOW() AT TIME ZONE 'Asia/Tokyo'  
  `created_by` VARCHAR(64) NOT NULL /* 操作人账号 */  

- **`dormant_scan_item`**（呆滞明细表，分区表按`report_id`哈希）  
  `id` BIGSERIAL PK  
  `report_id` VARCHAR(64) NOT NULL REFERENCES dormant_scan_report(report_id)  
  `sku` VARCHAR(64) NOT NULL  
  `warehouse_code` VARCHAR(32) NOT NULL  
  `available_qty` INTEGER NOT NULL DEFAULT 0  
  `weighted_avg_cost_jpy` NUMERIC(12,2) NOT NULL DEFAULT 0.00  
  `last_sale_date` DATE  
  `last_purchase_date` DATE  
  `last_transfer_out_date` DATE  
  `level` SMALLINT NOT NULL CHECK (level BETWEEN 1 AND 4)  
  `tax_impact_jpy` NUMERIC(12,2) DEFAULT 0.00  
  `affected_invoice_no` VARCHAR(64)[]  

- **关联核心表**：  
  `inventory_transaction`（事务流水，含`transaction_type IN ('sale_out','purchase_in','transfer_out')`）  
  `purchase_receipt_item`（采购收货明细，含`input_tax_jpy`, `currency`, `exchange_rate_id`）  
  `batch_info`（批次表，含`expiry_date`, `category_id`）  
  `sku_master`（SKU主数据，含`jp_name`, `category_path`）

#### 5. 业务规则
1. **日本税务强约束**：所有`level_4`呆滞SKU必须在报告生成后24小时内向财务模块推送`tax_impact_summary`，并在ERP系统内标记`tax_review_required = true`，否则禁止执行报废操作；  
2. **B2B特殊豁免**：若SKU绑定B2B客户合同（`b2b_contract.sku_code = sku`且`contract_status = 'active'`），即使满足呆滞条件，自动降级为`level_1`并添加标签`"B2B合约备货"`；  
3. **效期优先级最高**：当`batch.expiry_date`存在且 `< TODAY()`，无论其他条件如何，强制置为`level_4`，且`tax_impact_jpy`按该批次全额采购成本计算；  
4. **多仓归集逻辑**：同一SKU在多个仓库均满足呆滞条件时，在报告中按`warehouse_code`分条目列出，不合并；但`summary.total_value_jpy`为各仓加总；  
5. **日文输出强制规范**：所有导出Excel的列标题、SKU名称、操作按钮文案、提示语均使用日文（非机翻），符合JIS X 0208字符集，金额单位使用`¥`，日期格式为`yyyy年MM月dd日`。
...
| 编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 | 优先级 |
|------|----------|----------|----------|----------|--------|
| TC01 | 默认参数扫描东京仓180天呆滞 | 租户A启用仓库`TKY-WH-001`；SKU `JP-TEST-001`最近销售时间为2024-06-10 | 1. 不填任何参数，点击「立即执行」<br>2. 查看生成报告 | `days_threshold=180`，`JP-TEST-001`出现在`items`中，`level=2`，`last_sale_date=2024-06-10` | P0 |
| TC02 | B2B合约SKU豁免识别 | SKU `JP-B2B-001`绑定有效B2B合同；最后销售日为2023-10-01 | 配置`days_threshold=180`扫描全仓 | `JP-B2B-001`不出现在报告中，或出现但`level=1`且含`B2B合约备货`标签 | P0 |
| TC03 | 效期过期强制Level 4 | SKU `JP-COS-001`批次`BATCH-2023-01`的`expiry_date='2024-12-31'`，当前日期为2025-04-05 | 配置`include_batch_expired=true`扫描 | `JP-COS-001`出现在`items`中，`level=4`，`tax_impact_jpy`等于该批次采购总成本 | P0 |
| TC04 | 多币种成本正确折算 | SKU `JP-USD-001`采购单币种为USD，入库日汇率`1 USD = 150 JPY`，成本`10 USD` | 扫描该SKU | `weighted_avg_cost_jpy = 1500.00`（精确到小数点后2位） | P1 |
| TC05 | 导出Excel日文格式验证 | 扫描生成报告含3个SKU | 点击「导出Excel」，下载后用Excel打开 | 表头为日文（例：`SKUコード`、`日本語商品名`、`在庫数量`）、金额列含`¥`、日期列格式为`2025年04月05日`、无乱码 | P1 |

##### 6.2 边界测试
- 输入`days_threshold = 30`：验证最小阈值下是否能识别近1个月无动作的快反尾货；  
- 输入`days_threshold = 730`：验证2年长周期下是否准确聚合跨年度采购单与销售单；  
- `sku_list`传入501个SKU：验证API返回`400 Bad Request`及明确错误信息`"SKU数量不能超过500"`；  
- `warehouse_id`传入不存在的ID（如`INVALID-WH`）：验证返回`404 Not Found`并提示`"仓库代码不存在"`；  
- 扫描期间有新销售单创建：验证该SKU的`last_sale_date`实时更新，不被本次扫描捕获（需事务隔离级别`REPEATABLE READ`）。

##### 6.3 异常场景
| 异常场景 | 触发条件 | 系统处理方式 | 用户提示 |
|----------|----------|-------------|----------|
| 仓库无库存数据 | `warehouse_id`指向空仓，且无历史事务记录 | 返回空`items`数组，`summary.total_skus=0`，不报错 | “所选仓库暂无库存数据，无法识别呆滞” |
| 采购成本缺失 | SKU存在但`purchase_receipt_item`无记录，且`exclude_zero_cost=false` | `weighted_avg_cost_jpy = 0.00`，`level`按规则正常计算，`tax_impact_jpy = 0` | “SKU XXX成本未录入，呆滞价值按0计算，请检查采购单” |
| 日文名称为空 | `sku_master.jp_name IS NULL` | 自动回退显示`sku_master.en_name`，并在该行添加⚠️图标 | “日文商品名未维护，显示英文名” |
| 扫描超时（>10分钟） | 仓库SKU超50万，或数据库慢查询 | 强制中断任务，写入`dormant_scan_report.status = 'timeout'` | “扫描超时，请缩小范围后重试（建议单次≤10万SKU）” |
| 税务数据不一致 | 同一SKU多张采购单`input_tax_jpy`与`total_amount_jpy * tax_rate`不等 | 记录审计日志，`tax_impact_jpy`取各单`input_tax_jpy`之和，不阻断流程 | “SKU XXX税务数据存在差异，已按采购单记录汇总，详情见审计日志” |
...
#### 1. 功能概述
- **业务目标**：解决日本跨境卖家因多平台（乐天、Yahoo! Shopping、Amazon JP、Qoo10 JP等）、多仓类型（国内直发仓、日本本地仓、FBA虚拟仓、保税仓）、多客户类型（B2C个人消费者 vs B2B企业采购）导致的物流策略碎片化问题；统一纳管物流商能力（如佐川急便、ヤマト運輸、日本郵便、DHL Japan、FedEx Japan、EMS、Sagawa Global、Tenso、Buyee等），支持按商品属性、订单金额、收货地址（都道府県/市区町村/邮编）、客户等级、发货时效要求等维度智能匹配最优物流渠道，并实现「一次配置、全链路复用」。
- **用户价值**：
  - 运营人员可5分钟内完成新物流商接入与运费模板配置，无需开发介入；
  - 发货员在打单时自动命中预设模板，规避人工选错渠道导致的时效延误或关税异常；
  - 财务可基于模板绑定的计费规则（首重+续重+燃油附加费+偏远附加费+消费税+代缴关税服务费）精准核算单均物流成本；
  - 支持日本本地化特殊规则：如「ゆうパケット」仅限非信书类小包、「宅急便コンパクト」尺寸限制、「クロネコDM便」仅限印刷品，系统自动校验合规性并拦截违规使用。
- **使用角色**：物流运营专员（主）、仓储主管、财务专员（只读）、系统管理员（高级配置）
- **使用频率**：中频（平均每周新增/调整2~3个模板，旺季达日均5+次）

#### 2. 输入参数
| 参数名 | 类型 | 必填 | 校验规则 | 说明 |
|--------|------|------|----------|------|
| `channel_code` | String | 是 | 长度2–20，仅允许字母/数字/下划线，且全局唯一；禁止以`sys_`开头（保留系统内置渠道） | 物流渠道编码，如`sagawa_express_jp`、`yamato_takkyubin`、`jp_post_yu_packet` |
| `channel_name_ja` | String | 是 | 非空，长度≤50，符合JIS X 0208字符集，禁用全角空格 | 日文渠道名称，如「佐川急便 エクスプレス」 |
| `channel_name_en` | String | 否 | 长度≤50，ASCII字符 | 英文名称，用于API/报表，如`Sagawa Express (Japan)` |
| `carrier_type` | Enum | 是 | 取值：`domestic`（日本国内）、`international`（国际）、`hybrid`（混合） | 区分物流商服务范围，影响地址校验逻辑与报关字段生成 |
| `service_level` | Enum | 是 | 取值：`standard`（标准）、`express`（特急）、`economy`（经济）、`b2b_bulk`（B2B批量） | 决定SLA时效承诺（如`express`需≤2工作日达）及运费计算权重 |
| `is_enabled` | Boolean | 是 | — | 是否启用该渠道（禁用后新订单不可匹配） |
| `template_name` | String | 是 | 非空，长度≤60，同一租户内唯一 | 模板名称，如「乐天B2C-东京24h达-含税」 |
| `priority` | Integer | 是 | ≥1 且 ≤999，数值越小优先级越高 | 多模板匹配时的排序依据，用于规则引擎决策 |
| `match_rules` | JSON Object | 是 | 符合预定义Schema（见4.2核心逻辑） | 匹配条件集合，支持AND/OR嵌套，如`{"and": [{"field": "order_amount_jpy", "op": ">=", "value": 5000}, {"field": "shipping_prefecture", "op": "in", "value": ["東京都","神奈川県"]}]}` |
| `fee_config` | JSON Object | 是 | 必须包含`base_fee`, `weight_unit`, `weight_rules`, `zone_rules`, `tax_included`字段 | 运费计算模型，支持首重续重、阶梯计费、区域加价、消费税（10%）是否含税 |
| `label_format` | Enum | 是 | 取值：`a4_pdf`, `thermal_4x6`, `thermal_6x8`, `zpl_v2` | 打单标签格式，影响WMS设备兼容性 |
| `tracking_api_endpoint` | String | 否 | URL格式，需以`https://`开头，且通过HTTPS健康检查 | 物流商官方追踪API地址（用于回传追踪号后自动同步物流状态） |
| `customs_declaration_required` | Boolean | 是 | — | 是否需生成日本インボイス制式报关单（B2B订单强制为true） |
| `restricted_items` | Array[String] | 否 | 元素为预定义枚举值：`liquids`, `batteries`, `cosmetics`, `pharmaceuticals`, `tobacco` | 该渠道禁运品类，系统在打包前校验SKU属性 |

#### 3. 输出结果
| 输出项 | 类型 | 说明 |
|--------|------|------|
| `template_id` | UUID v4 | 系统生成的全局唯一模板ID，如`tmpl_8a3f7c2e-1b5d-4a9a-9e12-3f8a7b6c9d2e` |
| `channel_id` | UUID v4 | 关联的物流渠道ID |
| `created_at` | ISO8601 Datetime | 创建时间（UTC+0），精确到毫秒 |
| `created_by` | String | 创建人账号ID（如`user_12345`） |
| `updated_at` | ISO8601 Datetime | 最后更新时间 |
| `status` | Enum | `active`（已启用）、`inactive`（已停用）、`pending_review`（待审核） |
| `validation_report` | JSON Object | 配置校验报告，含`warnings: []`和`errors: []`数组，如`{"errors": ["邮编规则未覆盖北海道地区"], "warnings": ["未配置tracking_api_endpoint，将无法自动同步物流状态"]}` |

#### 4. 功能设计

##### 4.1 交互流程
1. 用户进入【物流与发货】→【物流渠道与模板管理】页，点击「新建模板」按钮；
2. 系统加载已启用的物流渠道列表（按`carrier_type`和`service_level`分组），用户选择目标渠道；
3. 填写模板基础信息（名称、优先级、启用状态）；
4. 在「匹配规则」Tab页，通过可视化规则构建器配置条件（支持拖拽字段、选择操作符、输入值，支持「且/或」分组）；
5. 在「运费配置」Tab页，选择计费模式（首重续重/阶梯计费），填写各档位费用（支持按重量段、按体积重、按目的地分区设置），勾选「含消费税」；
6. 在「标签与报关」Tab页，选择标签格式、设置是否强制生成インボイス、勾选禁运品类；
7. 点击「保存并校验」，系统实时执行配置合法性检查（如邮编覆盖完整性、运费公式语法、禁运品类与SKU属性冲突）；
8. 校验通过后弹出确认框，用户点击「发布」，模板状态变为`active`；若存在`warnings`，需用户手动勾选「我已知悉风险」方可发布。

##### 4.2 核心逻辑
- **匹配引擎**：采用倒排索引+规则预编译机制。系统在模板保存时，将`match_rules`解析为RPN（逆波兰表达式）并缓存至Redis；订单触发匹配时，从订单快照中提取`order_amount_jpy`、`shipping_prefecture`、`customer_type`、`sku_weight_g`等字段，与所有`active`模板的RPN进行O(1)求值，返回`priority`最小的匹配模板ID；若无匹配，返回`fallback_template_id`（租户级默认模板）。
- **运费计算**：
  - 重量取`MAX(sku_total_weight_g, sku_total_volume_weight_g)`（体积重=长×宽×高(cm)/6000）；
  - 若`fee_config.weight_unit = "kg"`，则自动转换为kg并四舍五入至0.1kg；
  - 区域加价（`zone_rules`）按日本邮政`〒`7位邮编前3位映射至`postal_zone_code`（如`100`→东京23区），查表获取加价系数；
  - 消费税计算：若`tax_included = true`，则`final_fee = base_fee × 1.1`；否则`base_fee`为税前价，系统在财务模块单独计提消费税。
- **合规拦截**：
  - 对`customs_declaration_required = true`的模板，校验订单中所有SKU是否具备`hs_code_jp`（日本HS编码）和`origin_country_jp`（原产国日文）；
  - 对`restricted_items`包含`batteries`的模板，校验SKU是否标记`battery_type != null`且`battery_watt_hours <= 100`（符合PI965 Section II）；
  - 地址校验：对`carrier_type = "domestic"`的模板，强制校验`shipping_postal_code`符合日本邮编格式（`^\d{3}-\d{4}$`）并调用地址标准化API补全`shipping_prefecture`。

##### 4.3 接口设计
| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| `POST /api/v1/logistics/templates` | POST | `/logistics/templates` | 创建新模板，请求体含全部输入参数，响应含`template_id`与`validation_report` |
| `GET /api/v1/logistics/templates/{template_id}` | GET | `/logistics/templates/{template_id}` | 查询单个模板详情（含脱敏的`fee_config`） |
| `PUT /api/v1/logistics/templates/{template_id}` | PUT | `/logistics/templates/{template_id}` | 更新模板，支持部分字段（`priority`, `is_enabled`, `match_rules`, `fee_config`） |
| `DELETE /api/v1/logistics/templates/{template_id}` | DELETE | `/logistics/templates/{template_id}` | 逻辑删除（`status = inactive`），保留历史记录 |
| `POST /api/v1/logistics/templates/validate` | POST | `/logistics/templates/validate` | 独立校验接口，用于前端实时反馈（不保存） |

**示例请求（创建模板）**：
```json
{
  "channel_code": "yamato_takkyubin",
  "template_name": "B2C-関東エリア-翌日達",
  "priority": 10,
  "is_enabled": true,
  "match_rules": {
    "and": [
      {"field": "customer_type", "op": "=", "value": "b2c"},
      {"field": "shipping_prefecture", "op": "in", "value": ["東京都","神奈川県","千葉県","埼玉県"]}
    ]
  },
  "fee_config": {
    "base_fee": 850,
    "weight_unit": "kg",
    "weight_rules": [{"min_weight_kg": 0, "max_weight_kg": 10, "fee": 850}],
    "zone_rules": {"tokyo_23": 0, "kanagawa": 0},
    "tax_included": true
  },
...
```json
{
  "template_id": "tmpl_8a3f7c2e-1b5d-4a9a-9e12-3f8a7b6c9d2e",
  "channel_id": "ch_yamato_001",
  "created_at": "2024-06-15T08:22:33.123Z",
  "created_by": "user_admin_jp",
  "updated_at": "2024-06-15T08:22:33.123Z",
  "status": "active",
  "validation_report": {
    "errors": [],
    "warnings": ["未配置tracking_api_endpoint，将无法自动同步物流状态"]
  }
}
```

##### 4.4 数据模型
- **`logistics_channels` 表**（物流渠道主表）
  - `id`: UUID PK
  - `tenant_id`: VARCHAR(32) NOT NULL（多租户隔离）
  - `channel_code`: VARCHAR(20) UNIQUE NOT NULL
  - `channel_name_ja`: VARCHAR(50) NOT NULL
  - `channel_name_en`: VARCHAR(50)
  - `carrier_type`: ENUM('domestic','international','hybrid') NOT NULL
  - `service_level`: ENUM('standard','express','economy','b2b_bulk') NOT NULL
  - `is_enabled`: BOOLEAN DEFAULT TRUE
  - `created_at`: DATETIME(3) NOT NULL
  - `updated_at`: DATETIME(3) NOT NULL

- **`logistics_templates` 表**（物流模板主表）
  - `id`: UUID PK
  - `channel_id`: UUID FK → `logistics_channels.id`
  - `tenant_id`: VARCHAR(32) NOT NULL
  - `template_name`: VARCHAR(60) NOT NULL
  - `priority`: TINYINT UNSIGNED NOT NULL CHECK (`priority` BETWEEN 1 AND 999)
  - `match_rules_json`: JSON NOT NULL（存储校验后的RPN元数据）
  - `fee_config_json`: JSON NOT NULL（加密存储敏感计费逻辑）
  - `label_format`: ENUM('a4_pdf','thermal_4x6','thermal_6x8','zpl_v2') NOT NULL
  - `customs_declaration_required`: BOOLEAN NOT NULL
  - `restricted_items`: JSON（字符串数组）
  - `status`: ENUM('active','inactive','pending_review') DEFAULT 'active'
  - `created_at`: DATETIME(3) NOT NULL
  - `created_by`: VARCHAR(32) NOT NULL
  - `updated_at`: DATETIME(3) NOT NULL

- **`logistics_template_audit` 表**（操作审计）
  - `id`: BIGINT PK AUTO_INCREMENT
  - `template_id`: UUID NOT NULL
  - `operator_id`: VARCHAR(32) NOT NULL
  - `action`: ENUM('create','update','delete','publish','rollback')
  - `before_state`: JSON（变更前快照）
  - `after_state`: JSON（变更后快照）
  - `ip_address`: VARCHAR(45)
  - `user_agent`: TEXT
  - `created_at`: DATETIME(3) NOT NULL

#### 5. 业务规则
1. **邮编覆盖强制规则**：当`carrier_type = "domestic"`且`service_level IN ("standard","express")`时，`zone_rules`必须覆盖全部47个都道府県的邮编前缀（如`060`→北海道札幌市），缺失任一前缀则校验失败。
2. **B2B订单强制报关**：若订单`customer_type = "b2b"`且任意SKU的`hs_code_jp`为空，则禁止匹配`customs_declaration_required = false`的模板，系统自动降级至租户级B2B默认模板。
3. **消费税一致性规则**：`fee_config.tax_included`必须与租户级税务配置中的`consumption_tax_mode`（`inclusive`/`exclusive`）保持一致，否则拒绝保存。
4. **模板互斥规则**：同一租户下，不允许两个`active`模板的`match_rules`存在完全重叠（即对同一订单同时满足），系统在保存时执行`MATCHES_ANY`逻辑检测并报错。
5. **时效承诺校验**：`service_level = "express"`的模板，其`match_rules`中必须包含`shipping_prefecture`字段约束（防止全国范围滥用特急服务）。
...
| 编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 | 优先级 |
|------|----------|----------|----------|----------|--------|
| TC01 | 创建合规的东京B2C特急模板 | 已存在`yamato_takkyubin`渠道且启用 | 1. 填写模板名「B2C-東京-翌日達」<br>2. 设置`priority=5`，`match_rules`限定`shipping_prefecture=["東京都"]`<br>3. `fee_config`配置首重850JPY含税<br>4. 点击「保存并校验」→「发布」 | 校验通过，返回`status=active`，`validation_report.errors=[]` | P0 |
| TC02 | 创建B2B模板但缺失HS编码校验 | 租户已启用B2B模式，SKU库中存在`hs_code_jp=null`的SKU | 配置`customs_declaration_required=true`的模板并发布 | 校验失败，`validation_report.errors`包含"HS编码缺失"提示 | P0 |
| TC03 | 邮编覆盖不全导致创建失败 | `carrier_type=domestic`，`zone_rules={"tokyo_23":0}`（仅配置东京） | 尝试保存该模板 | 校验失败，`errors`提示"未覆盖北海道、大阪府等46个地区邮编前缀" | P0 |
| TC04 | 多模板匹配优先级生效 | 已存在模板A（`priority=10`，匹配`prefecture=["東京都"]`）和模板B（`priority=5`，匹配`prefecture=["東京都","神奈川県"]`） | 创建收货地为东京的订单，触发匹配引擎 | 返回模板B的`template_id` | P1 |
| TC05 | 禁运品类拦截 | SKU标记`battery_type="li_ion"`且`battery_watt_hours=120`，模板配置`restricted_items=["batteries"]` | 该SKU加入订单并尝试匹配此模板 | 匹配失败，系统返回`fallback_template_id`并在日志记录"电池容量超100Wh，违反禁运规则" | P1 |

##### 6.2 边界测试
- 输入`priority=1000`（超出范围）应被后端拦截并返回400错误；
- `match_rules`嵌套深度达5层（AND→OR→AND→OR→AND）时，RPN解析仍能在50ms内完成；
- `fee_config.weight_rules`配置100个重量档位（0.1kg~10kg每0.1kg一档），运费计算耗时≤100ms；
- 同一租户下创建第1001个模板时，数据库插入不出现锁表或超时；
- `channel_code`输入含全角空格`　`或emoji（如🚀）时，前端JS校验立即报错，阻止提交。

##### 6.3 异常场景
| 异常场景 | 触发条件 | 系统处理方式 | 用户提示 |
|----------|----------|-------------|----------|
| 渠道已被删除 | 模板关联的`channel_id`在`logistics_channels`中`status=inactive` | 拒绝保存，事务回滚 | 「所选物流渠道已停用，请先启用渠道或选择其他渠道」 |
| Redis缓存失效 | RPN缓存Key过期后首次匹配请求 | 自动重建RPN并写入缓存，匹配延迟增加200ms | 无前端提示（后台告警） |
| 日本邮编格式错误 | `shipping_postal_code="1234567"`（缺短横线） | 地址标准化API返回`INVALID_POSTAL_CODE`，匹配引擎跳过该模板 | 「收货地址邮编格式错误，请修正后重试」（在打单页显红） |
| 运费计算溢出 | `base_fee=999999999`导致`final_fee > 99999999`（超日元最大整数） | 拦截保存，抛出`FeeOverflowException` | 「运费配置超出系统上限（99,999,999 JPY），请调整计费规则」 |
...
#### 1. 功能概述
- **业务目标**：解决日本跨境卖家在亚马逊日本站（Amazon.co.jp）、乐天市场（Rakuten Ichiba）、Yahoo! Shopping、Qoo10 Japan 及自建站（Shopify/EC-CUBE 日文版）等多平台分散沟通导致的响应延迟、漏回复、SLA超时、客服协同低效等问题；实现消息统一接入、语义归一、上下文关联、日文优先处理与合规拦截，支撑B2C快速响应（≤2小时）与B2B专业会话（含报价单/合同附件）双模态服务。
- **用户价值**：
  - 客服人员：单界面查看全部平台未读消息+历史会话，支持快捷回复模板、AI草稿生成、日文敏感词实时高亮、自动翻译（中↔日）、附件预览（PDF/Excel/图片），平均响应时间缩短47%（基准：乐天要求2h内首次响应）；
  - 管理者：通过消息来源分布热力图、平均首次响应时长（FRT）、SLA达标率、会话满意度（CSAT）埋点数据，定位服务瓶颈；
  - 合规团队：自动识别并拦截含“免税”“消费税非课税”等违规表述、未授权价格承诺、无资质医疗宣称等日文敏感内容，满足日本《景品表示法》《特定商取引法》监管要求。
- **使用角色**：一线客服专员（日语N1级）、客服主管、合规风控专员、运营总监（查看报表）
- **使用频率**：高频（客服专员日均操作≥50次，消息轮询间隔≤30秒）

#### 2. 输入参数
| 参数名 | 类型 | 必填 | 校验规则 | 说明 |
|--------|------|------|----------|------|
| `platform_code` | String | 是 | 枚举值：`amazon_jp`, `rakuten`, `yahoo_shopping`, `qoo10_jp`, `shopify_jp`, `eccube_jp` | 平台唯一编码，用于路由消息解析器与权限校验（需绑定租户已授权店铺） |
| `shop_id` | String | 是 | 非空，长度≤64，匹配`shop_config.id` | 对应店铺ID，用于归属店铺、计费分摊与SLA策略绑定 |
| `message_id` | String | 是 | UUID v4格式，全局唯一 | 原生平台消息ID（如Amazon的`messageId`、Rakuten的`mailId`），用于幂等去重与状态回传 |
| `sender_id` | String | 是 | 非空，长度≤128 | 发送方标识（买家ID/邮箱/手机号，经脱敏处理：如`user_***@rakuten.co.jp`） |
| `receiver_id` | String | 是 | 非空，长度≤128 | 接收方标识（本店客服账号或系统工单号） |
| `content` | String | 是 | UTF-8编码，长度≤10000；日文字符占比≥30%时触发日文NLP分析 | 原始消息正文（含HTML标签需清洗，保留换行与链接） |
| `content_type` | String | 是 | 枚举：`text`, `image`, `pdf`, `xlsx`, `zip` | 消息类型，附件需经病毒扫描与OCR（日文）后提取文本 |
| `timestamp` | Long | 是 | Unix毫秒时间戳，范围：`[1609459200000, 当前时间+300000]`（即2021-01-01至今+5分钟容错） | 消息在原平台产生时间，用于排序与SLA计算（以平台时间为准） |
| `is_customer_initiated` | Boolean | 是 | — | 是否买家主动发起（true=新会话起点，false=客服回复链） |
| `attachments` | Array<Object> | 否 | 最多5个，每个含`url`(String)、`filename`(String, ≤256)、`size_kb`(Integer, ≤50000) | 附件元数据，URL为OSS预签名地址（有效期24h），文件名需做日文Unicode标准化（NFKC） |

#### 3. 输出结果
| 输出项 | 类型 | 说明 |
|--------|------|------|
| `aggregated_message_id` | String | 系统生成的聚合消息唯一ID（格式：`AM-{tenant_id}-{YYYYMMDD}-{8位随机}`），用于全链路追踪 |
| `conversation_id` | String | 会话ID（格式：`CV-{platform_code}-{shop_id}-{normalized_sender_id}`），同一买家在同平台同店铺的全生命周期会话标识 |
| `normalized_sender` | Object | 归一化发送方信息：`{ "type": "individual"|"corporate", "name": "山田太郎", "contact": "yamada@xxx.co.jp", "region": "JP-KANAGAWA" }`（region来自日本邮编解析） |
| `sentiment_score` | Float | 情感分（-1.0~1.0），基于日文BERT模型计算，≤-0.3标记为“负面”，≥0.5标记为“积极” |
| `compliance_flags` | Array<String> | 合规风险标签：`["tax_misrepresentation", "unauthorized_medical_claim", "price_commitment"]` |
| `ai_suggestion` | Object | AI助手建议：`{ "reply_draft": "お問い合わせありがとうございます。在庫確認中です。", "translation": "Thank you for your inquiry. Checking stock.", "urgency_level": "high" }` |
| `slas` | Object | SLA履约信息：`{ "target_frt_minutes": 120, "elapsed_minutes": 42, "is_on_time": true, "next_deadline": 1735689200000 }` |

#### 4. 功能设计

##### 4.1 交互流程
1. **消息接入**：各平台Webhook/API定时拉取（Amazon MWS/SP API、Rakuten Mail API、Yahoo Shopping Messaging API）将新消息推送至`/api/v1/message/inbound`，携带签名验证（HMAC-SHA256）；
2. **前端加载**：客服登录后，WebSocket连接`/ws/messages?shop_ids=xxx,yyy`，实时接收`CONVERSATION_UPDATE`事件；
3. **会话列表页**：按`conversation_id`分组展示，置顶显示`is_customer_initiated=true`且`sentiment_score≤-0.3`的会话，右上角显示红点未读数；
4. **会话详情页**：左侧消息流（按`timestamp`升序），右侧显示买家画像（历史订单数、最近30天GMV、VIP等级）、AI建议框、SLA倒计时条；
5. **回复操作**：输入框支持插入模板（`/template order_status`）、调用AI草稿（点击`💡`）、上传附件（自动压缩≤5MB），提交后调用`POST /api/v1/message/outbound`；
6. **状态同步**：成功发送后，向原平台API回传已读/已回复状态（如Rakuten的`markAsRead`），更新本地`message_status=SENT`。

##### 4.2 核心逻辑
- **消息归一化引擎**：
  - 发送方标准化：对`sender_id`进行正则清洗（移除平台前缀如`rakuten_user_`），结合店铺配置的`buyer_contact_mapping`表（映射邮箱/手机号→买家ID）补全`normalized_sender`；
  - 会话聚类：采用`Levenshtein距离+时间窗口`算法——若两条消息`sender_id`相同、`platform_code+shop_id`相同、`timestamp`差≤15分钟，则归入同一`conversation_id`；跨平台（如Amazon+Rakuten）不聚合，避免法律主体混淆；
  - 日文NLP处理：调用内部微服务`jpn-nlp-service`，执行分词（MeCab+NEologd词典）、命名实体识别（公司名/商品名/地址）、情感分析（Japanese RoBERTa-base-finetuned-on-jnli）；
- **合规拦截规则**：
  - 实时匹配预置规则库（JSON Schema）：`{"pattern": "免税|消費税.*非課税", "severity": "block", "action": "mask_and_alert"}`；
  - 对`content`进行Unicode NFKC标准化后全文匹配，命中`block`级规则则拒绝入库，返回`400 Bad Request`并记录审计日志；
- **SLA计算逻辑**：
  - FRT（首次响应时间）= `outbound_message.timestamp - inbound_message.timestamp`（毫秒）；
  - 目标值由`shop_config.sla_policies`配置：`{"b2c": {"frt_minutes": 120}, "b2b": {"frt_minutes": 1440}}`，根据`normalized_sender.type`动态选择；
  - 超时预警：`elapsed_minutes > target_frt_minutes * 0.8`时，前端弹窗提醒+企业微信机器人推送。

##### 4.3 接口设计
| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| `InboundMessageReceive` | POST | `/api/v1/message/inbound` | 接收平台原始消息，幂等处理，返回`202 Accepted`或`400/401/403` |
| `OutboundMessageSend` | POST | `/api/v1/message/outbound` | 发送客服回复，校验SLA余量，同步至原平台，返回`201 Created`含`aggregated_message_id` |
| `ConversationList` | GET | `/api/v1/conversations` | 分页查询会话列表，支持`filter=unread/urgent/sentiment_negative`、`sort=last_message_time_desc` |
| `ConversationDetail` | GET | `/api/v1/conversations/{conversation_id}` | 获取会话详情及消息流，含买家画像与AI建议 |
| `MessageStatusUpdate` | PUT | `/api/v1/messages/{aggregated_message_id}/status` | 手动更新消息状态（如标记为`RESOLVED`），触发工单关闭 |

**`/api/v1/message/inbound` 请求示例**：
```json
{
  "platform_code": "rakuten",
  "shop_id": "shop_rakuten_001",
  "message_id": "mail_abc123xyz",
  "sender_id": "user_987654321",
  "receiver_id": "cs_team_jp",
  "content": "注文番号RKT20240001の発送状況を教えてください。",
  "content_type": "text",
  "timestamp": 1735685600000,
...
```json
{
  "code": 202,
  "data": {
    "aggregated_message_id": "AM-tenant_jp-20240101-a1b2c3d4",
    "conversation_id": "CV-rakuten-shop_rakuten_001-user_987654321",
    "normalized_sender": {
      "type": "individual",
      "name": "山田太郎",
      "contact": "yamada@example.co.jp",
      "region": "JP-TOKYO"
    },
    "sentiment_score": 0.12,
    "compliance_flags": [],
    "ai_suggestion": {
      "reply_draft": "ご注文番号RKT20240001の発送状況を確認いたします。少々お待ちください。",
      "translation": "We will check the shipping status of your order RKT20240001. Please wait a moment.",
      "urgency_level": "medium"
    },
    "slas": {
      "target_frt_minutes": 120,
      "elapsed_minutes": 0,
      "is_on_time": true,
      "next_deadline": 1735689200000
    }
  }
}
```
...
##### 4.4 数据模型
- **`message_aggregate`**（聚合消息主表）  
  `id` VARCHAR(64) PK, `tenant_id` VARCHAR(32) NOT NULL, `platform_code` VARCHAR(32) NOT NULL, `shop_id` VARCHAR(64) NOT NULL, `message_id` VARCHAR(128) NOT NULL, `conversation_id` VARCHAR(128) NOT NULL, `sender_id` VARCHAR(128), `receiver_id` VARCHAR(128), `content` TEXT, `content_type` VARCHAR(16), `timestamp` BIGINT NOT NULL, `is_customer_initiated` BOOLEAN DEFAULT FALSE, `status` ENUM('NEW','READ','SENT','RESOLVED','BLOCKED') DEFAULT 'NEW', `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP, `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP  

- **`conversation_summary`**（会话摘要表，物化视图）  
  `conversation_id` VARCHAR(128) PK, `latest_message_time` BIGINT, `unread_count` INT DEFAULT 0, `sentiment_avg` FLOAT, `last_sent_by` ENUM('customer','agent'), `slas_target_minutes` INT, `slas_elapsed_minutes` INT, `is_sla_breached` BOOLEAN  

- **`message_attachment`**（附件表）  
  `id` BIGINT PK AUTO_INCREMENT, `aggregated_message_id` VARCHAR(64) NOT NULL, `url` VARCHAR(512) NOT NULL, `filename` VARCHAR(256) NOT NULL, `size_kb` INT NOT NULL, `mime_type` VARCHAR(64), `ocr_text` TEXT, `scan_result` ENUM('clean','virus','suspicious') DEFAULT 'clean'  

- **`compliance_rule`**（合规规则表）  
  `id` BIGINT PK AUTO_INCREMENT, `rule_code` VARCHAR(64) UNIQUE, `pattern` TEXT NOT NULL, `severity` ENUM('info','warn','block') NOT NULL, `action` ENUM('log','mask','alert','block') NOT NULL, `description_ja` VARCHAR(256), `effective_from` DATE, `effective_to` DATE  

#### 5. 业务规则
1. **跨平台隔离原则**：同一买家在Amazon与Rakuten的消息绝不聚合为同一`conversation_id`，因法律主体（契约方）不同，避免混同责任；
2. **日文优先处理**：所有消息入库前强制执行Unicode NFKC标准化，并调用日文分词服务；若`content`中日文字符占比＜10%，自动触发`/api/v1/translate`调用DeepL API转译为日文再分析；
3. **SLA豁免机制**：当`normalized_sender.type=corporate`且`content`含关键词`「見積書依頼」`或`「契約書」`时，自动切换至B2B SLA（24小时），并在会话标题添加`[B2B]`标识；
4. **附件安全策略**：所有附件必须经ClamAV扫描，`scan_result != 'clean'`则拒绝入库并通知安全团队；PDF/Excel需OCR提取文本存入`message_attachment.ocr_text`供搜索；
5. **隐私脱敏规则**：`sender_id`在前端展示时自动脱敏（如`yamada@xxx.co.jp` → `yamada@***.co.jp`），完整信息仅客服主管及以上角色可点击查看（需二次密码认证）。
...
| 编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 | 优先级 |
|------|----------|----------|----------|----------|--------|
| TC01 | 正常Rakuten消息聚合与会话创建 | 租户已授权Rakuten店铺`shop_rakuten_001`；合规规则库启用 | 1. 调用`/api/v1/message/inbound`推送一条Rakuten新消息<br>2. 查询`conversation_summary`表 | `conversation_id`正确生成；`unread_count=1`；`sentiment_avg≈0.12` | P0 |
| TC02 | 跨平台消息不聚合验证 | 同一买家`user_987654321`在Rakuten和Amazon各发1条消息 | 1. 先推Rakuten消息，再推Amazon消息（相同`sender_id`）<br>2. 查询`message_aggregate`表 | 生成2条独立记录，`conversation_id`前缀分别为`CV-rakuten-`和`CV-amazon_jp-` | P0 |
| TC03 | 日文敏感词拦截（block级） | 合规规则库存在`{"pattern":"免税","severity":"block"}` | 1. 调用`/api/v1/message/inbound`发送含“免税”的消息<br>2. 检查响应码与数据库 | 返回`400 Bad Request`；`message_aggregate`表无新增记录；审计日志记录拦截事件 | P0 |
| TC04 | B2B会话SLA自动切换 | 消息`content`含`「見積書依頼」`且`sender_id`对应企业买家 | 1. 推送该消息<br>2. 查询`conversation_summary.slas_target_minutes` | `slas_target_minutes=1440`（24小时）；会话标题显示`[B2B]` | P1 |
| TC05 | 附件OCR与搜索验证 | 消息含PDF附件（含日文文本“在庫あり”） | 1. 推送带PDF的消息<br>2. 调用`/api/v1/conversations?search=在庫あり` | 返回包含该会话；`message_attachment.ocr_text`字段含“在庫あり” | P1 |

##### 6.2 边界测试
- 消息`content`长度=10000字符（UTF-8），验证截断与存储完整性；
- `timestamp`为2021-01-01 00:00:00（Unix时间戳1609459200000），验证历史消息兼容性；
- 同一`message_id`重复推送3次，验证幂等性（仅1条入库，后续返回`202`但不更新`updated_at`）；
- `attachments`数组为空、含1个、含5个附件（最大上限），验证附件处理稳定性；
- `sender_id`为纯数字`123456789`（无邮箱/手机号特征），验证`normalized_sender`生成逻辑（fallback为`{type:"individual", name:"匿名ユーザー"}`）。

##### 6.3 异常场景
| 异常场景 | 触发条件 | 系统处理方式 | 用户提示 |
|----------|----------|-------------|----------|
| 平台授权失效 | `shop_id`对应店铺的OAuth token过期 | 拒绝接收消息，记录`ERROR: shop_auth_expired`，触发告警通知运营 | “店铺授权已过期，请前往【店铺与渠道管理】重新授权” |
| 日文NLP服务不可用 | `jpn-nlp-service` HTTP 503 | 跳过NLP分析，`sentiment_score=0.0`，`compliance_flags=[]`，记录降级日志 | （无前端提示，后台自动降级） |
| 附件病毒扫描超时 | ClamAV响应>30秒 | 中断扫描，`scan_result='suspicious'`，禁止下载，提示“附件可能存在风险” | “该附件扫描超时，为安全起见暂不可下载，请联系管理员” |
| SLA计算溢出 | `elapsed_minutes`计算结果为负数（时钟漂移） | 强制设为`0`，记录`WARN: timestamp_drift_detected` | （无前端提示，确保SLA状态正常） |
| 多租户越权访问 | 非本租户`shop_id`调用`/api/v1/conversations` | 返回`403 Forbidden`，审计日志记录越权尝试 | “无权限访问该店铺数据” |
...
#### 1. 功能概述
- **业务目标**：解决日本跨境卖家在亚马逊JP、乐天市场（Rakuten）、Yahoo! Shopping等主流平台分散的评价/Review数据难以统一采集、实时预警、情感分析与协同响应的问题，避免差评发酵导致Listing降权、广告ACOS飙升及JIS合规风险（如《景品表示法》违规用语）。
- **用户价值**：
  - 实时聚合多平台商品级Review（含标题、正文、星级、时间、买家ID、是否Verified Purchase），支持日文NLP情感打分（正面/中性/负面）与敏感词高亮（如「偽り」「詐欺」等违反《景品表示法》词汇）；
  - 自动识别低分（≤2星）、含投诉关键词（「返金」「不良品」「届かない」）或涉法律风险（「個人情報漏洩」）的Review，触发SLA工单并推送企业微信/钉钉告警；
  - 支持按店铺、SPU、时间范围、星级分布、情感倾向进行多维下钻分析，生成月度口碑健康度报告（含竞品Review对比、日文文案合规率、客服响应时效达标率）；
  - 与「智能客服助手」联动，为客服人员自动生成日文回复草稿（含道歉模板、补偿方案、合规话术），并关联订单中心调取该买家历史履约记录（如物流延迟、退货记录）。
- **使用角色**：客服主管（配置监控规则与SLA）、一线客服专员（处理预警Review）、运营经理（分析口碑趋势）、合规专员（审核敏感内容）
- **使用频率**：高频（客服专员每日需处理≥50条新Review；运营经理每周生成1次分析报告）

#### 2. 输入参数
| 参数名 | 类型 | 必填 | 校验规则 | 说明 |
|--------|------|------|----------|------|
| `shop_ids` | string[] | 是 | 非空数组，元素为合法店铺ID（需存在于`shop_info`表且状态为`active`） | 指定监控的店铺列表，支持跨平台（如同时监控亚马逊JP与乐天店铺） |
| `spu_ids` | string[] | 否 | 若存在则每个SPU ID需存在于`pim_spu`表且`status='on_sale'` | 商品维度过滤，为空时监控店铺全部在售SPU |
| `review_platforms` | string[] | 是 | 值必须为`['amazon_jp', 'rakuten', 'yahoo_shopping', 'qoo10_jp']`子集 | 指定数据源平台，需与店铺已授权的平台类型匹配 |
| `start_time` | string (ISO8601) | 是 | ≥ 系统当前时间前30天，≤ 当前时间 | 监控起始时间（UTC+9），默认为`now-7d` |
| `end_time` | string (ISO8601) | 是 | > `start_time`，且 ≤ 当前时间 | 监控截止时间（UTC+9），默认为`now` |
| `sentiment_threshold` | number | 否 | 范围[0.0, 1.0]，默认0.3 | 情感分析置信度阈值，低于此值不标记情感倾向 |
| `star_filter` | number[] | 否 | 元素必须为[1,2,3,4,5]整数，长度≤5 | 星级筛选条件，如`[1,2]`仅拉取1-2星Review |

#### 3. 输出结果
| 输出项 | 类型 | 说明 |
|--------|------|------|
| `review_list` | array of object | Review原始数据集合，每条包含`review_id`, `platform`, `shop_id`, `spu_id`, `sku_id`, `star_rating`, `review_title`, `review_body`, `review_date`, `buyer_id`, `is_verified_purchase`, `review_url` |
| `sentiment_analysis` | object | 情感分析结果：`overall_score`（-1.0~1.0）、`sentiment_label`（"positive"/"neutral"/"negative"）、`confidence`（0.0~1.0）、`risk_keywords`（string[]，如["偽り","詐欺"]） |
| `compliance_check` | object | 合规检测结果：`is_compliant`（boolean）、`violation_type`（"keihin_hyojiho"/"personal_info_leak"/"none"）、`violated_terms`（string[]） |
| `related_orders` | array of object | 关联订单摘要：`order_id`, `order_date`, `shipping_status`, `return_count`, `last_logistics_event`（最近物流事件） |
| `auto_reply_suggestion` | string | AI生成的日文客服回复草稿（UTF-8编码，含敬语与合规免责条款），长度≤500字符 |

#### 4. 功能设计

##### 4.1 交互流程
1. 用户进入【客服与口碑管理】→【评价与Review监控】页面，系统自动加载当前租户下所有已授权店铺（`shop_info.status=active`且`auth_status=success`）；
2. 用户选择目标店铺（支持多选）、可选SPU（通过PIM商品树搜索）、平台类型、时间范围（默认近7天），点击【开始监控】；
3. 系统调用各平台API（亚马逊MWS/SP API、乐天RMS API、Yahoo Shopping API）拉取原始Review数据，同步至本地缓存库；
4. 后端启动异步任务链：  
   → 步骤1：调用日文NLP模型（基于BERT-Japanese微调）对`review_title`+`review_body`进行情感分析与关键词提取；  
   → 步骤2：匹配日本《景品表示法》《個人情報保護法》敏感词库（内置2,387个合规术语，每月OTA更新）；  
   → 步骤3：关联订单中心`order_master`表，查询该`buyer_id`近90天订单履约数据；  
   → 步骤4：根据预设SLA规则（如“2星以下Review需2小时内响应”）生成工单并推送到客服工作台；
5. 页面实时展示监控结果：左侧为Review列表（支持按星级/情感/风险标签筛选），右侧为分析看板（情感分布环图、风险词云、TOP5差评商品SPU）；
6. 用户点击某条Review，弹出详情浮层：显示原始内容、情感分析结果、合规检测报告、关联订单摘要，并提供【一键生成回复】按钮（调用AI助手生成日文草稿）及【转交工单】操作。

##### 4.2 核心逻辑
- **多平台数据归一化**：  
  定义统一Review Schema，将各平台差异字段映射为标准字段。例如：  
  - 亚马逊`verified_purchase` → `is_verified_purchase`（boolean）  
  - 乐天`review_date`（JST字符串）→ `review_date`（ISO8601 UTC+9）  
  - Yahoo Shopping无SKU字段 → 通过`item_id`反查PIM `sku_mapping`表补全`sku_id`
- **日文情感分析引擎**：  
  使用`bert-base-japanese-v3`模型（Fine-tuned on Japanese e-commerce review corpus of 120K samples），输入为`[CLS] + title + [SEP] + body + [SEP]`，输出3分类概率分布。当`max(prob)` < `sentiment_threshold`时标记为`sentiment_label="uncertain"`。
- **合规风险判定规则**：  
  - `keihin_hyojiho`：检测`review_body`中是否含《景品表示法》禁止词汇（如「最安」「史上初」）且出现频次≥1次；  
  - `personal_info_leak`：检测是否含日本个人情报定义字段（`電話番号`、`住所`、`メールアドレス`）且未脱敏（如未替换为`***`）；  
  - 所有检测均启用正则模糊匹配（支持平假名/片假名/汉字混写，如「でんわばんごう」匹配「電話番号」）。
- **SLA工单触发逻辑**：  
  满足任一条件即触发：  
  (1) `star_rating ≤ 2` AND `sentiment_label = "negative"`；  
  (2) `risk_keywords`非空；  
  (3) `review_body`含投诉动词（「返金希望」「交換依頼」「キャンセル要望」）且`review_date`在近24小时内。  
  工单自动分配给`role_code="customer_service_rep"`的在线客服，并设置`due_time = review_date + SLA_hours`（默认2小时）。

##### 4.3 接口设计
| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| `GET /api/v1/review/monitor/status` | GET | `/review/monitor/status?shop_ids[]=shop_001&shop_ids[]=shop_002` | 查询指定店铺当前监控任务状态（`idle`/`running`/`failed`）及最后成功拉取时间 |
| `POST /api/v1/review/monitor/execute` | POST | `/review/monitor/execute` | 启动一次手动监控任务，请求体为JSON（含`shop_ids`, `spu_ids`, `review_platforms`, `start_time`, `end_time`等） |
| `GET /api/v1/review/list` | GET | `/review/list?shop_id=shop_001&spu_id=spu_123&star=1,2&page=1&size=20` | 分页获取Review列表，支持多条件过滤，响应体含`review_list`, `sentiment_analysis`, `compliance_check`等字段 |
| `POST /api/v1/review/auto-reply` | POST | `/review/auto-reply` | 生成AI客服回复，请求体含`review_id`和`context_type`（"complaint"/"quality_issue"/"logistics_delay"），返回`auto_reply_suggestion` |

**示例请求（`/review/monitor/execute`）**：
```json
{
  "shop_ids": ["shop_amazon_jp_001", "shop_rakuten_002"],
  "spu_ids": ["spu_8899"],
  "review_platforms": ["amazon_jp", "rakuten"],
  "start_time": "2024-05-01T00:00:00+09:00",
  "end_time": "2024-05-08T23:59:59+09:00",
...
```json
{
  ...
  "data": {
    "review_list": [
      {
        "review_id": "amzjp_rev_7x9a",
        "platform": "amazon_jp",
        "shop_id": "shop_amazon_jp_001",
        "spu_id": "spu_8899",
        "sku_id": "sku_8899_blue_m",
        "star_rating": 1,
        "review_title": "全く届きません！",
        "review_body": "注文から15日経っても商品が届かず、カスタマーサポートの返信もありません。詐欺サイトでしょうか？",
        "review_date": "2024-05-05T14:22:33+09:00",
        "buyer_id": "B001234567",
        "is_verified_purchase": true,
        "review_url": "https://www.amazon.co.jp/.../ref=cm_cr_arp_d_viewpnt?ie=UTF8"
      }
    ],
    "sentiment_analysis": {
      "overall_score": -0.82,
    ...
    "compliance_check": {
      "is_compliant": false,
      "violation_type": "keihin_hyojiho",
      "violated_terms": ["詐欺"]
    },
    "related_orders": [
      {
        "order_id": "ORD-20240501-8899-001",
        "order_date": "2024-05-01T10:15:22+09:00",
        "shipping_status": "shipped",
        "return_count": 0,
        "last_logistics_event": "2024-05-03T16:45:00+09:00 - 出荷処理完了"
      }
    ],
    "auto_reply_suggestion": "お世話になっております。この度は大変ご迷惑をおかけし、誠に恐れ入ります。注文番号ORD-20240501-8899-001につきまして、現在物流状況を確認中です。本日中に追跡情報をご連絡いたします。※当社は個人情報保護法および景品表示法を厳守しております。"
  }
}
```

##### 4.4 数据模型
- **`review_raw`**（原始Review存储表）  
  `id` BIGINT PK, `review_id` VARCHAR(64) NOT NULL, `platform` ENUM('amazon_jp','rakuten','yahoo_shopping','qoo10_jp') NOT NULL, `shop_id` VARCHAR(32) NOT NULL, `spu_id` VARCHAR(32), `sku_id` VARCHAR(64), `star_rating` TINYINT CHECK (star_rating BETWEEN 1 AND 5), `review_title` TEXT, `review_body` TEXT, `review_date` DATETIME NOT NULL, `buyer_id` VARCHAR(64), `is_verified_purchase` BOOLEAN DEFAULT FALSE, `review_url` VARCHAR(512), `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP, `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP

- **`review_analysis`**（分析结果表）  
  `id` BIGINT PK, `review_id` VARCHAR(64) NOT NULL, `sentiment_score` DECIMAL(3,2) CHECK (sentiment_score BETWEEN -1.0 AND 1.0), `sentiment_label` ENUM('positive','neutral','negative','uncertain'), `confidence` DECIMAL(3,2) CHECK (confidence BETWEEN 0.0 AND 1.0), `risk_keywords` JSON, `compliance_violation` JSON, `ai_reply_suggestion` TEXT, `analysis_version` VARCHAR(16) DEFAULT 'v2.3.0', `analyzed_at` DATETIME DEFAULT CURRENT_TIMESTAMP

- **`review_monitor_task`**（监控任务表）  
  `id` BIGINT PK, `task_id` VARCHAR(40) NOT NULL, `tenant_id` VARCHAR(32) NOT NULL, `shop_ids` JSON NOT NULL, `spu_ids` JSON, `platforms` JSON NOT NULL, `time_range` JSON NOT NULL, `status` ENUM('pending','running','success','failed') DEFAULT 'pending', `error_message` TEXT, `started_at` DATETIME, `finished_at` DATETIME, `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP

#### 5. 业务规则
1. **数据时效性规则**：Review拉取任务必须在用户触发后5分钟内完成首次同步，后续增量同步间隔≤15分钟（通过平台Webhook或轮询实现），超时未完成自动重试3次，第4次失败则标记`status=failed`并邮件通知租户管理员。
2. **合规豁免规则**：若Review中出现敏感词但上下文为引用第三方内容（如「他社のレビューで『最安』と書いてありました」），且经人工复核确认为客观转述，则`compliance_violation.violation_type`设为`"none"`，并在`review_analysis.ai_reply_suggestion`中添加免责说明：“お客様が他社レビューを引用されたものと判断し、当社の表現とは異なります”。
3. **隐私保护规则**：所有Review数据在ERP系统内存储时，`buyer_id`字段必须AES-256加密（密钥由租户独立管理），前端展示时仅显示`B001****67`格式；导出报表时自动脱敏，不可还原原始ID。
4. **SLA升级规则**：若Review触发工单后2小时内未被客服领取，系统自动升级至客服主管；4小时内未处理，推送企业微信告警并同步至BI看板「SLA超时率」指标。
5. **多语言支持规则**：当Review语言检测为非日语（如中文、英文）时，强制调用内置翻译引擎（基于Transformer的JP↔CN/EN模型）生成日文摘要，并在`review_analysis`中存储`original_language`字段，确保客服可理解内容。
...
| 编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 | 优先级 |
|------|----------|----------|----------|----------|--------|
| TC01 | 多平台Review聚合成功 | 租户已授权亚马逊JP与乐天店铺，且两店铺均有1星Review | 1. 在监控页选择两店铺+全部平台+近1天时间<br>2. 点击【开始监控】 | `review_list`中包含来自`amazon_jp`和`rakuten`的Review，`platform`字段正确，总数≥2 | P0 |
| TC02 | 日文情感分析准确率 | Review正文为「最高でした！また買います！」 | 执行监控任务，查看该Review的`sentiment_analysis` | `sentiment_label="positive"`，`overall_score>0.7`，`confidence>0.9` | P0 |
| TC03 | 合规风险词高亮 | Review含「偽りの商品」 | 执行监控任务，查看该Review的`compliance_check` | `is_compliant=false`，`violation_type="keihin_hyojiho"`，`violated_terms=["偽り"]` | P0 |
| TC04 | SLA工单自动创建 | Review为2星且含「返金」，时间在24小时内 | 执行监控任务 | 系统在`ticket_master`表中创建新工单，`status="assigned"`，`assignee_role="customer_service_rep"`，`due_time`为当前时间+2小时 | P0 |
| TC05 | AI客服回复生成 | Review为「届かない」，关联订单物流状态为`shipped` | 点击该Review的【一键生成回复】 | `auto_reply_suggestion`包含日文，提及订单号、物流确认动作及合规免责条款，长度≤500字符 | P1 |

##### 6.2 边界测试
- 测试`start_time`为系统上线日（2023-01-01）且`end_time`为当前时间，验证大数据量（>10万条Review）下的分页查询性能（响应时间<3s）；
- 测试`review_body`长度达50,000字符（含大量emoji与换行符），验证NLP模型不崩溃且能正确截断处理；
- 测试`shop_ids`传入100个非法ID（如不存在的`shop_xxx`），验证接口返回`400 Bad Request`并提示“无效店铺ID：shop_xxx”。

##### 6.3 异常场景
| 异常场景 | 触发条件 | 系统处理方式 | 用户提示 |
|----------|----------|-------------|----------|
| 平台API限流 | 亚马逊SP API返回`429 Too Many Requests` | 自动退避重试（指数退避：1s→3s→9s），最多3次；失败后记录到`review_monitor_task.error_message`并标记`status=failed` | “亚马逊平台访问受限，请稍后重试或联系平台管理员调整API配额” |
| 敏感词库加载失败 | `/opt/erp/config/jp_compliance_dict.json`文件损坏 | 回退至内存缓存的上一版词库（`v2.2.0`），并触发后台告警通知运维 | “合规检测使用历史词库（v2.2.0），最新版词库加载异常，已自动修复” |
| NLP模型服务宕机 | `nlp-service:8080/analyze` HTTP连接超时 | 跳过情感分析，`sentiment_analysis`中`sentiment_label="uncertain"`，`confidence=0.0`，日志记录错误 | “情感分析服务暂不可用，已跳过分析，建议稍后刷新” |
...
- **用户价值**：
  - 客服专员平均单条消息处理时间缩短 62%（实测基准：从 180s → 68s）；
  - 日文回复合规率提升至 99.3%（自动规避消费税表述错误、敬语等级错配、敏感词、未授权促销用语等）；
  - SLA 24h 响应达标率从 76% 提升至 94%+；
  - 支持 B2B 批量询盘（如“見積り依頼”“納期確認”）与 B2C 个案咨询（如“返品方法”“不良品対応”）双模式识别与差异化应答。
- **使用角色**：客服专员（一线执行）、客服主管（模板审核/知识库管理）、运营经理（SLA看板监控）、系统管理员（AI模型配置）
- **使用频率**：高频（日均调用量 ≥ 5,000 次/万订单量级租户；单客服日均触发 80–120 次）

#### 2. 输入参数
| 参数名 | 类型 | 必填 | 校验规则 | 说明 |
|--------|------|------|----------|------|
| `message_id` | string | 是 | 非空、长度 ≤ 64、符合 UUID v4 格式 | 平台原始消息唯一ID（如 Rakuten 的 `msg_id`、Amazon MWS 的 `messageId`） |
| `platform_code` | string | 是 | 枚举值：`rakuten_jp`, `amazon_jp`, `yahoo_jp`, `shopee_jp`, `custom_api` | 消息来源平台编码，用于路由翻译策略与合规规则集 |
| `sender_id` | string | 是 | 非空、长度 ≤ 128 | 买家ID（Rakuten会员ID / Amazon buyer-id / Yahoo user-id） |
| `message_content` | string | 是 | 非空、UTF-8 编码、长度 ≤ 4096 字符、需通过日文敏感词初筛（调用 `/v1/compliance/sensitive-check`） | 原始买家咨询内容（日文为主，含少量英文/中文混杂） |
| `order_id` | string | 否 | 若存在，需匹配租户内有效订单号格式（支持 `RK-XXXXXX`, `AMZ-JP-XXXXX`, `YAH-YYYYYY` 等多平台前缀） | 关联订单号，用于上下文补全（如物流状态、退货政策） |
| `conversation_history` | array[object] | 否 | 最多 10 条，每条含 `role`（"user"/"assistant"）、`content`（≤1024字符）、`timestamp`（ISO 8601） | 近期对话上下文（含历史客服回复），用于多轮意图识别 |
| `customer_segment` | string | 否 | 枚举值：`b2c_individual`, `b2c_corporate`, `b2b_sme`, `b2b_large` | 买家客户分群标签（来自CRM同步或规则引擎打标），影响语气与条款严谨度 |
| `tenant_timezone` | string | 是 | IANA 时区标识符，如 `Asia/Tokyo` | 租户默认时区，用于时效类话术生成（如“本日中にお送りします”） |

#### 3. 输出结果
| 输出项 | 类型 | 说明 |
|--------|------|------|
| `suggestion_id` | string | 本次AI建议唯一ID（格式：`SUG-{tenant_id}-{unix_ts_ms}-{rand6}`） |
| `generated_reply` | string | AI生成的日文客服回复正文（已做敬语分级、无敏感词、含必要法律提示） |
| `confidence_score` | number | 置信度分数（0.0–1.0），≥0.85 为高置信，<0.65 建议人工重写 |
| `intent_class` | string | 识别的咨询意图（枚举：`return_request`, `shipping_inquiry`, `product_qa`, `price_promotion`, `invoice_request`, `b2b_quote`, `complaint`, `other`） |
| `compliance_flags` | array[string] | 触发的合规检查项（如 `["no_tax_mention", "no_guarantee_claim"]`），为空表示全部通过 |
| `suggested_templates` | array[object] | 推荐复用的客服模板ID及匹配度（例：`{"template_id": "TPL-RET-003", "match_score": 0.92}`） |
| `action_suggestions` | array[object] | 可联动操作建议（如 `{"action": "create_return_label", "params": {"order_id": "RK-202405001"}}`） |

#### 4. 功能设计

##### 4.1 交互流程
1. 客服专员在「客服工作台」打开某平台新消息卡片（如 Rakuten 站内信）；
2. 系统自动加载该消息元数据（`message_id`, `platform_code`, `sender_id`, `order_id`）及最近3条对话历史；
3. 页面右侧面板实时显示「AI助手建议」区域（3秒内返回）：含生成回复、置信度、意图分类、合规提示；
4. 专员可点击「插入模板」快速替换段落，或点击「润色优化」触发二次AI改写（指定风格：更礼貌/更简洁/更正式）；
5. 点击「发送」时，系统记录本次AI采纳行为（是否修改、修改字数、最终发送内容），用于模型反馈训练；
6. 若置信度 < 0.65，面板顶部显示黄色警示条：“AI建议置信度偏低，建议人工撰写”，并提供「转工单」快捷按钮。

##### 4.2 核心逻辑
- **多阶段意图理解流水线**：
  1. **平台语义归一化**：将各平台非结构化消息（如 Rakuten 的“返品手続きについて教えてください”、Amazon 的“Can I return this item?”）统一映射至标准意图ID（`return_request`）及子意图（`return_eligibility`, `return_label_generation`, `refund_method`）；
  2. **上下文增强检索（RAG）**：基于 `order_id` 查询订单状态、退货政策（WMS/订单中心API）、商品SPU属性（PIM）、历史相似咨询（向量数据库，embedding模型：`japanese-sentence-transformers-v2`）；
  3. **合规约束解码（Constrained Decoding）**：在LLM生成阶段硬性注入规则：
     - 消费税相关：禁止出现“税込価格”以外的税率描述，必须包含“消費税は別途必要です”（B2B场景）或“税込表示”（B2C场景）；
     - 敬语控制：根据 `customer_segment` 自动选择丁寧語（`〜ます/です`）、尊敬語（`お～になります`）、謙譲語（`～させていただきます`）三级策略；
     - 敏感词过滤：实时调用 `/v1/compliance/sensitive-check` 接口，对生成草稿逐句扫描（含同音异字变体，如“無料”→“むりょう”→“ムリョウ”）；
  4. **B2B/B2C差异化生成**：
     - B2B：强制包含公司抬头、联系人职位、报价有效期（默认30日）、付款条件（`振込先口座`）、交付方式（`PDF見積書添付`）；
     - B2C：启用情感补偿机制（检测到“不満”“残念”等词时，自动添加道歉句式+补偿选项）；
- **反馈闭环机制**：所有人工修改后的终版回复，经脱敏后存入强化学习样本池，每周触发一次 LoRA 微调（基座模型：`rinna/japanese-gpt-neox-3.6b-instruction-ppo`）。

##### 4.3 接口设计
| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| `POST /v1/customer-service/ai-suggest` | POST | `/v1/customer-service/ai-suggest` | 主请求接口：接收消息输入，返回AI建议 |
| `GET /v1/customer-service/templates/{template_id}` | GET | `/v1/customer-service/templates/{template_id}` | 获取模板详情（含变量占位符定义） |
| `POST /v1/customer-service/feedback` | POST | `/v1/customer-service/feedback` | 上报AI建议采纳行为（是否采纳、修改diff、耗时） |
| `POST /v1/compliance/sensitive-check` | POST | `/v1/compliance/sensitive-check` | 敏感词实时校验（同步阻塞调用） |

**`/v1/customer-service/ai-suggest` 请求示例**：
```json
{
  "message_id": "MSG-RK-202405001-8a9f",
  "platform_code": "rakuten_jp",
  "sender_id": "USR-RK-778291",
  "message_content": "注文番号RK-202405001の商品が届きません。配送状況を教えてください。",
  "order_id": "RK-202405001",
  "conversation_history": [
    {
      "role": "user",
      "content": "注文しましたが、まだ届いていません。",
      "timestamp": "2024-05-10T14:22:18+09:00"
    }
  ],
  "customer_segment": "b2c_individual",
...
```json
{
  "suggestion_id": "SUG-jp-tenant-001-1715352138123-abcd12",
  "generated_reply": "お問い合わせありがとうございます。\n\n注文番号 RK-202405001 の商品につきまして、現在は佐川急便にて配送中で、本日中に配達予定となっております。\n追跡番号は「KU123456789JP」で、佐川急便公式サイトよりご確認いただけます。\n\n万一明日までに届かない場合は、再度ご連絡ください。その際は、お手数ですが注文番号とお名前をお知らせいただければ幸いです。\n\n今後ともよろしくお願いいたします。",
  "confidence_score": 0.94,
  "intent_class": "shipping_inquiry",
  "compliance_flags": [],
  "suggested_templates": [
    {"template_id": "TPL-SHP-007", "match_score": 0.91}
  ],
  "action_suggestions": [
    {"action": "track_shipment", "params": {"tracking_number": "KU123456789JP"}}
  ]
}
```

##### 4.4 数据模型
- **`cs_ai_suggestion_log`**（AI建议日志表）  
  `id` BIGINT PK,  
  `tenant_id` VARCHAR(32) NOT NULL,  
  `suggestion_id` VARCHAR(64) UNIQUE NOT NULL,  
  `message_id` VARCHAR(64) NOT NULL,  
  `platform_code` VARCHAR(32) NOT NULL,  
  `order_id` VARCHAR(64),  
  `intent_class` VARCHAR(32),  
  `confidence_score` DECIMAL(3,2) CHECK (confidence_score BETWEEN 0.0 AND 1.0),  
  `generated_reply_hash` CHAR(64) COMMENT 'SHA256摘要，用于去重',  
  `is_adopted` TINYINT(1) DEFAULT 0 COMMENT '0=未采用，1=直接发送，2=修改后发送',  
  `edit_char_count` INT DEFAULT 0,  
  `response_latency_ms` INT NOT NULL,  
  `created_at` DATETIME(3) DEFAULT CURRENT_TIMESTAMP(3)

- **`cs_compliance_rule`**（合规规则表）  
  `id` INT PK,  
  `rule_code` VARCHAR(64) UNIQUE NOT NULL COMMENT '如 no_tax_mention',  
  `platform_scope` JSON COMMENT '适用平台列表 ["rakuten_jp","amazon_jp"]',  
  `segment_scope` JSON COMMENT '适用客户分群 ["b2c_individual"]',  
  `rule_expression` TEXT COMMENT '正则或逻辑表达式',  
  `violation_message_ja` TEXT NOT NULL COMMENT '日文违规提示',  
  `is_active` TINYINT(1) DEFAULT 1

- **`cs_template_variable`**（模板变量映射表）  
  `id` INT PK,  
  `template_id` VARCHAR(32) NOT NULL,  
  `variable_name` VARCHAR(64) NOT NULL COMMENT '如 {{tracking_number}}',  
  `data_source` ENUM('order','shipment','customer','static') NOT NULL,  
  `field_path` VARCHAR(128) COMMENT 'JSON路径，如 $.shipment.tracking_number'

#### 5. 业务规则
1. **敬语强制分级规则**：当 `customer_segment = 'b2b_large'` 时，所有回复必须使用最高级尊敬語（例：“お取り扱いいただきます”而非“お取り扱いになります”），且首句须含公司名（从CRM同步）；违反则 `compliance_flags` 加入 `"missing_honorific_level"`。
2. **B2B报价时效锁定**：对 `intent_class = 'b2b_quote'` 的请求，AI生成的报价单有效期必须为 `30日`（不可配置），且必须包含 `「本見積書は発行日より30日間有効です」` 法定声明；否则标记 `"invalid_quote_validity"`。
3. **退货政策强绑定**：若 `order_id` 存在且订单状态为 `shipped`，AI回复中涉及退货时，必须引用该订单对应仓库的退货地址（从 WMS API 实时获取），禁止使用静态模板地址；缺失则标记 `"missing_warehouse_return_address"`。
4. **多平台时效话术隔离**：Rakuten 消息禁止使用“即日発送”（违反其平台规则），必须用“当日中の発送”；Amazon JP 消息禁止使用“楽天市場”字样；违者触发 `"platform_brand_violation"`。
5. **隐私信息零回显**：AI生成内容中严禁出现买家真实姓名、电话、邮箱、地址全文；仅允许模糊化展示（如“山田○○様”、“東京都○○区”）；检测到则拦截并返回错误。
...
| 编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 | 优先级 |
|------|----------|----------|----------|----------|--------|
| TC01 | 正常物流查询（Rakuten + 有效订单） | 租户已接入Rakuten；订单RK-202405001状态为`shipped`且有有效追踪号 | 输入message_content="届きません"，order_id=RK-202405001 | `generated_reply`含正确追踪号与佐川急便官网链接；`intent_class=shipping_inquiry`；`confidence_score≥0.90` | P0 |
| TC02 | B2B报价请求（Yahoo! Shopping） | 租户CRM中客户`USR-YH-8899`标签为`b2b_sme`；无关联订单 | 输入message_content="見積り依頼"，platform_code=yahoo_jp | 回复含公司抬头、报价有效期30日声明、PDF附件提示；`compliance_flags=[]` | P0 |
| TC03 | 敏感词触发拦截（含“保証”） | 消息含“この商品は永久保証です” | 输入该消息，platform_code=amazon_jp | 返回`compliance_flags=["no_guarantee_claim"]`；`generated_reply`为空或替换为合规表述（“品質には自信がありますが、法定保証期間は1年となります”） | P0 |
| TC04 | 低置信度场景（模糊投诉） | message_content="なんか嫌な感じ"（无明确意图） | 发送该消息 | `confidence_score<0.65`；面板显示黄色警示条；`intent_class=other`；`suggested_templates`为空数组 | P1 |
| TC05 | 跨平台语义归一（Amazon英文问退货） | platform_code=amazon_jp；message_content="How to return?" | 输入该消息 | `intent_class=return_request`；生成日文回复含Rakuten退货流程（因租户主渠道为Rakuten）；非直译英文 | P1 |

##### 6.2 边界测试
- 输入 `message_content` 为纯空白字符（`"　"`、`\u3000`）或全角空格组合，验证接口返回 `400 Bad Request` 及明确错误码 `INVALID_MESSAGE_CONTENT`；
- `conversation_history` 传入11条记录（超限），验证系统自动截断为前10条，不报错；
- `tenant_timezone` 传入非法值（如 `"GMT+99"`），验证返回 `400` 并提示 `"Invalid timezone format"`；
- 并发压测：100 QPS 持续5分钟，`response_latency_ms` P95 ≤ 1200ms，错误率 < 0.1%；
- 极端长文本：`message_content` = 4096个日文字符（含emoji），验证生成回复不截断、无乱码、合规检查完整执行。

##### 6.3 异常场景
| 异常场景 | 触发条件 | 系统处理方式 | 用户提示 |
|----------|----------|-------------|----------|
| 平台API不可达 | 调用 `/v1/wms/warehouse-return-address` 超时（>3s） | 降级为使用租户默认退货地址，并记录告警日志；`compliance_flags` 添加 `"fallback_to_default_address"` | “倉庫情報取得に失敗したため、デフォルト返品先をご案内します。” |
| 合规服务异常 | `/v1/compliance/sensitive-check` 返回5xx | 跳过敏感词检查，但 `compliance_flags` 加入 `"compliance_service_unavailable"`；继续生成回复 | “一時的に法令チェックが利用できません。最終送信前にご確認ください。” |
| 模板ID不存在 | `suggested_templates` 中某 `template_id` 在DB中无记录 | 忽略该模板项，不中断主流程；日志记录 `TEMPLATE_NOT_FOUND` 事件 | 无前端提示（模板建议为可选功能） |
| 租户AI配额超限 | 当日调用量 > 套餐限制（如基础版5000次/日） | 返回HTTP 429；`generated_reply` 为空；记录配额超限审计日志 | “AIアシスタントの本日の利用上限に達しました。翌日0時にリセットされます。” |
| 意图识别冲突 | RAG检索返回矛盾信息（如订单状态为`delivered`但物流API返回`in_transit`） | 采用“强一致性优先”策略：以订单中心状态为准；`action_suggestions` 清空；`compliance_flags` 加入 `"data_conflict_resolved_by_order_center"` | 无前端提示（内部决策） |
...
#### 1. 功能概述
- **业务目标**：  
  满足日本《インボイス制度》（合格請求書等保存方式）强制合规要求，实现面向日本消费者的B2C订单及面向日本企业的B2B交易中消费税（消費税，JCT）的自动、准确、可审计计算，并生成符合国税厅（NTA）格式要求的电子/纸质消费税发票（合格請求書），支持与日本主流会计软件（如freee、マネーフォワード、弥生）对接预留，规避税务稽查风险与开票罚则（最高50万日元罚款+进项税额不可抵扣）。

- **用户价值**：  
  ✅ 自动识别交易类型（B2C/B2B）、买家资质（是否为登録事業者）、商品课税属性（課税/非課税/不課税/軽減税率対象）、发货地/收货地（判断是否属国内取引）、币种与汇率，零人工干预完成合规计税；  
  ✅ 一键生成含14项法定要素（如登録番号、取引日、課税区分、税率、税額明細等）的PDF/CSV格式合格請求書，支持批量导出与API回传至电商平台/物流系统；  
  ✅ 发票全生命周期追踪（草稿→已开具→已交付→已作废），与订单、收款、应付账款强关联，支撑月度/季度消费税申报表（確定申告書・消費税申告書）自动生成；  
  ✅ B2B场景下自动校验买家JCT登记号有效性（通过NTA公开API或本地缓存白名单），拦截无效开票请求，降低税务争议风险。

- **使用角色**：  
  财务专员（主操作）、财务主管（审批与复核）、运营经理（B2B客户信息维护）、IT管理员（发票模板与对接配置）、合规负责人（规则审计）

- **使用频率**：  
  高频（每日订单结算触发，B2C订单按单开票；B2B订单按合同/批次开票，平均日均300–2000张）

#### 2. 输入参数
| 参数名 | 类型 | 必填 | 校验规则 | 说明 |
|--------|------|------|----------|------|
| `order_id` | String | 是 | 非空、长度≤32、匹配`^[a-zA-Z0-9_-]{8,32}$` | 系统内唯一订单ID，关联订单中心数据 |
| `transaction_type` | Enum | 是 | ∈ `["B2C", "B2B"]` | 交易类型，决定开票逻辑与买家资质校验强度 |
| `buyer_jct_reg_no` | String | 否（B2C必为空，B2B必填） | 若非空：需满足`^T[0-9]{12}$`（日本JCT登记号格式）且通过NTA校验缓存或实时API验证 | B2B买家消费税登记编号（登録事業者番号），用于进项税抵扣凭证 |
| `shipping_address_prefecture` | String | 是 | ∈ 日本47都道府県代码列表（如`"tokyo"`、`"osaka"`） | 收货地址所属都道府県，用于判断是否适用軽減税率（食品・新聞紙）及国内取引判定 |
| `items` | Array[Object] | 是 | 非空，每项含`sku_code`, `quantity`, `unit_price_jpy`, `tax_category` | 商品明细数组，`tax_category` ∈ `["standard", "reduced", "exempt", "non_taxable", "import_duty"]` |
| `currency` | String | 是 | ∈ `["JPY", "USD", "CNY"]` | 订单结算币种，影响汇率换算与税基计算 |
| `exchange_rate_jpy_to_currency` | Decimal(10,6) | 是（当`currency ≠ "JPY"`时） | > 0，精度6位小数 | 结算日当日有效汇率（1 JPY = X currency），来源：财务模块汇率管理API |
| `invoice_issue_date` | Date | 否（默认为系统当前日期） | ≥ 订单支付成功时间，≤ 当前日期+7天 | 发票开具日期（請求日），须在取引发生后30日内 |
| `invoice_template_id` | String | 否（默认使用租户主模板） | 存在于`invoice_templates`表且`status = 'active'` | 自定义发票模板ID，支持多品牌/多法人差异化抬头与条款 |

#### 3. 输出结果
| 输出项 | 类型 | 说明 |
|--------|------|------|
| `invoice_id` | String | 全局唯一发票编号，格式：`INV-JP-{YYYYMMDD}-{6位随机大写字母数字}`（例：`INV-JP-20241015-AB3X9K`） |
| `invoice_status` | Enum | `["draft", "issued", "delivered", "voided", "rejected"]` |
| `total_amount_jpy` | Decimal(18,2) | 含税总金额（円），四舍五入至个位 |
| `tax_summary` | Object | 各税率段汇总：`{"standard": {"rate": 10.0, "base": 12500.00, "tax": 1250.00}, "reduced": {"rate": 8.0, "base": 8000.00, "tax": 640.00}}` |
| `pdf_url` | String | 生成的PDF发票文件直链（有效期7天，带JWT鉴权） |
| `csv_url` | String | 符合NTA CSV规范（JIS X 0208编码）的结构化发票数据下载链接 |
| `jct_declaration_data` | Object | 供消费税申报使用的结构化数据：`{"declaration_period": "2024Q3", "taxable_sales": 20500.00, "input_tax_credit": 0.00, "net_tax_payable": 1890.00}` |

#### 4. 功能设计

##### 4.1 交互流程
1. **触发入口**：  
   - 场景A（B2C）：订单中心完成“已发货”状态后，系统自动触发消费税计算与发票生成（后台任务）；  
   - 场景B（B2B）：财务专员在【财务→发票管理】页点击“新建B2B发票”，选择订单/采购单，填写买家JCT号并提交审核；  
2. **前端校验**：输入表单实时校验JCT号格式、地址有效性、商品税率配置完整性；  
3. **提交审核**（仅B2B）：提交至财务主管审批流（支持会签/或签），审批通过后进入开票队列；  
4. **后台处理**：  
   - 调用汇率服务获取当日JPY兑结算币种汇率；  
   - 将所有商品单价按汇率折算为JPY，按`shipping_address_prefecture`与`tax_category`分组应用税率（标准10% / 軽減8% / 非課税0%）；  
   - 校验B2B买家JCT号有效性（查本地缓存 → 缓存失效则调用NTA公开校验API `https://api.nta.go.jp/v1/jct/validate`）；  
   - 生成含14项法定字段的PDF（使用iText7 + 日文Noto Sans CJK字体）与CSV；  
   - 写入发票主表与明细表，更新订单状态为“已开票”；  
5. **交付通知**：  
   - B2C：自动邮件发送PDF发票至买家邮箱（模板可配置）；  
   - B2B：推送站内信+邮件，附PDF/CSV链接及交付确认按钮；  
6. **归档与同步**：发票元数据写入`jct_invoice_archive`表，同时向`accounting_system_webhook`推送JSON（含`invoice_id`, `pdf_url`, `tax_summary`）供freee等会计软件拉取。

##### 4.2 核心逻辑
- **税率动态判定引擎**：  
  ```python
  def get_tax_rate(item, prefecture):
      if item.tax_category == "reduced":
          # 軽減税率适用条件：①食品・新聞紙 ②国内取引（发货地=收货地）③非外食/非宅配便
          if (item.category in ["food", "newspaper"]) and \
             (item.shipping_origin_prefecture == prefecture) and \
             (item.fulfillment_type not in ["restaurant_delivery", "courier_only"]):
              return 8.0
      elif item.tax_category == "standard":
          return 10.0
      elif item.tax_category == "exempt":
          return 0.0  # 例：出口货物（非課税）
      elif item.tax_category == "non_taxable":
          return 0.0  # 例：土地转让（不課税）
      else:
          raise TaxCalculationError("Invalid tax_category")
  ```
- **B2B买家资质强校验**：  
  - 本地缓存JCT号有效性（TTL=24h），缓存命中直接通过；  
  - 缓存未命中时，调用NTA API（限流：100次/分钟/租户），响应含`valid: true/false`、`business_name`、`registration_date`；  
  - 若校验失败，发票状态置为`rejected`，记录`rejection_reason: "JCT number invalid or expired"`；  
- **汇率锁定机制**：  
  - 以`invoice_issue_date`为基准日，从`exchange_rates`表查询该日`JPY/{currency}`中间价；  
  - 若当日无数据，则向前查找最近有效汇率（最多追溯7日），否则报错终止；  
- **四舍五入规则**：  
  - 单品税额 = `round(unit_price_jpy * quantity * rate / 100, 0)`（个位四舍五入）；  
  - 总税额 = `sum(单品税额)`（非`round(总基数 * 税率)`），符合NTA实操要求。

##### 4.3 接口设计
| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| `POST /api/v1/jct/invoices` | POST | `/jct/invoices` | 创建新发票（B2C自动触发/B2B手动提交） |
| `GET /api/v1/jct/invoices/{invoice_id}` | GET | `/jct/invoices/{invoice_id}` | 查询发票详情（含PDF/CSV下载URL） |
| `PUT /api/v1/jct/invoices/{invoice_id}/deliver` | PUT | `/jct/invoices/{invoice_id}/deliver` | 标记发票为“已交付”（触发交付确认） |
| `DELETE /api/v1/jct/invoices/{invoice_id}` | DELETE | `/jct/invoices/{invoice_id}?reason=xxx` | 作废发票（需提供合规理由，如`"duplicate_issue"`） |
| `POST /api/v1/jct/invoices/batch-issue` | POST | `/jct/invoices/batch-issue` | 批量开票（按订单ID列表，限100条/次） |

**请求示例（POST `/jct/invoices`）**：
```json
{
  "order_id": "ORD-JP-2024-987654",
  "transaction_type": "B2B",
  "buyer_jct_reg_no": "T123456789012",
  "shipping_address_prefecture": "tokyo",
  "items": [
    {
      "sku_code": "SKU-ABC-001",
      "quantity": 2,
      "unit_price_jpy": 5000.00,
      "tax_category": "standard"
    }
  ],
  "currency": "USD",
  "exchange_rate_jpy_to_currency": 0.006852,
  "invoice_issue_date": "2024-10-15"
}
```
...
```json
{
  "invoice_id": "INV-JP-20241015-AB3X9K",
  "invoice_status": "issued",
  "total_amount_jpy": 11000,
  "tax_summary": {
    "standard": {"rate": 10.0, "base": 10000.00, "tax": 1000.00}
  },
  "pdf_url": "https://erp.example.com/invoices/INV-JP-20241015-AB3X9K.pdf?token=xxx",
  "csv_url": "https://erp.example.com/invoices/INV-JP-20241015-AB3X9K.csv?token=xxx",
  "jct_declaration_data": {
    "declaration_period": "2024Q3",
    "taxable_sales": 10000.00,
...
- **`jct_invoices`（发票主表）**  
  `id` BIGINT PK,  
  `invoice_id` VARCHAR(32) UK,  
  `tenant_id` VARCHAR(32) NOT NULL,  
  `order_id` VARCHAR(32) NOT NULL,  
  `transaction_type` ENUM('B2C','B2B') NOT NULL,  
  `buyer_jct_reg_no` VARCHAR(16),  
  `invoice_issue_date` DATE NOT NULL,  
  `status` ENUM('draft','issued','delivered','voided','rejected') DEFAULT 'draft',  
  `total_amount_jpy` DECIMAL(18,2) NOT NULL,  
  `currency` ENUM('JPY','USD','CNY') NOT NULL,  
  `exchange_rate` DECIMAL(10,6) NOT NULL,  
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,  
  `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP  

- **`jct_invoice_items`（发票明细表）**  
  `id` BIGINT PK,  
  `invoice_id` VARCHAR(32) FK → `jct_invoices.invoice_id`,  
  `sku_code` VARCHAR(64) NOT NULL,  
  `quantity` INT NOT NULL,  
  `unit_price_jpy` DECIMAL(18,2) NOT NULL,  
  `tax_category` ENUM('standard','reduced','exempt','non_taxable','import_duty') NOT NULL,  
  `tax_rate` DECIMAL(5,2) NOT NULL,  
  `tax_amount_jpy` DECIMAL(18,2) NOT NULL  

- **`jct_invoice_deliveries`（交付记录表）**  
  `id` BIGINT PK,  
  `invoice_id` VARCHAR(32),  
  `delivery_method` ENUM('email','api_push','manual_upload'),  
  `delivery_target` VARCHAR(255), // email or system name  
  `delivered_at` DATETIME,  
  `delivery_status` ENUM('success','failed','pending')  

#### 5. 业务规则
1. **B2C订单必须100%自动开票**：订单状态变为“已发货”后2小时内完成发票生成，超时触发告警（企业微信/钉钉通知财务组）；  
2. **B2B发票必须经双人审批**：财务专员提交 → 财务主管审批 → 系统自动校验JCT号 → 校验通过即开票，任一环节拒绝则状态为`rejected`并冻结该买家后续3单开票权限；  
3. **軽減税率严格受限**：仅当商品类目为`food`或`newspaper`、收货地与发货地同属一都道府県、且履约方式非`restaurant_delivery`或`courier_only`时方可启用8%税率，否则强制降级为10%；  
4. **汇率锁定不可覆盖**：发票开具后，即使汇率变动，已生成发票的`exchange_rate`字段不可修改，确保税务审计一致性；  
5. **作废发票需留痕**：`DELETE /jct/invoices/{id}`操作必须携带`reason`参数（枚举：`duplicate_issue`, `wrong_buyer_info`, `system_error`），日志留存≥7年，且原发票PDF URL立即失效。
...
| 编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 | 优先级 |
|------|----------|----------|----------|----------|--------|
| TC01 | B2C标准税率订单自动开票 | 订单`ORD-JP-2024-001`状态为“已发货”，商品为`standard`类目，收货地为`osaka` | 等待2分钟，查看发票列表 | 生成`INV-JP-20241015-XXXXXX`，`tax_summary.standard.rate=10.0`，`status=issued` | P0 |
| TC02 | B2B买家JCT号校验失败 | 提交B2B开票请求，`buyer_jct_reg_no="T000000000000"`（无效号） | 提交开票表单 | 返回400错误，`message="JCT number validation failed"`，发票状态=`rejected` | P0 |
| TC03 | 軽減税率正确触发 | 商品类目=`food`，`shipping_address_prefecture="kyoto"`，`fulfillment_type="warehouse_pickup"` | 提交B2C订单并发货 | 发票中`tax_summary.reduced.rate=8.0`，且`base`金额正确 | P0 |
| TC04 | 多币种汇率换算准确性 | 订单币种=`USD`，`unit_price_usd=100.00`，当日汇率=`0.006852`（即1 USD = 145.94 JPY） | 查看生成发票明细 | `unit_price_jpy=14594`（四舍五入），`tax_amount_jpy=1459`（10%） | P0 |
| TC05 | 发票作废与审计留痕 | 已开具发票`INV-JP-20241015-AB3X9K` | 调用`DELETE /jct/invoices/INV-JP-20241015-AB3X9K?reason=duplicate_issue` | 返回204，数据库`jct_invoices.status=voided`，`jct_audit_logs`新增记录含reason | P1 |

##### 6.2 边界测试
- 输入`exchange_rate_jpy_to_currency = 0.000001`（极端低汇率）：系统应拒绝并提示“汇率值异常，请检查配置”；  
- `items`数组含1000个SKU（超大批量）：接口响应时间≤8s，PDF生成不OOM，分页导出CSV；  
- `invoice_issue_date`设为订单支付日前1天：系统拦截并返回“开票日期不得早于交易发生日”；  
- `shipping_address_prefecture = "okinawa"`（偏远县）：仍正常计算，不触发特殊税率；  
- 并发100个B2B开票请求：系统限流（令牌桶算法），第101个返回429，其余成功生成且JCT校验串行化防重复。

##### 6.3 异常场景
| 异常场景 | 触发条件 | 系统处理方式 | 用户提示 |
|----------|----------|-------------|----------|
| NTA校验API不可用 | 调用`https://api.nta.go.jp/v1/jct/validate`超时（>5s）或返回5xx | 启用本地缓存兜底（若存在有效缓存），否则将发票置为`rejected`，记录`error_code="NTA_API_UNAVAILABLE"` | “JCT号校验暂时失败，请稍后重试或联系IT支持” |
| PDF生成失败（字体缺失） | 服务器未安装Noto Sans CJK字体 | 捕获iText异常，回滚事务，发票状态=`rejected`，触发告警工单 | “发票生成失败，请检查系统配置并重试” |
| 重复发票申请 | 同一`order_id`二次提交B2C开票 | 前置幂等校验（`SELECT COUNT(*) FROM jct_invoices WHERE order_id=? AND status IN ('issued','delivered')`），直接返回已有`invoice_id` | “该订单已开具发票：INV-JP-XXXXXX” |

### 财务与税务-日本会计软件对接预留

#### 1. 功能概述
- **业务目标**：为满足日本本地化财务合规要求，提前构建标准化、可扩展、高兼容性的会计系统对接能力，支持未来无缝接入主流日本会计软件（如 freee、マネーフォワード、弥生会計、勘定奉行、YAYOI 税務等），实现销售订单、采购入库、物流费用、消费税（消費税）、进项税抵扣、发票（適格請求書）及银行流水等核心财务数据的自动、双向、可审计同步，规避手工录入导致的税务风险与效率瓶颈。
- **用户价值**：  
  ✅ 减少财务人员80%以上重复性账务录入工作；  
  ✅ 确保消费税申报数据（課税売上/非課税売上/課税仕入/免税仕入）与JCT（適格請求書発行者登録）状态实时一致；  
  ✅ 支持多店铺（乐天/亚马逊/Japan Crate等）、多币种（JPY/CNY/USD）交易按日本税法自动拆分课税类型与税率（8%/10%）；  
  ✅ 提供对接日志、映射校验、失败重试、人工干预通道，满足日本国税厅（NTA）对电子账簿保存法（電子帳簿保存法）第12条“記録の正確性・改ざん防止”要求。
- **使用角色**：  
  - 财务管理员（租户内具备「财务-税务配置」权限的主账号或子账号）  
  - 实施顾问（ERP交付团队，负责对接参数配置与映射调试）  
  - 系统管理员（后台运维，监控对接服务健康度与审计日志）
- **使用频率**：低频（首次配置+年度合规升级时高频，日常为后台静默运行）

#### 2. 输入参数
| 参数名 | 类型 | 必填 | 校验规则 | 说明 |
|--------|------|------|----------|------|
| `tenant_id` | string | 是 | 非空、长度≤32、仅含字母数字与下划线 | 当前租户唯一标识，用于多租户数据隔离与权限控制 |
| `accounting_system_code` | string | 是 | 枚举值：`freee`, `moneyforward`, `yayoi`, `keisan`, `other` | 目标会计软件编码，决定适配器加载策略与字段映射模板 |
| `api_auth_type` | string | 是 | 枚举值：`oauth2`, `api_key`, `basic_auth` | 认证方式，影响凭证存储加密策略与token刷新逻辑 |
| `api_client_id` | string | 条件必填 | 若`api_auth_type=oauth2`则必填；长度≤128 | OAuth2 Client ID，用于freee/moneyforward等平台授权 |
| `api_client_secret` | string | 条件必填 | 若`api_auth_type=oauth2`则必填；长度≤128 | OAuth2 Client Secret，AES-256-GCM加密存储 |
| `api_access_token` | string | 条件必填 | 若`api_auth_type=oauth2`且已授权则必填；JWT格式校验 | 用户级访问令牌，有效期≤180天，自动续期需开启refresh_token机制 |
| `api_base_url` | string | 是 | URL格式、HTTPS协议、域名白名单校验（仅允许`*.freee.co.jp`, `*.moneyforward.com`, `*.yayoi-kk.co.jp`等） | 会计软件API根地址，区分沙箱（`sandbox.`）与生产环境 |
| `jct_registration_number` | string | 是 | 符合日本国税厅格式：`T{12位数字}` 或 `T{13位数字}`，正则`^T\d{12,13}$` | 适格請求書発行者登録番号，用于消费税发票类型判定与税务报表生成 |
| `tax_rate_mapping` | object | 是 | JSON Schema校验：包含`standard`(10%)、`reduced`(8%)、`exempt`、`non_taxable`四类，每类含`item_categories`数组（字符串枚举） | 定义ERP内商品类目/订单类型到日本消费税税率的映射关系，支持按`楽天市場カテゴリ`、`Amazon JP メインカテゴリ`、`自定义SKU标签`三级匹配 |
| `sync_scope` | array[string] | 是 | 元素为枚举：`sales_orders`, `purchase_invoices`, `bank_statements`, `tax_summary_reports` | 指定需同步的数据域，最小粒度控制数据流向与合规范围 |
| `sync_frequency` | string | 是 | 枚举：`realtime`, `hourly`, `daily_at_02:00`, `manual_only` | 同步触发策略，`realtime`需启用Webhook回调，`daily_at_02:00`按JST时区执行 |

#### 3. 输出结果
| 输出项 | 类型 | 说明 |
|--------|------|------|
| `config_id` | string | 对接配置唯一ID，格式：`AC-{tenant_id}-{timestamp(yyyyMMddHHmmss)}`，用于审计追踪 |
| `status` | string | 当前状态：`draft`（草稿）、`validating`（认证中）、`active`（已激活）、`error`（认证失败）、`disabled`（手动停用） |
| `last_sync_time` | datetime | 最后一次成功同步时间（JST时区），ISO8601格式，如`2025-04-05T02:00:00+09:00` |
| `sync_summary` | object | 同步统计摘要：`{ "sales_orders": {"success": 127, "failed": 0, "skipped": 3}, "tax_summary_reports": {"success": 1, "failed": 0} }` |
| `error_log_url` | string | 可下载的详细错误日志URL（带短期签名，有效期24h），含HTTP状态码、会计软件返回错误码（如freee的`40001`）、原始请求/响应payload脱敏片段 |
| `jct_compliance_status` | string | JCT合规状态：`registered`（已注册）、`pending_verification`（待国税厅审核）、`invalid_format`（格式错误）、`expired`（过期） |

#### 4. 功能设计

##### 4.1 交互流程
1. 财务管理员进入【财务与税务】→【日本会计软件对接】页面，点击「新建对接配置」；  
2. 选择目标会计软件（如`freee`），系统自动加载该厂商预置的OAuth2授权页URL与字段说明；  
3. 填写`JCT登録番号`、`同步范围`（默认勾选`sales_orders`+`tax_summary_reports`）、`同步频率`（默认`daily_at_02:00`）；  
4. 点击「验证连接」：前端跳转至freee授权页 → 用户登录并授予权限 → freee回调ERP系统 `/api/v1/accounting/callback/freee` → 后端完成token交换与账户信息拉取（公司名、事业所代码）；  
5. 验证通过后，显示「配置成功」，状态变为`active`，并自动生成首份`tax_summary_report`（覆盖上月1日至今）推送到freee；  
6. 管理员可在列表页查看各配置的`last_sync_time`、`sync_summary`，点击「详情」查看映射规则、错误日志、手动触发同步按钮。

##### 4.2 核心逻辑
- **双模认证适配器**：  
  - OAuth2模式（freee/moneyforward）：采用PKCE流程，`code_verifier`由前端生成并存于session，后端校验`code_challenge`；token存储加密使用租户专属密钥（KMS托管）；  
  - API Key模式（YAYOI）：Key经HMAC-SHA256签名后传输，后端校验签名时效性（≤5分钟）与nonce防重放。  
- **消费税智能拆分引擎**：  
  - 基于`jct_registration_number`判断是否启用`適格請求書`功能；  
  - 对每笔销售订单，按`tax_rate_mapping`逐级匹配：先查SKU绑定的`jct_tax_category`标签 → 无则查商品SPU所属`楽天カテゴリID` → 再无则查店铺渠道类型（亚马逊JP默认10%，乐天市场部分品类适用8%）；  
  - 自动拆分`課税売上`（含税金额）、`消費税額`（税额）、`適格請求書番号`（格式：`ERP-{tenant_id}-{order_id}-{jct_reg_no}`）。  
- **数据同步幂等性保障**：  
  - 所有同步请求携带`X-ERP-Request-ID: AC-{config_id}-{unix_ts_ms}-{rand6}`；  
  - 会计软件侧需支持`If-None-Match`头校验`ETag`（基于ERP订单号+更新时间哈希），避免重复记账；  
  - ERP侧维护`accounting_sync_log`表，记录`erp_record_id`、`accounting_record_id`、`sync_status`、`retry_count`（最大3次），失败后触发企业微信告警。  
- **电子账簿合规封装**：  
  - 向freee推送的`SalesInvoice`对象强制包含`electronic_invoice_flag: true`及`invoice_issue_date_jst`（JST时区）；  
  - 所有PDF发票附件经SHA-256哈希后存入`document_hash`字段，并在ERP侧保留原始文件≥7年（符合電子帳簿保存法第12条）。

##### 4.3 接口设计
| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| `POST /api/v1/accounting/configs` | POST | `/api/v1/accounting/configs` | 创建对接配置，请求体含全部输入参数，响应返回`config_id`与`status=validating` |
| `GET /api/v1/accounting/callback/{system}` | GET | `/api/v1/accounting/callback/{system}` | OAuth2回调入口，处理code交换token，重定向至配置页 |
| `POST /api/v1/accounting/configs/{config_id}/sync` | POST | `/api/v1/accounting/configs/{config_id}/sync` | 手动触发同步，支持query参数`scope=sales_orders&force=true`（强制全量） |
| `GET /api/v1/accounting/configs/{config_id}/logs` | GET | `/api/v1/accounting/configs/{config_id}/logs` | 分页查询同步日志，响应含`error_log_url`字段 |
| `PUT /api/v1/accounting/configs/{config_id}/status` | PUT | `/api/v1/accounting/configs/{config_id}/status` | 启用/禁用配置，请求体：`{"status": "active" \| "disabled"}` |

**示例请求（创建freee配置）**：
```json
{
  "tenant_id": "jp-ryoko-co-ltd",
  "accounting_system_code": "freee",
  "api_auth_type": "oauth2",
  "api_client_id": "f1a2b3c4d5e6f7g8h9i0j1k2l3m4n5o6",
  "api_client_secret": "x7y8z9a0b1c2d3e4f5g6h7i8j9k0l1m2",
  "api_base_url": "https://api.freee.co.jp",
  "jct_registration_number": "T123456789012",
  "tax_rate_mapping": {
    "standard": { "item_categories": ["楽天_家電", "Amazon_JP_Electronics"] },
    "reduced": { "item_categories": ["楽天_食品", "Amazon_JP_Grocery"] },
    "exempt": { "item_categories": ["楽天_医療機器"] },
    "non_taxable": { "item_categories": ["楽天_送料"] }
  },
  "sync_scope": ["sales_orders", "tax_summary_reports"],
  "sync_frequency": "daily_at_02:00"
}
```
...
```json
{
  "config_id": "AC-jp-ryoko-co-ltd-20250405020000",
  "status": "validating",
  "last_sync_time": null,
  "sync_summary": {},
  "error_log_url": null,
  "jct_compliance_status": "pending_verification"
}
```

##### 4.4 数据模型
- **`accounting_configs`**（对接配置主表）  
  `id` (BIGSERIAL, PK), `tenant_id` (VARCHAR(32), FK to tenants), `system_code` (VARCHAR(32)), `auth_type` (VARCHAR(20)), `client_id_encrypted` (TEXT), `client_secret_encrypted` (TEXT), `access_token_encrypted` (TEXT), `refresh_token_encrypted` (TEXT), `base_url` (VARCHAR(255)), `jct_reg_no` (VARCHAR(20)), `tax_rate_mapping_json` (JSONB), `sync_scope` (JSONB), `sync_frequency` (VARCHAR(32)), `status` (VARCHAR(20)), `created_by` (VARCHAR(64)), `updated_at` (TIMESTAMP WITH TIME ZONE), `jct_status` (VARCHAR(32))

- **`accounting_sync_logs`**（同步日志表）  
  `id` (BIGSERIAL, PK), `config_id` (VARCHAR(64), FK), `erp_record_type` (VARCHAR(32)), `erp_record_id` (VARCHAR(64)), `accounting_record_id` (VARCHAR(128)), `sync_direction` (VARCHAR(10) = 'erp_to_accounting'), `http_status` (SMALLINT), `error_code` (VARCHAR(32)), `error_message` (TEXT), `request_payload_hash` (CHAR(64)), `retry_count` (SMALLINT DEFAULT 0), `created_at` (TIMESTAMP WITH TIME ZONE)

- **`jct_compliance_audit`**（JCT合规审计表）  
  `id` (BIGSERIAL, PK), `tenant_id` (VARCHAR(32)), `jct_reg_no` (VARCHAR(20)), `verification_status` (VARCHAR(20)), `verified_at` (TIMESTAMP WITH TIME ZONE), `nta_response_xml` (TEXT), `is_valid` (BOOLEAN), `expires_at` (DATE)

#### 5. 业务规则
1. **JCT强校验规则**：任何`jct_registration_number`必须通过日本国税厅公开API（`https://www.nta.go.jp/.../jct_check`）实时验证有效性，若返回`status=invalid`或超时（>5s），配置状态强制置为`error`且禁止激活。  
2. **汇率锁定规则**：所有推送到会计软件的金额（JPY/CNY/USD）必须使用订单创建时刻的ERP内置汇率快照（`exchange_rate_snapshot`），禁止使用同步时刻实时汇率，确保税务报表可追溯。  
3. **数据不可篡改规则**：已同步至会计软件的销售订单，若ERP侧发生`订单取消`或`金额修改`，必须生成独立的`credit_note`（贷项凭证）同步至会计软件，而非直接更新原记录，满足電子帳簿保存法第12条「記録の改ざん防止」。  
4. **多店铺税号隔离规则**：同一租户下多个日本店铺（如乐天A店、亚马逊JP店）若使用不同JCT登録番号，必须创建独立对接配置，禁止跨店铺共享`jct_reg_no`。  
5. **失败熔断规则**：单日同步失败次数≥50次，或连续3次`sync_frequency=daily_at_02:00`失败，系统自动将`status`置为`disabled`并发送邮件告警至财务管理员与实施顾问。
...
| 编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 | 优先级 |
|------|----------|----------|----------|----------|--------|
| TC01 | 创建freee OAuth2配置并验证成功 | 租户`jp-ryoko-co-ltd`已存在；freee沙箱环境可用 | 1. 填写freee client_id/client_secret<br>2. 输入有效JCT号`T123456789012`<br>3. 点击「验证连接」并完成OAuth授权 | 状态变为`active`；`jct_compliance_status=registered`；首份`tax_summary_report`成功推送 | P0 |
| TC02 | JCT号格式错误导致配置失败 | 同TC01 | 输入JCT号`T1234567890`（仅11位） | 状态为`error`；错误提示「JCT登録番号格式不正确：需以T开头，后接12或13位数字」 | P0 |
| TC03 | 多币种订单同步至freee | TC01配置已激活；存在一笔USD结算订单（含JPY汇率快照） | 手动触发`sync_scope=sales_orders` | freee中生成`SalesInvoice`，`amount`字段为JPY金额，`currency`=`JPY`，`exchange_rate`字段等于ERP快照值 | P0 |
| TC04 | 消费税税率自动匹配 | TC01配置中`tax_rate_mapping`已配置`楽天_食品→reduced` | 创建乐天店铺订单，商品类目为`楽天_食品` | 推送至freee的发票中`tax_rate=8`，`tax_amount`精确计算（含税价×8/108） | P0 |
| TC05 | 同步失败后重试机制 | freee API临时返回503 | 手动触发同步 → 观察`accounting_sync_logs`表 | `retry_count`从0→1→2→3；第3次失败后`status`仍为`active`，但触发企业微信告警 | P1 |

##### 6.2 边界测试
- 输入`jct_registration_number`为`T12345678901234`（14位）——应拒绝并提示格式错误  
- `sync_frequency=daily_at_02:00`时，系统时区为UTC+0，但需强制按JST（UTC+9）执行，验证任务调度是否偏移9小时  
- `tax_rate_mapping`中`item_categories`数组长度达200项（模拟大型卖家全类目映射），验证匹配性能是否<200ms  

##### 6.3 异常场景
| 异常场景 | 触发条件 | 系统处理方式 | 用户提示 |
|----------|----------|-------------|----------|
| freee OAuth2 token刷新失败 | refresh_token过期且`api_auth_type=oauth2` | 自动将`status`置为`error`，停止所有同步任务，记录`error_code=OAUTH_REFRESH_FAILED` | 「与freee的连接已失效，请重新授权」+「立即重新授权」按钮 |
| 会计软件返回409冲突（Duplicate Invoice） | freee检测到相同`invoice_number`已存在 | 跳过该笔记录，`sync_summary.failed++`，记录`error_code=DUPLICATE_INVOICE` | 后台日志标记「跳过重复发票：ERP-xxx-xxx-T123456789012」，不中断整体同步 |
| JCT登録番号被国税厅注销 | NTA API返回`status=revoked` | 立即置`jct_compliance_status=expired`，`status=disabled`，冻结所有消费税相关同步 | 「JCT登録番号已失效，请联系国税厅更新」+「联系客服」链接 |
...
- **用户价值**：
  ...
  - 财务人员提前3天获知“某SKU消费税发票未生成，即将影响JCT进项抵扣申报”；
  - 管理层通过预警热力图快速掌握全集团高风险模块（如“广告模块预警量周环比+65%，集中于雅虎购物关键词竞价失控”）。
- **使用角色**：运营总监、仓储主管、广告优化师、客服经理、财务专员、采购经理、系统管理员（配置者）、租户管理员（租户级预警策略管理者）
- **使用频率**：高频（预警规则执行为每5分钟定时扫描 + 实时事件触发；人工查看预警看板日均≥3次；规则配置中频，平均每周1.2次）

#### 2. 输入参数
| 参数名 | 类型 | 必填 | 校验规则 | 说明 |
|--------|------|------|----------|------|
| `tenant_id` | string | 是 | 非空、长度≤32、符合`^[a-z0-9]{8}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{12}$` UUIDv4格式 | 租户唯一标识，用于多租户数据隔离与策略归属 |
| `rule_code` | string | 是 | 非空、长度2–20、仅含字母/数字/下划线、以字母开头 | 预警规则编码，全局唯一，如`STOCK_LOW_RAKUTEN_JP`、`AD_ROI_DROP_AMAZON_JP` |
| `rule_name` | string | 是 | 非空、长度≤50、UTF-8日文兼容 | 规则中文/日文名称，如「乐天店铺库存低于安全线预警」 |
| `rule_type` | enum | 是 | ∈ `{order, inventory, ad, logistics, customer_service, finance, product, listing, tax, compliance}` | 预警所属业务域，决定可选指标与数据源 |
| `trigger_condition` | object | 是 | JSON Schema校验（见4.3接口设计） | 条件表达式结构体，支持单指标阈值、多指标组合（AND/OR）、同比/环比变化率、时间窗口聚合 |
| `trigger_frequency` | enum | 是 | ∈ `{realtime, 5min, 15min, 1h, 1d}` | 扫描执行频率，`realtime`仅对订单创建、库存变更等关键事件触发 |
| `notify_channels` | array[string] | 否 | 每项∈ `{email, in_app, wecom, slack, webhook}`，数组长度≤5 | 通知渠道列表，空则仅存档不通知 |
| `notify_users` | array[string] | 否 | 每项为有效`user_id`（子账号ID），长度≤20 | 接收人列表，支持按角色（如`role:finance_manager`）动态解析 |
| `severity` | enum | 是 | ∈ `{low, medium, high, critical}` | 风险等级，影响通知优先级与UI颜色（critical=红色闪烁） |
| `description` | string | 否 | 长度≤200，UTF-8日文兼容 | 规则业务说明，如「当FBA仓SKU在售数量<安全库存×1.2且未来3天预测销量>当前库存时触发」 |
| `enabled` | boolean | 是 | — | 是否启用该规则，默认`true` |

#### 3. 输出结果
| 输出项 | 类型 | 说明 |
|--------|------|------|
| `alert_id` | string | 预警实例唯一ID，格式`ALERT_{tenant_id}_{timestamp_ms}_{seq}`，如`ALERT_tnt-abc123-xyz_1718234567890_001` |
| `rule_code` | string | 关联的规则编码 |
| `rule_name` | string | 关联规则名称（日文/中文） |
| `severity` | enum | 当前预警等级（可能与规则定义不同，如规则为medium但实际指标超阈值200%则升为high） |
| `trigger_time` | datetime | 触发时间（ISO 8601，JST时区，`YYYY-MM-DDTHH:mm:ss+09:00`） |
| `trigger_value` | object | 实际触发值快照，含`current_value`、`threshold_value`、`comparison_type`（`gt/lt/gte/lte/percent_change`）等字段 |
| `affected_entities` | array[object] | 受影响业务实体列表，每项含`type`（`sku`, `shop`, `ad_group`, `warehouse`, `invoice`等）、`id`、`name`（日文名） |
| `suggestion` | string | AI生成的处置建议（日文），如「请立即检查SKU『ABC-123』在乐天店铺『XX旗舰店』的库存同步状态，并确认FBA仓入仓计划」 |
| `status` | enum | 当前状态：`active`（待处理）、`acknowledged`（已确认）、`resolved`（已解决）、`ignored`（已忽略） |
| `acknowledged_by` | string | 确认人`user_id`（可为空） |
...
2. 点击「新建规则」，选择业务域（如`inventory`），系统动态加载该域可用指标（如`warehouse_stock_quantity`, `safety_stock_level`, `forecasted_sale_7d`）及运算符；
3. 配置条件：拖拽`warehouse_stock_quantity < safety_stock_level × 1.3`，设置`trigger_frequency=5min`，勾选`notify_channels=["in_app","wecom"]`，指定`notify_users=["u-1001","u-1002"]`；
4. 点击「保存并启用」，系统校验语法与权限后返回成功提示；
5. 系统后台每5分钟执行一次扫描：查询`inventory_realtime_view`（物化视图），对每个仓库+SKU组合计算`warehouse_stock_quantity / safety_stock_level`比值；
6. 当发现比值<0.77（即<1/1.3）时，生成`alert_id`，填充`affected_entities`（含仓库名、SKU日文名、当前库存数），调用AI引擎生成日文处置建议；
7. 向Wecom机器人推送消息（含预警标题、详情链接、一键确认按钮），同时在用户「我的预警」看板顶部红点提示；
8. 用户点击预警卡片，查看详情 → 点击「已确认」按钮 → 状态变更为`acknowledged` → 可填写处理备注 → 系统记录操作日志。

##### 4.2 核心逻辑
- **多源数据融合**：预警引擎不直接查原始表，而是基于预计算的**宽表视图**（如`bi_alert_inventory_snapshot_jp`）运行，该视图每日凌晨ETL更新，并每5分钟增量刷新关键字段（库存、销量、广告花费）。视图已预关联：`sku_info`（含日文品名）、`warehouse_info`（含日文仓名）、`shop_info`（含平台类型、店铺名）、`tax_config`（含消费税适用标志）。
- **动态阈值计算**：支持三种阈值模式：
  - 固定值（如`库存<50`）；
  - 表达式（如`safety_stock_level * 1.3`，其中`safety_stock_level`来自WMS模块的`warehouse_sku_config`表）；
  - 自适应（如`7d_avg_sale_quantity * 2.5`，由BI模块的`sales_forecast_daily`模型提供）。
- **实时事件钩子**：对`order_created`, `inventory_updated`, `ad_spend_report_synced`等关键事件，通过Kafka Topic广播，预警服务监听并即时评估是否满足`trigger_condition`（仅适用于`trigger_frequency=realtime`的规则）。
- **AI建议生成**：调用内部`ai-alert-suggestion-service`，输入`alert_id`及上下文（`affected_entities`, `trigger_value`, `rule_description`），返回日文建议。模型训练数据含10万+历史预警工单处置方案，支持领域术语（如「インボイス未発行」「FBA入荷遅延」）。
- **去重与抑制**：同一SKU在1小时内重复触发相同规则（仅`trigger_value`微调）时，合并为一条预警并更新`trigger_value`；若`rule_code`含`_TAX_`前缀，则自动抑制所有`inventory`类低库存预警（避免税务问题被库存问题掩盖）。

##### 4.3 接口设计
| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| `CreateAlertRule` | POST | `/api/v1/tenants/{tenant_id}/alert-rules` | 创建预警规则 |
| `ListAlertRules` | GET | `/api/v1/tenants/{tenant_id}/alert-rules` | 分页查询规则列表（支持`rule_type`, `enabled`, `search`） |
| `GetAlertRule` | GET | `/api/v1/tenants/{tenant_id}/alert-rules/{rule_code}` | 获取单条规则详情 |
| `UpdateAlertRule` | PUT | `/api/v1/tenants/{tenant_id}/alert-rules/{rule_code}` | 更新规则（禁用/修改条件等） |
| `ListAlerts` | GET | `/api/v1/tenants/{tenant_id}/alerts` | 查询预警列表（支持`status`, `severity`, `start_time`, `end_time`, `rule_code`过滤） |
| `AcknowledgeAlert` | POST | `/api/v1/tenants/{tenant_id}/alerts/{alert_id}/acknowledge` | 确认预警（请求体含`remark: string`） |
| `ResolveAlert` | POST | `/api/v1/tenants/{tenant_id}/alerts/{alert_id}/resolve` | 标记为已解决（请求体含`resolution_summary: string`） |

**`CreateAlertRule` 请求示例（JSON）**：
```json
{
  "rule_code": "TAX_INVOICE_MISSING_JP",
  "rule_name": "消費税インボイス未発行警告",
  "rule_type": "tax",
  "trigger_condition": {
    "metric": "unissued_invoice_count",
    "operator": "gt",
    "value": 0,
    "time_window": "1d",
    "aggregation": "sum"
  ...
  "notify_users": ["role:finance_manager"],
  "severity": "high",
  "description": "過去24時間に消費税対応インボイスが未発行の注文が存在する場合に警告",
  "enabled": true
}
```

**`ListAlerts` 响应示例（JSON）**：
```json
{
  "data": [
    {
      "alert_id": "ALERT_tnt-abc123-xyz_1718234567890_001",
      "rule_code": "STOCK_LOW_RAKUTEN_JP",
      "rule_name": "楽天市場在庫不足警告",
      "severity": "critical",
      "trigger_time": "2024-06-15T09:22:47+09:00",
      "trigger_value": {
        "current_value": 12,
        "threshold_value": 35,
        "comparison_type": "lt"
      ...
      "suggestion": "SKU『防水スマートウォッチ（ブラック）』の楽天市場店在庫が安全在庫を大幅に下回っています。FBA倉庫への入荷スケジュールを確認し、緊急補充を検討してください。",
      "status": "active",
...
- **`alert_rule`**（预警规则主表）
  - `id`: BIGSERIAL PK
  - `tenant_id`: VARCHAR(32) NOT NULL, FK → `tenant.id`
  - `rule_code`: VARCHAR(20) NOT NULL, UNIQUE
  - `rule_name`: VARCHAR(50) NOT NULL
  - `rule_type`: VARCHAR(20) NOT NULL — ENUM约束
  - `trigger_condition`: JSONB NOT NULL — 存储条件表达式
  - `trigger_frequency`: VARCHAR(10) NOT NULL
  - `notify_channels`: JSONB — 存储字符串数组
  - `notify_users`: JSONB — 存储字符串数组或角色标识
...
- **`alert_instance`**（预警实例表）
  - `id`: VARCHAR(64) PK — `alert_id`
  - `tenant_id`: VARCHAR(32) NOT NULL
  - `rule_code`: VARCHAR(20) NOT NULL, FK → `alert_rule.rule_code`
  - `severity`: VARCHAR(10) NOT NULL
  - `trigger_time`: TIMESTAMPTZ NOT NULL
  - `trigger_value`: JSONB NOT NULL
  - `affected_entities`: JSONB NOT NULL — 数组，每项含`type`,`id`,`name`
  - `suggestion`: TEXT
  - `status`: VARCHAR(20) DEFAULT 'active' — ENUM约束
...
- **`alert_notification_log`**（通知日志表）
  - `id`: BIGSERIAL PK
  - `alert_id`: VARCHAR(64) NOT NULL, FK → `alert_instance.id`
  - `channel`: VARCHAR(20) NOT NULL
  - `recipient`: VARCHAR(255) NOT NULL — email地址/Wecom UserID等
  - `status`: VARCHAR(20) NOT NULL — `sent`, `failed`, `ignored`
  - `sent_at`: TIMESTAMPTZ
  - `error_message`: TEXT

#### 5. 业务规则
1. **日本税务强约束**：所有`rule_type=tax`的预警必须关联日本消费税（JCT）合规检查，当`unissued_invoice_count > 0`且`order_created_at >= '2023-10-01'`（インボイス制度生效日）时，`severity`强制设为`critical`，且`notify_channels`必须包含`email`（法律留痕要求）。
2. **多平台差异化阈值**：同一规则（如库存预警）在不同平台需独立配置阈值——乐天市场要求`stock_ratio < 0.8`即告警，而亚马逊FBA因物流周期长，阈值为`stock_ratio < 0.4`；系统通过`affected_entities.type=shop`动态绑定平台类型。
3. **预警抑制链**：当`STOCK_OUT_OF_STOCK_JP`（缺货）预警触发时，自动抑制同SKU的`AD_BUDGET_WASTE_JP`（广告预算浪费）预警（因无货时广告无效），抑制关系在`alert_rule.suppresses`字段以JSON数组存储被抑制的`rule_code`。
4. **日文本地化强制**：所有`rule_name`、`description`、`suggestion`字段在入库前必须通过`japanese_text_validator`服务校验——禁止出现半角片假名/平假名混排、禁止使用非JIS标准汉字（如「剰」应为「余」）、敏感词库（含政治、宗教词汇）命中则拒绝保存。
5. **租户数据硬隔离**：`alert_instance`表的`tenant_id`为分区键，按月分表（`alert_instance_202406`），且所有查询WHERE条件必须包含`tenant_id = ?`，否则SQL拦截器抛出`TenantAccessViolationError`。
...
| 编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 | 优先级 |
|------|----------|----------|----------|----------|--------|
| TC01 | 创建乐天库存预警规则并触发 | 1. 租户`tnt-jp-001`存在；2. SKU `SKU-JP-1001`在乐天店铺`SHOP-RK-001`安全库存=100；3. 当前库存=75 | 1. 创建规则：`rule_code=STOCK_LOW_RK_JP`, `trigger_condition={metric:"warehouse_stock_quantity", operator:"lt", value:80}`；2. 手动触发扫描任务；3. 查询`alert_instance` | 生成1条`alert_id`，`affected_entities`含`SKU-JP-1001`日文名，`suggestion`为日文 | P0 |
| TC02 | 税务预警自动升级为critical | 1. 存在`TAX_INVOICE_MISSING_JP`规则；2. 有1笔2024年订单未开消费税发票 | 执行日级扫描 | 生成预警且`severity=critical`，`notify_channels`含`email` | P0 |
| TC03 | Wecom通知发送与确认 | 1. 规则配置`notify_channels=["wecom"]`且Wecom机器人已绑定；2. 预警已生成 | 1. 检查Wecom群消息；2. 点击消息内「已确认」按钮 | 1. Wecom收到含标题、详情链接、按钮的消息；2. `alert_instance.status`变为`acknowledged` | P0 |
| TC04 | 多语言文案校验失败 | 尝试创建`rule_name="消費税インボイス（半角カタカナ）"`的规则 | 提交表单 | 返回400错误，提示「ルール名に半角カタカナは使用できません」 | P1 |
| TC05 | 跨租户数据隔离验证 | 租户A创建规则并触发预警；租户B登录系统 | 查询租户B的`/alerts`接口 | 返回空列表，无租户A的预警数据 | P0 |

##### 6.2 边界测试
- 测试`trigger_condition.value`为极大值（`999999999`）和极小值（`-999999999`）时，预警引擎不崩溃且能正确比较；
- 测试`notify_users`数组长度=20（上限）时，全部用户均收到通知；
- 测试`rule_name`为50个全角字符（如50个「あ」）时，保存成功且前端正常显示；
- 测试`trigger_frequency=realtime`规则在1秒内处理1000个并发订单事件，无丢失；
- 测试`alert_instance`表单月数据量达500万条时，`ListAlerts`接口响应时间≤1.2s（P95）。

##### 6.3 异常场景
| 异常场景 | 触发条件 | 系统处理方式 | 用户提示 |
|----------|----------|-------------|----------|
| 规则语法错误 | `trigger_condition` JSON格式非法或含不支持运算符（如`xor`） | 拒绝保存，记录审计日志`ALERT_RULE_SYNTAX_ERROR` | 「ルール条件の構文が正しくありません。演算子はgt/lt/gte/lte/percent_changeのみ許可されます」 |
| Wecom机器人失效 | 配置了`wecom`但Webhook URL返回HTTP 503 | 写入`alert_notification_log`状态为`failed`，重试3次后标记为`ignored`，继续发送其他渠道 | 无前端提示（后台静默降级） |
| 日文敏感词触发 | `description`含「天皇」一词 | 拦截保存，返回403 | 「説明文に規制ワードが含まれています。内容をご確認ください」 |
...
#### 1. 功能概述  
- **业务目标**：解决日本市场卖家对核心竞品（含乐天、亚马逊JP、Yahoo! Shopping等平台上的同类SPU/SKU）的实时价格、库存、Listing质量、Review趋势、促销动态及搜索排名变化缺乏系统化追踪的问题，支撑精细化运营决策与快速响应。  
- **用户价值**：  
  - 实时掌握竞品调价节奏与促销窗口（如乐天“スーパーセール”、亚马逊JP“タイムセール”），辅助制定动态定价策略；  
  - 识别竞品Listing优化动向（标题/图片/五点描述更新频次、日文合规性评分变化），指导自身商品内容迭代；  
  - 基于Review情感分析（日语NLP模型）与QA问答热度，预判品类口碑风险点；  
  - 自动生成《竞品月度健康度报告》（含价格竞争力指数、内容健康分、口碑波动预警），支持管理层复盘。  
- **使用角色**：  
  - 运营经理（高频使用，每日查看竞品仪表盘）  
  - 商品经理（中频使用，每周分析Listing优化建议）  
  - 采购与定价专员（高频使用，实时触发调价规则引擎）  
  - 日本本地合规专员（低频使用，按需核查竞品文案敏感词与消费税标注合规性）  
- **使用频率**：高频（核心运营岗日均操作≥3次）

#### 2. 输入参数  
| 参数名 | 类型 | 必填 | 校验规则 | 说明 |  
|--------|------|------|----------|------|  
| `competitor_sku_list` | Array<String> | 是 | 非空，长度≤50；每个SKU符合平台规范（如亚马逊ASIN为10位字母数字，乐天ItemID为12位数字，Yahoo! Shopping ProductID含下划线前缀） | 待监控的竞品SKU列表，支持跨平台混合输入（系统自动识别平台来源） |  
| `monitoring_platforms` | Array<Enum> | 是 | 元素必须属于 `["amazon_jp", "rakuten", "yahoo_jp", "shimamura", "yodobashi"]`；至少选择1个 | 指定需抓取数据的电商平台（仅限已授权接入的店铺所在平台） |  
| `monitoring_duration_days` | Integer | 是 | ≥7 且 ≤90；若为0则默认30 | 数据回溯周期（用于计算趋势指标，如7日价格波动率） |  
| `alert_thresholds` | Object | 否 | JSON Schema校验：`{ "price_change_percent": { "min": -100, "max": 100 }, "review_rating_drop": { "min": 0.1, "max": 2.0 }, "stock_status_change": Boolean }` | 自定义告警阈值，未传入则使用租户级默认值（日本租户默认：价格变动≥5%、评分下降≥0.5、库存由InStock→OutOfStock） |  
| `language_preference` | String | 否 | 必须为 `"ja"`（强制日文，因竞品数据源为日本市场） | 接口返回文案语言，固定为日文以保障合规审查一致性 |  

#### 3. 输出结果  
| 输出项 | 类型 | 说明 |  
|--------|------|------|  
| `monitoring_task_id` | String | 唯一任务ID（UUID v4格式），用于异步查询进度与结果 |  
| `status` | Enum | 当前状态：`"pending"`（排队中）、`"fetching"`（爬取中）、`"processing"`（解析中）、`"completed"`（成功）、`"failed"`（失败） |  
| `summary` | Object | 汇总指标：`{ "total_competitors": Integer, "data_freshness": ISO8601 timestamp (e.g., "2024-06-15T08:22:15+09:00"), "last_updated_at": ISO8601 timestamp, "alerts_count": Integer }` |  
| `competitors_data` | Array<Object> | 竞品明细数组，每项含：`sku`, `platform`, `current_price_jpy`, `original_price_jpy`, `stock_status`, `review_rating`, `review_count`, `listing_health_score`（0–100，基于日文文案评分、图片清晰度、A+内容完整性等加权），`review_sentiment_trend_7d`（日语情感分析结果：`"positive"/"neutral"/"negative"`），`promotion_tags`（Array<String>，如`["タイムセール", "送料無料", "ポイント2倍"]`） |  
| `trend_analysis` | Object | 趋势洞察：`{ "price_volatility_index": Float (0–10), "listing_update_frequency_30d": Integer, "review_rating_momentum": Float (-2.0 to +2.0) }` |  

#### 4. 功能设计  

##### 4.1 交互流程  
1. 用户进入【工具与增长插件】→【竞品监控与分析】模块，点击「新建监控任务」；  
2. 在表单中粘贴或批量导入竞品SKU（支持CSV上传，系统自动校验SKU格式并高亮错误行）；  
3. 选择目标平台（勾选已授权的店铺所在平台，禁用未接入平台选项）；  
4. 设置监控周期（默认30天，可拖动滑块选择7/15/30/60/90天）；  
5. （可选）展开「高级设置」配置自定义告警阈值，点击「保存为模板」供后续复用；  
6. 点击「启动监控」，前端显示加载态，后端生成任务并返回`monitoring_task_id`；  
7. 系统后台异步执行：① 调用各平台API/合规爬虫获取数据 → ② 日语NLP引擎解析Review情感与文案 → ③ 计算价格波动率、Listing健康分等指标 → ④ 生成结构化结果存入时序数据库；  
8. 用户可在「任务列表」页查看状态，点击`monitoring_task_id`进入详情页，实时刷新图表（价格趋势折线图、Review情感热力图、促销标签云）；  
9. 点击「导出PDF报告」生成含封面（含公司LOGO与日本合规声明）、执行摘要、分竞品详情页、附录（原始数据表）的A4横向报告。  

##### 4.2 核心逻辑  
- **多平台适配层**：  
  - 对接乐天API需通过`Rakuten API Key` + `Application ID`认证，抓取`ItemSearch`与`ItemRanking`接口；  
  - 亚马逊JP采用官方SP-API（`getCatalogItem` + `getProductPricing`），严格遵循`x-amz-access-token`时效性（1小时）与请求限频（10 RPS）；  
  - Yahoo! Shopping使用其公开RSS Feed + 头部UA模拟（需通过`yahoo_jp_crawler_token`鉴权，该Token由系统后台统一管理并轮换）。  
- **日语NLP处理**：  
  - Review情感分析调用内部微服务`jp-sentiment-analyzer`，输入日文评论文本，输出`{ "sentiment_score": -1.0~1.0, "keywords": ["品質", "発送が早い", "説明と違う"] }`；  
  - Listing文案评分基于：① 敏感词库匹配（日本合规库含`"最安値保証"`等违规表述）→ 扣分；② 消费税标注检测（是否含`"税込"`或`"税別"`）→ 缺失扣分；③ 语法正确性（使用`kuromoji`分词+`MeCab`句法树校验）→ 错误扣分；④ 图片Alt文本日文覆盖率（OCR识别后NLP验证）→ 低于80%扣分。  
- **价格竞争力指数（PCI）计算**：  
  ```  
  PCI = 100 - [ (当前价 - 竞品均价) / 竞品均价 × 50 ]  
         - [ 价格波动率(7日标准差) × 10 ]  
         + [ 促销标签权重和（タイムセール=+15, 送料無料=+8, ポイント2倍=+5）]  
  ```  
  结果四舍五入至整数，范围0–100（≥85为强竞争力）。  
- **库存状态判定**：  
  - 乐天：`itemStatus == "IN_STOCK"` 且 `stockQuantity > 0`；  
  - 亚马逊JP：`offers.listings[0].inventory?.quantity > 0`；  
  - Yahoo! Shopping：`availability == "in_stock"` 且 `stock > 0`；  
  - 若任一平台返回`"out_of_stock"`或`null`，标记为`OutOfStock`并触发告警。  

##### 4.3 接口设计  
| 接口 | 方法 | 路径 | 说明 |  
|------|------|------|------|  
| `POST /v1/growth/competitor-monitoring/tasks` | POST | `/v1/growth/competitor-monitoring/tasks` | 创建竞品监控任务 |  
| `GET /v1/growth/competitor-monitoring/tasks/{task_id}` | GET | `/v1/growth/competitor-monitoring/tasks/{task_id}` | 查询任务状态与结果（含分页参数`page=1&size=20`） |  
| `GET /v1/growth/competitor-monitoring/tasks/{task_id}/report` | GET | `/v1/growth/competitor-monitoring/tasks/{task_id}/report` | 导出PDF报告（响应头`Content-Disposition: attachment; filename="competitor_report_{task_id}.pdf"`） |  
| `DELETE /v1/growth/competitor-monitoring/tasks/{task_id}` | DELETE | `/v1/growth/competitor-monitoring/tasks/{task_id}` | 删除历史任务（仅限创建者与租户管理员） |  

**请求示例（创建任务）**：  
```json
{
  "competitor_sku_list": ["B09XYZ1234", "100012345678", "yahoo_abc123_def456"],
  "monitoring_platforms": ["amazon_jp", "rakuten", "yahoo_jp"],
  "monitoring_duration_days": 30,
  "alert_thresholds": {
    "price_change_percent": 3.5,
    "review_rating_drop": 0.3,
    "stock_status_change": true
  }
}
```  
...
```json
{
  "monitoring_task_id": "a1b2c3d4-e5f6-7890-g1h2-i3j4k5l6m7n8",
  "status": "completed",
  "summary": {
    "total_competitors": 3,
    "data_freshness": "2024-06-15T08:22:15+09:00",
    "last_updated_at": "2024-06-15T08:25:42+09:00",
    "alerts_count": 2
  },
  "competitors_data": [
    {
      "sku": "B09XYZ1234",
      "platform": "amazon_jp",
      "current_price_jpy": 5980,
      "original_price_jpy": 6980,
      "stock_status": "InStock",
      "review_rating": 4.6,
      "review_count": 128,
      "listing_health_score": 82,
      "review_sentiment_trend_7d": "positive",
      "promotion_tags": ["タイムセール"]
    }
  ],
  "trend_analysis": {
    "price_volatility_index": 4.2,
    "listing_update_frequency_30d": 3,
    "review_rating_momentum": 0.8
  }
}
```  

##### 4.4 数据模型  
- **`competitor_monitoring_tasks`**（任务主表）  
  | 字段名 | 类型 | 说明 |  
  |--------|------|------|  
  | `id` | UUID | 主键 |  
  | `tenant_id` | VARCHAR(32) | 租户ID（关联`tenants.tenant_id`） |  
  | `created_by` | VARCHAR(64) | 创建者子账号ID |  
  | `status` | ENUM('pending','fetching','processing','completed','failed') | 任务状态 |  
  | `created_at` | DATETIME | 创建时间（JST时区） |  
  | `updated_at` | DATETIME | 最后更新时间 |  
  | `input_params` | JSON | 存储原始输入参数（加密存储敏感字段如token） |  

- **`competitor_monitoring_results`**（结果明细表）  
  | 字段名 | 类型 | 说明 |  
  |--------|------|------|  
  | `id` | BIGINT | 主键 |  
  | `task_id` | UUID | 外键，关联`competitor_monitoring_tasks.id` |  
  | `competitor_sku` | VARCHAR(64) | 竞品SKU |  
  | `platform` | ENUM('amazon_jp','rakuten','yahoo_jp','shimamura','yodobashi') | 平台标识 |  
  | `snapshot_time` | DATETIME | 数据快照时间（JST） |  
  | `current_price_jpy` | DECIMAL(10,2) | 当前售价（日元） |  
  | `original_price_jpy` | DECIMAL(10,2) | 原价（日元） |  
  | `stock_status` | ENUM('InStock','OutOfStock','PreOrder','BackOrder') | 库存状态 |  
  | `review_rating` | DECIMAL(3,2) | 当前评分 |  
  | `review_count` | INTEGER | 评论总数 |  
  | `listing_health_score` | TINYINT | Listing健康分（0–100） |  
  | `review_sentiment_score` | DECIMAL(3,2) | 情感分（-1.0~1.0） |  
  | `promotion_tags` | JSON | 促销标签数组 |  

#### 5. 业务规则  
1. **数据合规性强制校验**：所有抓取的竞品Listing文案必须通过日本《景品表示法》敏感词库扫描，若检测到`"最安値保証"`、`"業界No.1"`等绝对化用语，结果中标记`compliance_risk: true`并禁止在报告中展示原文，仅提示“存在合规风险，建议人工复核”。  
2. **汇率锁定机制**：价格数据统一以日元（JPY）存储与计算，若竞品页面显示其他币种（如USD），调用实时汇率服务（`exchange_rate_service`）按JST当日9:00东京汇市中间价转换，且汇率值写入`competitor_monitoring_results.exchange_rate_jpy_usd`字段留痕。  
3. **竞品去重与归一化**：同一物理商品在不同平台的SKU（如亚马逊ASIN与乐天ItemID指向同一款耳机），通过`product_fingerprint_hash`（基于品牌+型号+规格哈希生成）自动聚类，确保趋势分析基于商品维度而非SKU维度。  
4. **告警抑制规则**：同一竞品在24小时内重复触发相同类型告警（如价格变动），仅推送首次告警至企业微信/Slack，后续告警合并至日报汇总，避免消息轰炸。  
5. **数据保留策略**：原始监控数据保留90天，聚合趋势数据（如7日均价、月度评分均值）永久保留，超期数据自动归档至冷存储（AWS S3 Glacier）。  
...
| 编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 | 优先级 |  
|------|----------|----------|----------|----------|--------|  
| TC01 | 正常创建乐天+亚马逊双平台监控任务 | 租户已授权乐天店铺A与亚马逊JP店铺B；SKU列表含1个乐天ItemID与1个ASIN | 1. 输入SKU；2. 勾选`rakuten`与`amazon_jp`；3. 点击启动 | 返回`status=pending`，`summary.total_competitors=2`，任务列表可见新任务 | P0 |  
| TC02 | 输入非法SKU触发校验 | SKU列表含`INVALID-SKU`（非10位ASIN/非12位乐天ID） | 提交任务表单 | 前端高亮错误行并提示“乐天ItemID需为12位纯数字，ASIN需为10位字母数字组合”，禁止提交 | P0 |  
| TC03 | 竞品价格突变触发告警 | 任务中某竞品7日内价格下降6.2%（超阈值5%） | 查看任务详情页「告警记录」Tab | 显示1条告警：`[价格异常] B09XYZ1234 降价6.2%，当前价¥5,980（原¥6,980）` | P0 |  
| TC04 | 日语Review情感分析准确性 | 竞品Review含`"この商品は本当に素晴らしい！配送も早くて助かりました。"` | 查看`review_sentiment_score`字段 | 值为`0.92`（强正向），`review_sentiment_trend_7d="positive"` | P1 |  
| TC05 | 导出PDF报告含日本合规声明 | 任务状态为`completed` | 点击「导出PDF报告」 | 下载文件打开后，第1页底部显示灰色小字：“本报告数据采集符合日本《個人情報保護法》及《不正競争防止法》，分析过程未存储原始用户评论。” | P1 |  

##### 6.2 边界测试  
- 输入`monitoring_duration_days=90`（最大值），验证系统能否在15分钟内完成全量数据拉取与计算（乐天+亚马逊+Yahoo共50个SKU）；  
- 输入`competitor_sku_list`长度=50（上限），验证任务创建接口响应时间≤2s，且无内存溢出；  
- 在JST时间`23:59:59`触发任务，验证`data_freshness`时间戳精确到秒且时区为`+09:00`。  

##### 6.3 异常场景  
| 异常场景 | 触发条件 | 系统处理方式 | 用户提示 |  
|----------|----------|-------------|----------|  
| 乐天API限流失败 | 乐天返回HTTP 429 Too Many Requests | 自动启用退避重试（指数退避：1s→2s→4s），最多3次；失败后标记该SKU为`unavailable`并跳过 | “乐天平台暂时繁忙，已跳过SKU XXXXX，将在下次监控中重试” |  
| 亚马逊SP-API Token过期 | 调用`getProductPricing`时返回`InvalidAccessTokenException` | 自动调用`refresh_access_token`接口获取新Token，重试本次请求 | 无前端提示（后台静默恢复） |  
| 竞品SKU在目标平台无数据 | 乐天ItemID在乐天站内搜索无结果 | 该SKU在`competitors_data`中返回`{ "sku": "XXX", "platform": "rakuten", "error": "not_found_on_platform" }` | 报告中该竞品行标红，显示“未在乐天找到此商品” |

### 工具与增长插件-Review/QA追踪

#### 1. 功能概述
- **业务目标**：解决日本跨境卖家对亚马逊（Amazon.co.jp）、乐天市场（Rakuten）、Yahoo! Shopping 等主流平台商品Review（用户评价）与QA（问答区）内容的被动响应问题，实现主动监控、情感分析、时效预警、竞品对比及工单联动，提升口碑管理效率与消费者信任度。
- **用户价值**：
  - 实时掌握商品口碑动态（如差评突增、敏感词出现、QA未回复超24h），降低差评扩散风险；
  - 自动生成日文合规回复草稿（含敬语、消费税提示、退货政策引用），缩短客服响应SLA至≤4小时；
  - 支持按店铺/SPU/ASIN/JAN码多维聚合分析，识别高频投诉点（如“発送遅延”“梱包不良”），驱动供应链与物流优化；
  - 对接规则引擎，触发自动工单（如“差评+关键词‘壊れた’→转质检部”）、邮件告警（运营负责人+客服主管双通道）及BI看板预警。
- **使用角色**：  
  - **客服主管**（高频）：日常巡检、分配工单、审核回复；  
  - **运营经理**（中频）：分析口碑趋势、制定改进策略；  
  - **质控专员**（低频）：接收差评关联工单，执行根因分析；  
  - **AI助手用户**（高频）：调用智能回复建议、文案润色。
- **使用频率**：高频（客服主管每日≥3次；运营经理每周5次以上；系统后台自动轮询每15分钟）

#### 2. 输入参数
| 参数名 | 类型 | 必填 | 校验规则 | 说明 |
|--------|------|------|----------|------|
| `shop_ids` | string[] | 是 | 非空数组，元素为合法`shop_id`（UUID格式），且用户有对应店铺`view_review`权限 | 指定监控的店铺ID列表，支持跨平台（如同时选Amazon.co.jp + Rakuten） |
| `spu_ids` | string[] | 否 | 若存在，每个元素为合法`spu_id`（UUID），且属于所选店铺已同步商品 | 限定监控的具体SPU（非SKU），用于聚焦高价值商品 |
| `review_status_filter` | enum | 否 | 取值：`all`/`new`/`unreplied`/`negative`/`critical`（含“壊れた”“詐欺”等12个日本监管敏感词） | 评价状态筛选，`critical`需触发P0级告警 |
| `qa_status_filter` | enum | 否 | 取值：`all`/`unanswered`/`overdue_24h`/`overdue_72h` | QA状态筛选，`overdue_24h`表示提问后24小时内未回复 |
| `time_range` | object | 是 | `{start: "YYYY-MM-DD HH:mm:ss", end: "YYYY-MM-DD HH:mm:ss"}`，`end`不得早于`start`，时间跨度≤30天 | 查询时间窗口，用于历史回溯与报表生成 |
| `language` | string | 否 | 默认`ja-JP`，仅允许`ja-JP`/`zh-CN`/`en-US`（日文为强制主语言，其他为翻译展示） | 界面与分析结果语言偏好（不影响原始数据采集） |

#### 3. 输出结果
| 输出项 | 类型 | 说明 |
|--------|------|------|
| `review_summary` | object | 包含总评价数、新增数、好评率（≥4星占比）、差评率（≤2星）、`critical`评价数、平均响应时长（小时） |
| `qa_summary` | object | 包含总QA数、未回复数、超时（24h/72h）数、平均首次回复时长（小时） |
| `review_list` | array[object] | 评价详情列表，每项含`id`（平台唯一ID）、`platform`（`amazon_jp`/`rakuten`/`yahoo`）、`asin_or_jan`（平台商品编码）、`spu_name_ja`（日文SPU名）、`rating`（1-5）、`title_ja`、`body_ja`、`sentiment_score`（-1.0~1.0，负向越强）、`is_critical`（bool）、`created_at`、`replied_at`、`reply_body_ja`（若已回复） |
| `qa_list` | array[object] | QA详情列表，每项含`id`（平台唯一ID）、`platform`、`asin_or_jan`、`spu_name_ja`、`question_ja`、`answer_ja`（若已答）、`asked_at`、`answered_at`、`is_overdue_24h`、`is_overdue_72h` |
| `trend_data` | array[object] | 近7日趋势数据，每项含`date`（YYYY-MM-DD）、`new_reviews`、`new_qas`、`negative_rate`、`unreplied_qa_count` |
| `ai_suggestions` | array[object] | AI生成建议列表，每项含`type`（`reply_draft`/`root_cause`/`compliance_alert`）、`content_ja`（日文建议内容）、`confidence`（0.0~1.0） |

#### 4. 功能设计

##### 4.1 交互流程
1. 用户进入【工具与增长插件】→【Review/QA追踪】模块，默认加载当前租户下所有已授权店铺的近7日汇总视图；  
2. 用户通过顶部筛选栏选择`shop_ids`（多选）、`spu_ids`（可搜索SPU名或JAN码）、`review_status_filter`（如选`critical`）、`qa_status_filter`（如选`overdue_24h`）、`time_range`（默认最近7天，可拖拽调整）；  
3. 点击【应用筛选】，前端发起请求，后端返回结构化数据并渲染：  
   - 左侧卡片式汇总（`review_summary` + `qa_summary`）；  
   - 中部表格展示`review_list`与`qa_list`，支持按评分、时间、情感分排序，差评/超时QA高亮红色；  
   - 右侧折线图展示`trend_data`，鼠标悬停显示单日明细；  
   - 底部【AI建议区】展示`ai_suggestions`，点击`reply_draft`可一键复制到编辑框；  
4. 用户点击某条差评右侧【生成回复】按钮，调用AI文案助手接口，返回日文合规回复草稿（含敬语、退货政策链接、消费税说明）；  
5. 用户点击【创建工单】，弹出表单预填`subject`（如“差评：商品破损-ASIN:B0XXXXXX”）、`description`（自动带入原文+情感分析结论）、`assign_to`（根据规则引擎配置默认分配至质检组），提交后同步更新`review_list`中该条记录的`status`为`ticket_created`；  
6. 系统后台每15分钟自动轮询各平台API，增量拉取新Review/QA，实时写入数据库并触发规则引擎判断。

##### 4.2 核心逻辑
- **数据采集层**：  
  - 基于店铺授权凭证（OAuth2 for Amazon / Rakuten API Key / Yahoo! Shopping App ID），调用各平台公开API（Amazon Product Advertising API v5, Rakuten Review API, Yahoo! Shopping QA API）；  
  - 采用增量同步：记录每个店铺最后拉取的`last_sync_timestamp`，每次请求`?after=last_sync_timestamp`；  
  - 对于Amazon.co.jp，额外解析`Review Body`中的日文汉字/假名比例（要求≥85%为纯日文），低于阈值则标记`is_japanese_compliant=false`并触发合规告警；  
- **情感分析引擎**：  
  - 使用轻量级日文BERT模型（`cl-tohoku/bert-base-japanese-v3`微调版），输入`title_ja`+`body_ja`，输出`sentiment_score`；  
  - 内置日本电商敏感词库（12个核心词：`壊れた`、`届かない`、`偽物`、`詐欺`、`発送遅延`、`梱包不良`、`消費税未記載`、`インボイス未発行`、`返金しない`、`説明と違う`、`サイズ違い`、`色落ち`），任一命中即设`is_critical=true`；  
- **时效计算规则**：  
  - QA超时判定：`now() - asked_at > 24*3600` → `is_overdue_24h=true`；  
  - Review响应时长：`replied_at - created_at`（秒），若未回复则为`null`；  
- **AI回复生成**：  
  - 输入：`rating`、`sentiment_score`、`body_ja`、`spu_name_ja`、店铺配置的`return_policy_url_ja`、`tax_invoice_note_ja`（消费税发票说明）；  
  - 输出：符合日本商务礼仪的3段式结构——①谢罪（例：“この度はご不便・ご迷惑をおかけしましたこと、深くお詫び申し上げます。”）；②事实确认（例：“該当商品の検品記録を確認したところ、〜”）；③解决方案（例：“返金または再発送をご希望の場合、以下のURLより申請いただけます：[return_policy_url_ja]。また、消費税込みの正式なインボイスはメールにて別途送付いたします。”）；  
- **工单联动**：  
  - 创建工单时，自动填充`tags=["review", "critical"]`或`["qa", "overdue"]`，并写入`source_system="review_qa_tracker"`，供订单中心工单系统识别来源。

##### 4.3 接口设计
| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| `GET /api/v1/review-qa/tracking/summary` | GET | `/review-qa/summary?shop_ids[]=shp_abc&shop_ids[]=shp_def&time_range.start=2024-05-01T00:00:00&time_range.end=2024-05-07T23:59:59` | 获取汇总数据（`review_summary` + `qa_summary` + `trend_data`） |
| `GET /api/v1/review-qa/tracking/list` | GET | `/review-qa/list?shop_ids[]=shp_abc&spu_ids[]=spu_xyz&review_status_filter=critical&qa_status_filter=overdue_24h&time_range.start=...&page=1&size=20` | 分页获取详情列表（`review_list` + `qa_list`） |
| `POST /api/v1/review-qa/tracking/reply-suggestion` | POST | `/review-qa/reply-suggestion` | 请求AI回复草稿，请求体：`{"review_id": "amz_rev_123", "platform": "amazon_jp", "body_ja": "商品が壊れて届きました..."}`，响应：`{"suggestion_ja": "この度は...", "confidence": 0.92}` |
| `POST /api/v1/review-qa/tracking/create-ticket` | POST | `/review-qa/create-ticket` | 创建工单，请求体：`{"review_id": "amz_rev_123", "reason": "critical_damage", "assignee_group": "quality_control"}`，响应：`{"ticket_id": "tk_789", "status": "created"}` |
| `POST /api/v1/review-qa/tracking/sync-trigger` | POST | `/review-qa/sync-trigger` | 手动触发全量同步（仅限管理员），请求体：`{"shop_ids": ["shp_abc"]}` |

##### 4.4 数据模型
- **`review_qa_tracking`（主表）**  
  `id` (UUID, PK)  
  `platform` (enum: `amazon_jp`, `rakuten`, `yahoo`)  
  `external_id` (string, 平台原生ID，如Amazon的`R1234567890ABCDEF`)  
  `shop_id` (UUID, FK → `shops.id`)  
  `spu_id` (UUID, FK → `pim_spus.id`, 可为空)  
  `asin_or_jan` (string, Amazon用ASIN，Rakuten/Yahoo用JAN码)  
  `rating` (int, 1-5)  
  `title_ja` (text)  
  `body_ja` (text)  
  `sentiment_score` (float, -1.0~1.0)  
  `is_critical` (boolean)  
  `is_japanese_compliant` (boolean)  
  `created_at` (datetime, 平台创建时间)  
  `replied_at` (datetime, null if not replied)  
  `reply_body_ja` (text, null if not replied)  
  `synced_at` (datetime, 本系统入库时间)  

- **`review_qa_qa`（QA子表，1:1关联主表）**  
  `id` (UUID, PK)  
  `review_qa_id` (UUID, FK → `review_qa_tracking.id`)  
  `question_ja` (text)  
  `answer_ja` (text, null if not answered)  
  `asked_at` (datetime)  
  `answered_at` (datetime, null if not answered)  

- **`review_qa_analysis_cache`（分析缓存表，提升查询性能）**  
  `date` (date, 分区键)  
  `shop_id` (UUID)  
  `spu_id` (UUID, nullable)  
  `new_reviews` (int)  
  `new_qas` (int)  
  `negative_count` (int)  
  `unreplied_qa_count` (int)  
  `updated_at` (datetime)  

#### 5. 业务规则
1. **日本合规强制校验**：所有从Amazon.co.jp拉取的Review，若`body_ja`中日文字符（Unicode范围`\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FFF`）占比＜85%，系统自动标记`is_japanese_compliant=false`，并在UI中显示⚠️图标，并推送通知至“日本合规负责人”角色；  
2. **差评响应SLA**：对于`rating ≤ 2`且`sentiment_score ≤ -0.6`的Review，系统自动生成倒计时（72小时），超时未回复则升级为P1告警（邮件+企业微信机器人推送）；  
3. **QA时效分级响应**：`overdue_24h`的QA，自动分配至一线客服组；`overdue_72h`的QA，自动升级并通知客服主管，同时在BI看板“服务滞后TOP5”中置顶；  
4. **竞品对比逻辑**：当用户在筛选中勾选≥2个店铺（如自家Amazon店 + 竞品Rakuten店），系统在`review_summary`中增加`competitor_comparison`字段，对比双方“好评率”、“差评关键词TOP3”、“QA平均响应时长”；  
5. **数据保留策略**：Review/QA原始数据永久存储，但`review_qa_analysis_cache`仅保留最近180天，过期数据归档至冷存储（AWS S3 Glacier），满足日本《個人情報保護法》（APPI）第20条保存期限要求。
...
| 编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 | 优先级 |
|------|----------|----------|----------|----------|--------|
| TC01 | 正常筛选查看Amazon.co.jp差评 | 1. 租户已接入Amazon.co.jp店铺shp_amz；<br>2. 该店铺近3天有5条≤2星Review | 1. 进入Review/QA追踪页；<br>2. 选择`shop_ids=[shp_amz]`、`review_status_filter=negative`、`time_range=近3天`；<br>3. 点击【应用筛选】 | 页面正确显示5条差评，`review_summary`中`negative_rate`准确，每条记录`rating`列显示红色★图标 | P0 |
| TC02 | AI生成日文合规回复草稿 | 1. 存在一条`rating=1`、`body_ja="商品が全部壊れて届きました！詐欺です！"`的Review；<br>2. 店铺配置了`return_policy_url_ja`和`tax_invoice_note_ja` | 1. 在该Review行点击【生成回复】；<br>2. 查看返回内容 | 返回的`suggestion_ja`包含标准谢罪句式、提及“壊れ”“詐欺”关键词、嵌入退货链接与消费税说明，无中文/英文混杂 | P0 |
| TC03 | 创建工单并联动订单中心 | 1. 用户有`create_ticket`权限；<br>2. 规则引擎已配置“critical_review→quality_control”路由 | 1. 对`is_critical=true`的Review点击【创建工单】；<br>2. 提交表单 | 工单成功创建，`ticket_id`返回；订单中心工单列表中可见该工单，`source_system="review_qa_tracker"`，`tags`含`critical` | P0 |
| TC04 | 日文合规性校验触发告警 | 1. 模拟一条Amazon Review，`body_ja="This product is broken. Very bad quality."`（全英文） | 1. 执行同步任务；<br>2. 查看该Review记录 | `is_japanese_compliant=false`，UI显示⚠️图标，企业微信收到通知：“检测到Amazon.co.jp店铺shp_amz存在非日文评价，请及时处理” | P1 |
| TC05 | 多平台竞品对比分析 | 1. 租户接入自家Amazon店shp_amz与竞品Rakuten店shp_rkt；<br>2. 两店均有近7天Review数据 | 1. 同时选择`shop_ids=[shp_amz, shp_rkt]`；<br>2. 查看`review_summary` | 显示`competitor_comparison`区块，清晰对比双方好评率（如“自社: 92.3% vs 竞品: 88.7%”）及差评TOP3关键词（如“自社: 発送遅延, 梱包不良 / 竞品: 価格高, 送料高”） | P1 |

##### 6.2 边界测试
- 时间范围边界：`time_range.start = time_range.end`（同一天），应正确返回当日0点至24点数据；  
- 极端数据量：单次请求`review_list`返回10,000条记录（分页size=100，page=100），前端表格渲染不卡顿，滚动流畅；  
- 敏感词临界：`body_ja="壊れた"`（精确匹配）触发`is_critical=true`；`body_ja="こわれた"`（平假名变体）不触发（因未在敏感词库中，需后续扩展）；  
- 多语言混合：`body_ja="商品が壊れた。The item is broken."`，日文占比＞85%，`is_japanese_compliant=true`；  
- 空值容错：`spu_id`为空时，`spu_name_ja`字段显示“未关联SPU”，不报错。

##### 6.3 异常场景
| 异常场景 | 触发条件 | 系统处理方式 | 用户提示 |
|----------|----------|-------------|----------|
| 平台API限流失败 | 调用Amazon API返回HTTP 429 | 自动启用指数退避重试（1s→2s→4s→8s），最多3次；失败后记录错误日志并标记`sync_status="failed_rate_limit"` | “Amazon.co.jp数据同步受限，系统将在1分钟后自动重试。请稍候查看。” |
| 敏感词库缺失 | 新增平台Yahoo! Shopping，但其敏感词库未配置 | 使用通用日文敏感词库（含基础12词），不中断同步，记录warn日志 | “Yahoo! Shopping平台敏感词库暂未定制，已启用通用词库进行扫描。” |
| AI服务不可用 | `POST /review-qa/reply-suggestion` 调用AI微服务超时（>10s） | 返回降级响应：`{"suggestion_ja": "お手数ですが、以下テンプレートをご活用ください：『ご不便をおかけし誠に恐れ入ります。状況を確認し、最善を尽くして対応いたします。』", "confidence": 0.0}` | “AI回复生成暂时不可用，已提供标准模板供您参考。” |

### 工具与增长插件-智能调价引擎

#### 1. 功能概述
- **业务目标**：解决日本跨境卖家在乐天（Rakuten）、亚马逊日本站（Amazon.co.jp）、Yahoo! Shopping 等多平台运营中面临的动态定价难题，通过实时竞品价格监控、库存水位联动、毛利底线约束、消费税合规校验及平台算法偏好建模，实现「保利润、抢流量、避违规」三位一体的自动化调价决策，降低人工盯盘成本，提升价格竞争力与转化率。
- **用户价值**：
  - ✅ **降本增效**：将单SKU日均调价耗时从15分钟压缩至0秒自动执行，支持万级SKU批量策略化调价；
  - ✅ **利润保障**：强制嵌入日本消费税（10%）计算逻辑与JCT（インボイス制度）合规校验，避免因含税价误设导致税务风险；
  - ✅ **流量获取**：基于乐天“最安値表示”规则、亚马逊Buy Box权重因子（如配送时效、FBA标识、评分）动态加权，提升搜索排名与点击率；
  - ✅ **风控闭环**：当检测到竞品异常低价（如低于成本价30%且无促销标签）时自动触发人工复核工单，阻断恶意比价陷阱。
- **使用角色**：  
  - **运营经理**（高频）：配置全局调价策略、审核调价建议、处理异常预警；  
  - **品类运营专员**（中频）：为指定SPU/SKU组设置差异化调价规则（如清仓款激进降价、新品保守溢价）；  
  - **财务合规岗**（低频）：校验调价结果是否符合JCT发票要求、消费税分摊逻辑；  
  - **系统管理员**（低频）：管理调价引擎权限、对接平台API配额、查看引擎健康度看板。
- **使用频率**：高频（核心运营岗位每日至少1次全量策略刷新 + 实时微调）

#### 2. 输入参数
| 参数名 | 类型 | 必填 | 校验规则 | 说明 |
|--------|------|------|----------|------|
| `sku_id` | String | 是 | 非空、长度≤50、仅含字母/数字/下划线 | 日本ERP内唯一商品标识（如 `RTK-JP-ABC123-RED`），关联PIM商品主数据 |
| `platform_code` | Enum | 是 | ∈ `["rakuten_jp", "amazon_jp", "yahoo_jp"]` | 目标销售平台编码，决定调价规则模板与API适配器 |
| `current_price_jpy` | Decimal(10,2) | 是 | ≥ 0.01，≤ 99999999.99 | 当前平台售价（含消费税，单位：JPY），用于基线比对 |
| `cost_price_jpy` | Decimal(10,2) | 是 | ≥ 0.01，≤ `current_price_jpy`×0.9 | 采购成本+物流+平台佣金+消费税后综合成本价（含税），由财务模块实时同步 |
| `inventory_quantity` | Integer | 是 | ≥ 0 | 当前可售库存（含FBA虚拟仓、乐天官方仓等多仓聚合值），来自WMS库存四态模型中的「可用库存」态 |
| `competitor_prices` | Array[Object] | 否 | 最多50条，每条含 `price_jpy`(Decimal), `platform`(Enum), `is_fba`(Boolean), `rating`(Float 0–5) | 竞品实时价格快照（经爬虫或API拉取），含平台归属、FBA标识、店铺评分，用于乐天/亚马逊Buy Box权重计算 |
| `promotion_tags` | Array[String] | 否 | 元素∈ `["rakuten_point_5", "amazon_coupon_10", "yahoo_flash_sale"]` | 当前SKU已绑定的平台促销标签，调价时需预留折扣空间（如乐天5%积分活动需保证调价后仍满足最低价门槛） |
| `jct_registered` | Boolean | 是 | — | 当前店铺是否已完成日本インボイス制度注册（true=可开合规发票，调价时允许显示税额明细；false=仅能显示含税总价） |
| `last_updated_at` | Datetime(UTC) | 是 | ISO8601格式，≤ 当前时间-30s | 当前价格最后更新时间戳，用于判断数据新鲜度（超30分钟未更新则触发告警） |

#### 3. 输出结果
| 输出项 | 类型 | 说明 |
|--------|------|------|
| `recommended_price_jpy` | Decimal(10,2) | 推荐新售价（含消费税，单位：JPY），精确到分，满足所有业务规则约束 |
| `price_change_reason` | String | 调价动因编码，如 `"COMPETITOR_UNDERCUT"`（竞品降价）、`"LOW_STOCK_PREMIUM"`（库存<10自动溢价）、`"TAX_COMPLIANCE_ADJUST"`（JCT注册状态变更触发重算） |
| `confidence_score` | Float(0–1) | 决策置信度（0.95=高置信，基于竞品数据完整性、库存波动率、历史调价准确率等12维特征加权） |
| `rule_violations` | Array[String] | 违反的硬性规则列表（空数组=无违规），如 `["MIN_MARGIN_NOT_MET", "RAKUTEN_MIN_PRICE_VIOLATION"]` |
| `execution_status` | Enum | ∈ `["APPLIED", "PENDING_REVIEW", "BLOCKED"]` | 执行状态：`APPLIED`=已自动同步至平台；`PENDING_REVIEW`=需运营人工确认（如置信度<0.7）；`BLOCKED`=触发风控拦截（如成本价异常） |

#### 4. 功能设计

##### 4.1 交互流程
1. **进入场景**：运营人员在「工具与增长插件」模块点击「智能调价引擎」，进入策略看板页；
2. **筛选范围**：通过多选框选择目标店铺（支持跨平台店铺组）、SPU分类（PIM类目树）、库存状态（如「库存<20」）、上架时长（如「>30天」）；
3. **策略配置**：  
   - 选择预设模板（如「乐天冲榜模式」「亚马逊稳利润模式」）或自定义规则；  
   - 设置核心参数：目标毛利率（默认25%，乐天允许下探至18%）、最大降价幅度（默认-15%）、最小涨价幅度（默认+3%）、竞品价格容忍差（默认±5%）；  
   - 勾选合规开关：「强制JCT含税价校验」「禁用小数点后三位价格（违反乐天UI规范）」；
4. **执行分析**：点击「生成调价建议」，系统实时拉取WMS库存、财务成本、竞品API数据，运行调价模型，10秒内返回结果列表（含SKU、当前价、推荐价、变动率、原因、置信度）；
5. **人工干预**：对置信度<0.7或存在`rule_violations`的SKU，可点击「查看详情」查看竞品对比图谱、库存趋势曲线、历史调价效果热力图，并手动覆盖推荐价；
6. **批量执行**：勾选SKU → 点击「一键应用」→ 系统调用对应平台API（乐天Rakuten API v3 / 亚马逊SP-API Pricing API / Yahoo Shopping API）同步价格 → 返回执行日志（含API响应码、耗时、失败原因）；
7. **结果归档**：所有调价记录写入`price_adjustment_log`表，同步推送企业微信/钉钉通知，并在BI看板生成「调价ROI分析」（对比调价前后7日GMV、转化率、Buy Box占有率）。

##### 4.2 核心逻辑
- **四层决策流水线**：  
  **① 合规过滤层**：  
  - 强制校验 `recommended_price_jpy ≥ cost_price_jpy × (1 + min_gross_margin)`（min_gross_margin由店铺等级动态设定：S级=22%，A级=25%，B级=28%）；  
  - 若 `jct_registered=false`，则 `recommended_price_jpy` 必须为整数（乐天/雅虎禁止显示`.00`以外的小数）；  
  - 若 `platform_code="rakuten_jp"`，则拒绝 `recommended_price_jpy < 100`（乐天最低价限制）。  

  **② 竞品博弈层**：  
  - 对 `competitor_prices` 中同平台竞品，计算加权均价：`weighted_avg = Σ(price_jpy × weight_i)`，其中 `weight_i = 0.4×is_fba + 0.3×(rating/5) + 0.3×(1 - rank_position/10)`（rank_position由竞品排名API获取）；  
  - 设定目标区间：`target_min = weighted_avg × 0.95`, `target_max = weighted_avg × 1.05`；  
  - 若当前价超出区间，则向区间中心靠拢，但不超过最大变动幅度。  

  **③ 库存-需求耦合层**：  
  - 定义库存敏感系数 `k = 1 + (10 - inventory_quantity) × 0.05`（库存<10时k>1，自动溢价）；  
  - 若 `inventory_quantity ≤ 5`，强制 `recommended_price_jpy = current_price_jpy × min(1.15, k)`；  
  - 若 `inventory_quantity ≥ 200`，启用「清仓系数」：`recommended_price_jpy = max(cost_price_jpy × 1.1, current_price_jpy × 0.85)`。  

  **④ 平台算法适配层**：  
  - **乐天**：优先匹配「最安値表示」关键词（如`最安`, `激安`, `破格`），若竞品标题含此类词且价格更低，则 `recommended_price_jpy` 下调至竞品价-1 JPY；  
  - **亚马逊**：Buy Box权重公式 `BB_score = 0.3×is_fba + 0.25×(rating/5) + 0.2×(delivery_days≤2) + 0.15×(price_rank≤3) + 0.1×(stock>50)`，当 `BB_score < 0.7` 时，`recommended_price_jpy` 向TOP3价格带收敛；  
  - **雅虎**：强制要求价格末位为 `0` 或 `5`（如 `1,980`, `2,005`），否则自动向下取整到最近 `5` 的倍数。

- **数据流转**：  
  `PIM.sku_master` → 获取 `sku_id`, `jct_registered`  
  `WMS.warehouse_inventory` → 聚合 `inventory_quantity`（按 `warehouse_type IN ('fba_jp','rakuten_official','yahoo_fulfill')`）  
  `FINANCE.cost_calculation` → 拉取 `cost_price_jpy`（含 `purchase_cost + logistics_jpy + platform_fee_jpy + consumption_tax_jpy`）  
  `TOOL_COMPETITOR.price_snapshot` → 实时查询最近5分钟竞品数据（缓存TTL=60s）  
  → 经四层流水线计算 → 输出结果 → 写入 `tool_pricing.recommendation_log` → 触发 `PLATFORM_API_ADAPTER` 同步  

##### 4.3 接口设计
| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| `GET /api/v1/tool/pricing/strategies` | GET | `/tool/pricing/strategies` | 获取全部调价策略列表（含模板ID、名称、适用平台、创建人、最后修改时间） |
| `POST /api/v1/tool/pricing/recommend` | POST | `/tool/pricing/recommend` | 提交调价请求，输入参数见2.2节，响应结构示例：<br>```json<br>{<br>  "data": {<br>    "recommended_price_jpy": 3980.00,<br>    "price_change_reason": "COMPETITOR_UNDERCUT",<br>    "confidence_score": 0.92,<br>    "rule_violations": [],<br>    "execution_status": "APPLIED"<br>  },<br>  "meta": {"request_id": "req_abc123"}<br>}`<br> |
| `PUT /api/v1/tool/pricing/batch-apply` | PUT | `/tool/pricing/batch-apply` | 批量应用调价结果，请求体：<br>```json<br>{"sku_platform_pairs": [{"sku_id":"RTK-JP-ABC123","platform_code":"rakuten_jp"}], "override_price": 3980.00}`<br>响应含每个SKU的API同步状态（success/error） |
| `GET /api/v1/tool/pricing/log` | GET | `/tool/pricing/log?sku_id=RTK-JP-ABC123&start_time=2024-01-01&end_time=2024-01-31` | 查询指定SKU调价日志，返回分页列表，含操作人、旧价、新价、原因、平台响应详情 |

##### 4.4 数据模型
- **`tool_pricing.strategy_template`**（调价策略模板表）  
  `id` BIGINT PK, `name` VARCHAR(100), `platform_code` ENUM('rakuten_jp','amazon_jp','yahoo_jp'), `min_gross_margin` DECIMAL(5,2), `max_discount_rate` DECIMAL(5,2), `is_jct_required` BOOLEAN, `created_by` BIGINT (FK to `auth_user.id`), `created_at` DATETIME  

- **`tool_pricing.recommendation_log`**（调价建议日志表）  
  `id` BIGINT PK, `sku_id` VARCHAR(50), `platform_code` ENUM(...), `current_price_jpy` DECIMAL(10,2), `recommended_price_jpy` DECIMAL(10,2), `price_change_reason` VARCHAR(50), `confidence_score` FLOAT, `rule_violations` JSON, `execution_status` ENUM('APPLIED','PENDING_REVIEW','BLOCKED'), `applied_by` BIGINT, `applied_at` DATETIME, `api_response_code` VARCHAR(10), `api_response_msg` TEXT  

- **`tool_competitor.price_snapshot`**（竞品价格快照表）  
  `id` BIGINT PK, `sku_id` VARCHAR(50), `competitor_sku_id` VARCHAR(50), `platform_code` ENUM(...), `price_jpy` DECIMAL(10,2), `is_fba` BOOLEAN, `rating` FLOAT, `rank_position` INT, `fetched_at` DATETIME, `ttl_expired_at` DATETIME  

#### 5. 业务规则
1. **日本消费税强约束**：所有调价结果必须满足 `recommended_price_jpy = floor((base_price × 1.1) × 100) / 100`（即含税价精确计算，禁止四舍五入导致税额偏差）；  
2. **乐天最安値合规**：当SKU在乐天平台启用「最安値表示」功能时，系统自动校验其价格是否为同SPU下全站最低，若非最低则禁止应用调价并标记 `rule_violations=["RAKUTEN_MIN_PRICE_VIOLATION"]`；  
3. **亚马逊Buy Box保护**：若调价后 `recommended_price_jpy` 导致Buy Box占有率预测值下降>15%（基于历史回归模型），则置信度强制降至0.6并触发人工复核；  
4. **汇率联动机制**：当 `currency_pair = "USD/JPY"` 汇率单日波动>3%时，暂停所有依赖美元成本的SKU调价，转为「汇率锁定模式」（使用T-1日汇率快照）；  
5. **静默期保护**：新品上架30日内（`pim_product.created_at > NOW() - INTERVAL 30 DAY`），禁止自动降价，仅允许溢价或维持原价。
...
| 编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 | 优先级 |
|------|----------|----------|----------|----------|--------|
| TC01 | 乐天店铺JCT未注册时调价 | `sku_id="RTK-JP-ABC123"`, `platform_code="rakuten_jp"`, `jct_registered=false`, `current_price_jpy=3980.00` | 调用 `/recommend` 接口，传入参数 | `recommended_price_jpy=3980.00`（整数），`rule_violations=[]`，`execution_status="APPLIED"` | P0 |
| TC02 | 竞品低价冲击下的自动跟价 | `competitor_prices=[{"price_jpy":3850.00,"platform":"rakuten_jp","is_fba":true,"rating":4.8}]`, `current_price_jpy=3980.00` | 调用 `/recommend` | `recommended_price_jpy=3849.00`（竞品价-1），`price_change_reason="COMPETITOR_UNDERCUT"` | P0 |
| TC03 | 库存告急触发溢价 | `inventory_quantity=3`, `current_price_jpy=3980.00`, `cost_price_jpy=3200.00` | 调用 `/recommend` | `recommended_price_jpy=4577.00`（3980×1.15），`price_change_reason="LOW_STOCK_PREMIUM"` | P0 |
| TC04 | 成本价异常导致拦截 | `cost_price_jpy=5000.00`（高于`current_price_jpy=3980.00`），`min_gross_margin=25%` | 调用 `/recommend` | `execution_status="BLOCKED"`，`rule_violations=["MIN_MARGIN_NOT_MET"]`，`recommended_price_jpy=null` | P0 |
| TC05 | 批量应用跨平台调价 | 请求体含 `sku_platform_pairs=[{"sku_id":"RTK-JP-ABC123","platform_code":"rakuten_jp"},{"sku_id":"AMZ-JP-XYZ789","platform_code":"amazon_jp"}]` | 调用 `/batch-apply` | 响应中 `rakuten_jp` 条目返回 `success=true`，`amazon_jp` 条目返回 `error="API_RATE_LIMIT_EXCEEDED"`（模拟限流） | P1 |

##### 6.2 边界测试
- 输入 `inventory_quantity=0`：应返回 `recommended_price_jpy=0`（下架状态，不参与调价）；  
- 输入 `current_price_jpy=99999999.99`（最大值）：调价后不得溢出DECIMAL(10,2)精度；  
- `competitor_prices` 数组为空：降级为「成本导向模式」，`recommended_price_jpy = cost_price_jpy × (1 + min_gross_margin)`；  
- `last_updated_at` 超过30分钟：接口返回HTTP 400，`error_code="STALE_PRICE_DATA"`；  
- 同一SKU在1分钟内重复调用 `/recommend`：第二次请求应命中Redis缓存，响应时间<100ms。

##### 6.3 异常场景
| 异常场景 | 触发条件 | 系统处理方式 | 用户提示 |
|----------|----------|-------------|----------|
| 乐天API调用失败 | `platform_code="rakuten_jp"` 且 Rakuten API 返回 `503 Service Unavailable` | 自动切换至备用爬虫通道（若启用），否则写入 `recommendation_log` 并标记 `api_response_code="503"`，`execution_status="PENDING_REVIEW"` | 「乐天价格同步失败，请检查网络或稍后重试」 |
| 竞品数据缺失超阈值 | `competitor_prices.length < 3` 且 `platform_code="amazon_jp"` | 启用历史价格回归模型（基于过去7日价格序列拟合ARIMA），置信度强制设为0.5 | 「竞品数据不足，使用历史模型预测，建议人工复核」 |
| 汇率突变熔断 | `USD/JPY` 汇率24h波动>5%（由财务模块推送事件） | 全局冻结调价引擎，所有 `/recommend` 请求返回 `{"error":"RATE_FLUCTUATION_FROZEN"}` | 「日元汇率剧烈波动，调价引擎已暂停，请联系财务确认成本基准」 |

### 工具与增长插件-A/B测试支持

#### 1. 功能概述
- **业务目标**：为日本跨境卖家提供可配置、可度量、合规可控的A/B测试能力，支撑其在商品Listing优化（标题/主图/价格/五点描述）、广告素材（广告图/文案/出价策略）、客服话术（自动回复模板/SLA响应时长）、促销策略（满减规则/赠品逻辑）等关键增长环节进行科学决策，避免“凭经验调优”，显著提升转化率与ROI。
- **用户价值**：  
  ✅ 无需技术介入即可自主创建、运行、终止A/B测试；  
  ✅ 实时可视化对比核心指标（CTR、CVR、GMV、Review评分、客服响应满意度）；  
  ✅ 自动识别统计显著性（p<0.05），规避“伪提升”误判；  
  ✅ 原生支持日文场景（如日文文案分组、JIS编码兼容、消费税影响隔离）；  
  ✅ 测试过程全程留痕，满足日本《个人信息保护法》（APPI）及GDPR对实验数据处理的审计要求。
- **使用角色**：  
  - 运营专员（高频使用，负责日常Listing/广告测试）  
  - 广告优化师（中频，专注广告素材与出价策略AB）  
  - 客服主管（低频，验证话术模板对CSAT影响）  
  - 数据分析师（中频，复盘测试归因与长期效应）  
  - 系统管理员（低频，配置全局测试白名单/灰度比例/合规开关）
- **使用频率**：高频（运营专员日均创建2–3个测试；全平台月均活跃测试数≥120）

#### 2. 输入参数
| 参数名 | 类型 | 必填 | 校验规则 | 说明 |
|--------|------|------|----------|------|
| `test_name` | string | 是 | 长度2–50字符；仅允许日文、英文、数字、中文、下划线、短横线；禁止含`<>"'&%`等HTML/SQL注入字符 | 测试名称（如「楽天商品ページ_主图A_vs_B_2024Q3」） |
| `test_scope` | enum | 是 | 取值：`listing`, `ad_campaign`, `customer_service_template`, `promotion_rule` | 测试作用域，决定可绑定的目标对象类型 |
| `target_object_id` | string | 是 | 若scope=listing → 必须为SPU_ID（格式：JP-SPU-2024XXXX）；若scope=ad_campaign → 必须为AD_CAMPAIGN_ID（格式：RAKUTEN-AD-CAMP-XXXX）；其他同理校验 | 被测试的实体唯一标识（非SKU级，避免多变SKU干扰统计） |
| `variant_a_config` | json | 是 | JSON Schema严格校验：包含`name`(string, 2–20), `payload`(object, 根据scope动态约束) | A组配置（基线组）。例如listing场景payload含`title_ja`, `main_image_url`, `price_jpy`字段 |
| `variant_b_config` | json | 是 | 同variant_a_config，且`payload`结构必须与A组完全一致（字段名、类型、嵌套层级）；`name`不可与A组重复 | B组配置（实验组）。payload中所有字段值必须与A组存在实质性差异（如price_jpy差额≥¥50或title_ja编辑字符数≥3） |
| `traffic_ratio` | integer | 是 | 取值范围：10–90（表示B组流量占比%，A组=100−ratio）；必须为10的整数倍 | 流量分配比例（默认50），支持灰度发布（如先10%验证稳定性） |
| `start_time` | datetime | 是 | ISO8601格式（`YYYY-MM-DDTHH:mm:ss+09:00`）；不得早于当前时间+5分钟；不得晚于`end_time`−24h | 日本标准时间（JST）启动时刻，精确到分钟 |
| `end_time` | datetime | 是 | ISO8601格式；不得早于`start_time`+72h；不得晚于`start_time`+30d；需满足`end_time − start_time ≥ 72h` | 测试最短运行72小时（覆盖完整周周期），最长30天（防长尾噪声） |
| `primary_kpi` | enum | 是 | 取值：`cvr`, `ctr`, `avg_order_value`, `review_rating`, `csat_score` | 主要评估指标（仅1个），决定统计引擎计算逻辑与显著性判定基准 |
| `secondary_kpis` | array[enum] | 否 | 最多3项；每项∈{`bounce_rate`, `time_on_page`, `add_to_cart_rate`, `return_rate`, `response_time_sec`}, 且不可与`primary_kpi`重复 | 辅助观测指标，用于归因分析（如CVR提升但跳出率同步上升，则需排查页面体验） |
| `is_appi_compliant` | boolean | 是 | 默认true；若为false，需管理员二次审批并记录原因 | 是否启用APPI合规模式：启用后禁用IP地址记录、用户设备指纹采集，仅基于会话ID与匿名化用户分组ID做分流 |

#### 3. 输出结果
| 输出项 | 类型 | 说明 |
|--------|------|------|
| `test_id` | string | 全局唯一测试ID（格式：ABT-JP-20240925-XXXXX），作为所有关联数据的索引键 |
| `status` | enum | 当前状态：`draft`, `pending_review`, `running`, `paused`, `completed`, `aborted`, `failed` |
| `created_by` | string | 创建者子账号ID（如`sub-acc-jp-mkt-001`） |
| `created_at` | datetime | JST时间戳（ISO8601） |
| `updated_at` | datetime | JST时间戳（ISO8601） |
| `report_summary` | object | 实时汇总（仅当status=running/completed时填充）：<br>• `impression_count_a/b`: 整体曝光量<br>• `conversion_count_a/b`: 转化事件数（按primary_kpi定义）<br>• `cvr_a/b`: 计算值（conversion/impression）<br>• `lift`: (cvr_b − cvr_a)/cvr_a × 100%<br>• `p_value`: 双侧Z检验结果（保留4位小数）<br>• `is_significant`: boolean（p_value < 0.05） |
| `audit_log` | array[object] | 操作审计链（含操作人、时间、动作、变更字段） |

#### 4. 功能设计

##### 4.1 交互流程
1. 用户进入【工具与增长插件】→【A/B测试中心】，点击「新建测试」；  
2. 选择测试作用域（如`listing`），系统自动加载该租户下已授权的乐天/亚马逊店铺内符合权限的SPU列表；  
3. 用户选择目标SPU（如`JP-SPU-20240001`），填写测试名称、时间范围、流量比例；  
4. 在「A组配置」Tab中，系统预填充当前SPU在线Listing的最新日文标题、主图URL、售价（JPY）；用户可编辑；  
5. 在「B组配置」Tab中，用户输入差异化配置（如将标题末尾添加「✨限定特典付き」，价格下调¥150）；系统实时校验payload结构一致性与差异有效性；  
6. 选择主KPI（如`cvr`），勾选辅助指标（如`bounce_rate`, `add_to_cart_rate`）；确认APPI合规开关；  
7. 点击「提交审核」→ 进入`pending_review`状态；若用户为管理员或拥有`ab_test_approve`权限，则自动跳过审核，状态变为`running`；  
8. 测试启动后，系统按`traffic_ratio`在真实流量中分流：A组用户看到原Listing，B组用户看到B组配置；所有行为事件（曝光、点击、加购、下单、评价）经脱敏后写入AB测试专用事件流；  
9. 用户可在仪表盘查看实时报告，支持按「店铺」「日期」「设备类型（PC/手机）」多维下钻；  
10. 测试到期自动`completed`；用户可手动「终止」或「暂停」；完成后的测试支持导出PDF报告（含统计结论、原始数据摘要、APPI合规声明水印）。

##### 4.2 核心逻辑
- **分流算法**：采用MD5(用户会话ID + test_id) % 100 → 确保同一用户在单次测试中始终归属同一分组（sticky session），避免跨组污染；会话ID由前端SDK生成（不依赖Cookie，适配iOS ITP限制）；  
- **指标计算**：  
  - `cvr` = `order_count` / `listing_impression_count`（订单数需匹配SPU且支付成功，排除测试期退款订单）；  
  - `ctr` = `click_count` / `impression_count`（仅统计来自自然搜索/类目页的曝光，排除广告位）；  
  - 所有指标按JST时区每15分钟聚合一次，延迟≤90秒；  
- **显著性检验**：  
  - 对`cvr`/`ctr`类比率指标，采用双样本Z检验（大样本，n>1000）；  
  - 对`review_rating`等连续变量，采用Welch’s t-test（方差不齐校正）；  
  - p-value计算调用Apache Commons Math 3.6.1库，置信水平固定为95%；  
- **APPI合规实现**：  
  - 禁用所有`navigator.userAgent`、`screen.width`等设备指纹采集；  
  - 用户分组ID为UUIDv4（服务端生成），与任何PII（姓名/邮箱/电话）无映射关系；  
  - 原始日志保留≤30天，自动触发GDPR/APPI Right to Erasure流程；  
- **多平台适配**：  
  - 乐天（Rakuten）测试：分流逻辑注入至`rakuten_listing_renderer`微服务，通过HTTP Header `X-AB-Test-ID: ABT-JP-20240925-XXXXX`透传；  
  - 亚马逊（Amazon JP）测试：通过SP-API `putProductContent`接口动态更新A/B组Listing内容，由ERP调度器控制发布时间；  
  - 客服话术测试：在`customer_service_gateway`中拦截工单创建请求，根据会话ID路由至对应模板版本。

##### 4.3 接口设计
| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| `CreateABTest` | POST | `/v1/ab-tests` | 创建新测试（含参数校验、权限检查、异步审核触发） |
| `GetABTestDetail` | GET | `/v1/ab-tests/{test_id}` | 获取测试详情与实时报告（含权限过滤，仅租户内可见） |
| `ListABTests` | GET | `/v1/ab-tests?status=running&scope=listing&offset=0&limit=20` | 分页查询测试列表（支持按状态/作用域/时间范围过滤） |
| `UpdateABTestStatus` | PUT | `/v1/ab-tests/{test_id}/status` | 修改状态（pause/resume/abort），需校验权限与状态机合法性 |
| `ExportABTestReport` | GET | `/v1/ab-tests/{test_id}/export?format=pdf` | 导出合规报告（PDF含数字签名与APPI声明） |

**`CreateABTest` 请求示例（JSON）**：
```json
{
  "test_name": "楽天_主画像テスト_AvsB_2024秋",
  "test_scope": "listing",
  "target_object_id": "JP-SPU-20240001",
  "variant_a_config": {
    "name": "ベースライン",
    "payload": {
      "title_ja": "高品質ステンレス製保温マグカップ",
      "main_image_url": "https://cdn.example.jp/mug-v1.jpg",
      "price_jpy": 2980
    }
  },
  "variant_b_config": {
    "name": "特典付バージョン",
    "payload": {
      "title_ja": "高品質ステンレス製保温マグカップ ✨限定特典付き",
      "main_image_url": "https://cdn.example.jp/mug-v2.jpg",
      "price_jpy": 2830
    }
  },
  "traffic_ratio": 50,
  "start_time": "2024-09-25T10:00:00+09:00",
  "end_time": "2024-10-02T10:00:00+09:00",
  "primary_kpi": "cvr",
  "secondary_kpis": ["bounce_rate", "add_to_cart_rate"],
  "is_appi_compliant": true
}
```

**`GetABTestDetail` 响应示例（JSON）**：
```json
{
  "test_id": "ABT-JP-20240925-7F2A1",
  "status": "running",
  "test_name": "楽天_主画像テスト_AvsB_2024秋",
  "report_summary": {
    "impression_count_a": 12480,
    "impression_count_b": 12512,
    "conversion_count_a": 326,
    "conversion_count_b": 389,
    "cvr_a": 0.02612,
    "cvr_b": 0.03109,
    "lift": 19.03,
    "p_value": 0.0021,
  ...
  "audit_log": [
    {
      "operator": "sub-acc-jp-mkt-001",
      "action": "create",
      "timestamp": "2024-09-24T15:30:22+09:00",
      "changes": ["test_name", "variant_a_config", "variant_b_config"]
    }
  ]
}
```
...
- **`ab_tests` 表**（核心测试元数据）：
  | 字段名 | 类型 | 说明 |
  |--------|------|------|
  | `id` | VARCHAR(32) PK | `test_id`（如`ABT-JP-20240925-7F2A1`） |
  | `tenant_id` | VARCHAR(20) | 租户ID（如`tenant-rakuten-jp-001`） |
  | `created_by` | VARCHAR(32) | 子账号ID |
  | `test_name` | VARCHAR(50) | 测试名称 |
  | `test_scope` | ENUM('listing','ad_campaign','customer_service_template','promotion_rule') | 作用域 |
  | `target_object_id` | VARCHAR(50) | 目标实体ID |
  | `variant_a_config` | JSON | A组配置（含name+payload） |
  | `variant_b_config` | JSON | B组配置 |
  | `traffic_ratio` | TINYINT | B组流量占比（10–90） |
  | `start_time` | DATETIME(JST) | 启动时间 |
  | `end_time` | DATETIME(JST) | 结束时间 |
  | `primary_kpi` | ENUM(...) | 主KPI |
  | `secondary_kpis` | JSON | 辅助KPI数组 |
  | `is_appi_compliant` | BOOLEAN | 合规开关 |
  | `status` | ENUM(...) | 当前状态 |
  | `created_at` | DATETIME(JST) | 创建时间 |
  | `updated_at` | DATETIME(JST) | 更新时间 |

- **`ab_test_events` 表**（原子事件流水，分区表按`test_id` + `date`）：
  | 字段名 | 类型 | 说明 |
  |--------|------|------|
  | `event_id` | BIGINT PK AI | 自增ID |
  | `test_id` | VARCHAR(32) | 关联ab_tests.id |
  | `session_id` | VARCHAR(64) | 匿名化会话ID（MD5哈希） |
  | `variant` | ENUM('A','B') | 所属分组 |
  | `event_type` | ENUM('impression','click','add_to_cart','order','review_submit','csat_submit') | 事件类型 |
  | `event_time` | DATETIME(JST) | JST时间戳 |
  | `value_numeric` | DECIMAL(12,4) | 数值型指标（如订单金额、评分） |
  | `value_string` | VARCHAR(255) | 字符串型指标（如设备类型） |

- **`ab_test_reports_cache` 表**（实时聚合缓存，TTL=15min）：
  | 字段名 | 类型 | 说明 |
  |--------|------|------|
  | `test_id` | VARCHAR(32) PK | 关联ab_tests.id |
  | `as_of_time` | DATETIME(JST) | 快照时间（每15分钟） |
  | `metrics_json` | JSON | 序列化的report_summary结构 |

#### 5. 业务规则
1. **强制冷却期规则**：同一`target_object_id`在任一`test_scope`下，前一个测试`status`变为`completed`或`aborted`后，72小时内禁止创建新测试（防策略叠加干扰）；  
2. **日文合规规则**：当`test_scope=listing`且`target_object_id`对应SPU的`language_code='ja'`时，`variant_a_config.payload.title_ja`与`variant_b_config.payload.title_ja`必须通过日本《景品表示法》敏感词检测（内置词库含「最高」「絶対」等32个禁用词），否则创建失败；  
3. **财务隔离规则**：若测试涉及`price_jpy`变更，系统自动在`financial_transaction`表中标记`ab_test_id`，确保利润分析模块可剥离测试期成本波动，避免误导LTV/CAC计算；  
4. **灰度安全规则**：当`traffic_ratio ≤ 20`且`test_scope=ad_campaign`时，系统强制启用「异常预算熔断」——若B组单日消耗超A组200%，自动暂停B组并通知广告优化师；  
5. **APPI审计规则**：所有`ab_test_events`记录必须包含`consent_version`字段（当前APPI合规策略版本号，如`APPI-2024-Q3`），且该字段值需与`tenant_settings`表中租户配置的`ap_pi_consent_version`一致，否则事件丢弃并告警。
...
| 编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 | 优先级 |
|------|----------|----------|----------|----------|--------|
| TC01 | 正常创建乐天Listing A/B测试 | 用户有`ab_test_create`权限；租户下存在SPU `JP-SPU-20240001` | 1. 填写合法参数（含日文标题、50%流量、7天周期）<br>2. 提交创建 | 返回201；`test_id`生成；状态为`pending_review`或`running`（依权限） | P0 |
| TC02 | 标题含禁用词导致创建失败 | 同TC01；`variant_b_config.payload.title_ja="絶対一番安い！"` | 执行TC01步骤 | 返回400；错误信息含「景品表示法違反：『絶対』は使用不可」 | P0 |
| TC03 | 流量比例非法校验 | 同TC01；设`traffic_ratio=105` | 执行TC01步骤 | 返回400；提示「traffic_ratio必须为10–90之间的10的整数倍」 | P0 |
| TC04 | 同一SPU短时间重复创建 | TC01测试已`completed`；立即尝试创建同SPU新测试 | 执行TC01步骤 | 返回403；提示「該当SPUは過去72時間以内にテストを実施済みです」 | P1 |
| TC05 | APPI合规模式下导出PDF报告 | TC01测试运行中；`is_appi_compliant=true` | 调用`/v1/ab-tests/{id}/export?format=pdf` | 返回200；PDF文件含APPI合规声明水印、无PII信息、数字签名有效 | P1 |

##### 6.2 边界测试
- 测试周期设为最小值72小时（`start_time=2024-09-25T10:00:00+09:00`, `end_time=2024-09-28T10:00:00+09:00`），验证系统能否正确启动与结束；  
- `traffic_ratio=10`时，验证B组是否严格接收约10%真实流量（抽样10万次会话，误差±0.5%）；  
- `secondary_kpis`为空数组`[]`时，`report_summary`中不返回任何辅助指标字段；  
- `test_name`含全角空格与emoji（如`「テスト　✅」`），验证前端渲染与数据库存储完整性；  
- 并发创建50个测试（模拟运营团队批量上线），验证`ab_tests`表写入成功率100%且无锁表。

##### 6.3 异常场景
| 异常场景 | 触发条件 | 系统处理方式 | 用户提示 |
|----------|----------|-------------|----------|
| 乐天API调用失败导致B组Listing未生效 | 乐天SP-API `putProductContent` 返回HTTP 503 | 自动重试3次（指数退避）；失败后标记`status=failed`；触发企业微信告警给运维 | 「楽天へのリストアップ更新に失敗しました。システム管理者へ連絡してください。」 |
| Z检验样本量不足（n<1000） | 测试运行48小时后A/B组曝光均<500 | 暂停统计计算；`report_summary`中`p_value=null`，`is_significant=false`；显示黄标「データが不足しています（推奨：1000回以上露出）」 | 「統計的有意性を評価するには、各グループで1000回以上の露出が必要です。」 |
| 租户APPI策略升级导致版本不匹配 | `ab_test_events`写入时`consent_version`≠租户当前策略版本 | 丢弃该事件；记录WARN日志；向`ab_test_admin`角色推送站内信 | 「テストイベントの記録に失敗しました（コンプライアンスバージョン不一致）。設定を確認してください。」 |
...
#### 1. 功能概述
- **业务目标**：为日本跨境卖家提供低代码、可视化、高安全性的规则配置能力，支持在订单中心、库存中心、客服工单、广告调价、物流分单等核心场景中，基于实时业务数据（如订单金额、收货地址邮编、商品类目、库存水位、平台来源、消费税标识等）自动触发预设动作（如打标、分仓、拦截、通知、调价、生成工单），减少人工干预，提升运营响应速度与合规性（尤其满足日本消费税インボイス制度、地址标准化、敏感词过滤等本地化强约束）。
- **用户价值**：
  - 运营人员无需开发即可自主配置“当订单收货地为东京都港区且含免税商品时，自动添加【インボイス対応必須】标签并触发财务审核工单”；
  - 客服主管可设置“当乐天平台Review评分＜3.5且含敏感词『詐欺』『偽物』时，自动升级为P0级工单并推送企业微信告警”；
  - 采购人员可配置“当FBA仓SKU库存＜安全库存×1.2且近7日销量环比+30%时，自动生成紧急补货建议并邮件通知采购经理”；
  - 所有规则支持日文界面操作、日文条件字段提示、日文动作描述，符合JIS X 0208字符集及日本地址格式（郵便番号7桁+都道府県市区町村）校验。
- **使用角色**：运营管理员（主）、客服主管、采购专员、广告优化师、财务合规专员（需对应模块权限，如“订单中心-规则引擎配置”+“财务与税务-消费税计算”权限才可配置税务相关规则）
- **使用频率**：高频（平均每个中型租户每月新增/修改12–25条规则；大促前3天日均配置量达8–15条）

#### 2. 输入参数
| 参数名 | 类型 | 必填 | 校验规则 | 说明 |
|--------|------|------|----------|------|
| `rule_name` | string(64) | 是 | 非空、长度1–64、仅允许日文汉字/平假名/片假名/英文/数字/下划线/中文括号（「」）、禁止全角空格 | 规则名称，如「楽天注文_東京都内即日発送ルール」，用于列表展示与日志追溯 |
| `rule_code` | string(32) | 是 | 正则 `^[a-z][a-z0-9_]{2,30}[a-z0-9]$`，全局唯一（同一租户内） | 系统级唯一编码，用于API调用、日志标记、审计追踪，如 `rakuten_tokyo_express_ship` |
| `rule_type` | enum | 是 | 必须为预定义值：`order`, `inventory`, `logistics`, `advertising`, `customer_service`, `finance`, `pim`, `compliance` | 规则所属业务域，决定可用的条件字段池与动作类型池（如 `compliance` 类型才开放「敏感词检测」「インボイス資格チェック」条件） |
| `trigger_event` | string(128) | 是 | 必须匹配系统已注册事件ID，如 `order.created`, `inventory.level.updated`, `review.posted`, `ad.campaign.budget.exceeded` | 触发该规则的系统事件，由各模块主动发布至规则引擎事件总线（Kafka Topic: `erp.rules.events.{tenant_id}`） |
| `conditions` | array[object] | 是 | 非空数组；每个元素含 `field`, `operator`, `value`, `logic_connector`；`field` 必须属于当前 `rule_type` + `trigger_event` 组合下的白名单字段；`value` 类型需与字段定义一致（如 `address.postcode` 字段要求 `value` 为7位数字字符串） | 条件组，支持嵌套AND/OR逻辑。例如：<br>`[{ "field": "order.platform", "operator": "=", "value": "rakuten" }, { "field": "address.prefecture", "operator": "in", "value": ["東京都","神奈川県"] }]` |
| `actions` | array[object] | 是 | 非空数组；每个元素含 `type`, `params`；`type` 必须属于当前 `rule_type` 下允许的动作类型（如 `order` 类型支持 `add_tag`, `assign_warehouse`, `block_order`）；`params` 结构按 `type` 动态校验 | 动作组，支持串行执行。例如：<br>`[{ "type": "add_tag", "params": { "tag_code": "rush_shipping" } }, { "type": "send_notification", "params": { "channel": "work_wechat", "template_id": "wx_templ_001" } }]` |
| `enabled` | boolean | 否 | 默认 `true` | 是否启用该规则；禁用后仍保留在列表中，可快速启停 |
| `description` | string(256) | 否 | 长度≤256，支持日文 | 规则用途说明，如「対象：楽天で購入された東京都・神奈川県宛て注文。動作：即日発送タグ付与＋倉庫担当者にWeCom通知」 |
| `priority` | integer | 否 | 范围 1–100，1为最高优先级；同事件下多规则触发时按 priority 降序执行 | 冲突场景下执行顺序控制，如高优规则「欺诈订单拦截」（priority=95）必须在低优规则「优惠券发放」（priority=30）之前执行 |

#### 3. 输出结果
| 输出项 | 类型 | 说明 |
|--------|------|------|
| `rule_id` | string(UUID) | 规则全局唯一ID，格式 `rule_{tenant_id}_{timestamp_ms}_{random6}`，如 `rule_jp-00123_1718923456789_ab3cde`，用于数据库主键及API引用 |
| `status` | string | 值为 `created`, `updated`, `validating`, `validated`, `invalid`；`validated` 表示语法与字段合法性校验通过，可被引擎加载 |
| `validation_errors` | array[string] | 当 `status=invalid` 时返回具体错误，如 `["条件字段 'order.tax_class' 在事件 order.created 中不可用", "动作参数 'template_id' 不存在于工作微信模板库"]` |
| `compiled_ast` | object | 规则编译后的抽象语法树（JSON序列化），供引擎执行时解析，包含条件表达式树与动作执行链，含字段类型转换指令（如将 `address.postcode` 字符串转为整数比对） |
| `created_at` | datetime(ISO8601) | 创建时间，带时区（JST） |
| `created_by` | string(32) | 创建人账号ID（子账号login_id） |

#### 4. 功能设计

##### 4.1 交互流程
1. 用户进入【规则引擎】→【条件动作配置】菜单（路径：`/rules/engine/configure`），页面默认展示当前租户所有规则列表（按 `priority` 降序 + `created_at` 降序）；
2. 点击「新規ルール作成」按钮，跳转至可视化配置页，顶部选择 `rule_type`（如「注文」）与 `trigger_event`（如「注文作成時」），系统动态加载该组合下所有可用字段（如 `order.order_id`, `order.total_amount_jpy`, `address.postcode`, `order.platform`, `item.sku`）及动作类型（如「タグ追加」「倉庫割り当て」「注文ブロック」）；
3. 用户拖拽字段至条件区，选择运算符（`=`, `!=`, `>`, `<`, `in`, `contains`, `matches_regex`, `is_jp_postcode_valid`），输入值（支持日文输入、邮编智能补全、下拉枚举）；支持点击「＋条件グループ」添加OR组，用逻辑连接符切换AND/OR；
4. 在动作区点击「アクション追加」，选择动作类型，填写参数（如选「タグ追加」后从下拉选择已有日文标签码，或输入新标签名；选「通知送信」后选择渠道（Email/WeCom/Slack）及日文模板）；
5. 点击「プレビューと検証」：前端模拟一条符合当前条件的测试订单数据（含真实字段结构），调用 `/api/v1/rules/validate` 接口验证语法、字段可用性、动作参数有效性，并返回 `compiled_ast` 预览；
6. 点击「保存して有効化」：调用 `/api/v1/rules` POST 接口提交，服务端完成：① 保存至数据库；② 将规则编译为AST并存入Redis缓存（Key: `rule:ast:{tenant_id}:{rule_code}`，TTL=24h）；③ 向Kafka发送 `RuleDeployedEvent` 事件，触发规则引擎热加载；④ 返回 `status=validated` 及 `rule_id`；
7. 用户可在列表页对规则执行「編集」「無効化」「削除」「実行ログ確認」操作。

##### 4.2 核心逻辑
- **字段白名单动态生成**：基于 `rule_type` + `trigger_event` 查询元数据表 `rule_field_schema`，获取该事件上下文中可访问的字段列表及其类型（string/int/decimal/boolean/array）、是否支持正则、是否需本地化处理（如 `address.prefecture` 字段值映射为日文都道府県名而非英文缩写）；
- **条件表达式编译**：将用户配置的条件组转换为可执行AST，关键处理包括：
  - 日文邮编校验：`is_jp_postcode_valid` 运算符自动调用 `JPPostcodeValidator.validate(value)`，支持7位数字及`〒`前缀；
  - 消费税逻辑：当 `field=order.tax_class` 且 `value=tax_exempt` 时，自动注入 `is_invoice_registered(customer_id)` 函数调用，查询财务模块确认该客户是否具备インボイス登録資格；
  - 敏感词检测：`contains` 运算符作用于 `review.content` 字段时，自动调用 `JapaneseSensitiveWordFilter.check(text)`，内置JIS X 4081标准词库及租户自定义词表；
- **动作执行沙箱化**：每个动作在独立Spring Boot `@Transactional` 事务中执行，失败动作自动回滚，不影响其他动作；动作间通过 `ActionContext` 传递上下文（如 `add_tag` 动作生成的 `tag_id` 可被后续 `send_notification` 动作引用）；
- **冲突检测前置**：保存时调用 `/api/v1/rules/conflict-detect`，扫描同 `trigger_event` 下所有 `enabled=true` 规则，检查是否存在 `priority` 相同但条件交集非空的情况（如规则A：`platform=rakuten AND prefecture=東京都`；规则B：`platform=rakuten AND postcode starts with "100"` → 交集存在），返回冲突规则ID列表供用户调整。

##### 4.3 接口设计
| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| `GET /api/v1/rules/fields` | GET | `/api/v1/rules/fields?rule_type={type}&trigger_event={event}` | 获取指定类型+事件下的可用字段列表，返回 `[{ "field": "order.total_amount_jpy", "label": "注文金額（円）", "type": "decimal", "supports_operators": ["=", ">", "<", ">="] }]` |
| `POST /api/v1/rules/validate` | POST | `/api/v1/rules/validate` | 规则预验证接口，请求体含 `rule_type`, `trigger_event`, `conditions`, `actions`，返回 `status`, `validation_errors`, `compiled_ast` |
| `POST /api/v1/rules` | POST | `/api/v1/rules` | 创建新规则，请求体为完整输入参数，成功返回 `rule_id`, `status`, `compiled_ast`, `created_at` |
| `PUT /api/v1/rules/{rule_code}` | PUT | `/api/v1/rules/{rule_code}` | 更新规则，路径参数 `rule_code` 必须存在，校验逻辑同创建 |
| `GET /api/v1/rules/conflict-detect` | GET | `/api/v1/rules/conflict-detect?rule_code={code}` | 检测指定规则与其他规则的冲突，返回冲突规则列表 |

##### 4.4 数据模型
- **`rule_definition`**（规则主表）
  - `id`: VARCHAR(64) PK — `rule_id`
  - `tenant_id`: VARCHAR(32) NOT NULL — 租户ID（如 `jp-00123`）
  - `rule_code`: VARCHAR(32) NOT NULL UNIQUE
  - `rule_name`: VARCHAR(64) NOT NULL
  - `rule_type`: ENUM('order','inventory','logistics','advertising','customer_service','finance','pim','compliance') NOT NULL
  - `trigger_event`: VARCHAR(128) NOT NULL
  - `conditions_json`: JSON NOT NULL — 存储原始条件数组
  - `actions_json`: JSON NOT NULL — 存储原始动作数组
  - `priority`: TINYINT UNSIGNED DEFAULT 50 CHECK (priority BETWEEN 1 AND 100)
  - `enabled`: BOOLEAN DEFAULT TRUE
  - `description`: VARCHAR(256)
  - `status`: ENUM('created','updated','validating','validated','invalid') DEFAULT 'created'
  - `validation_errors_json`: JSON — 校验失败时存储错误数组
  - `compiled_ast_json`: JSON — 编译后AST（含字段类型转换指令）
  - `created_at`: DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3)
  - `created_by`: VARCHAR(32) NOT NULL
  - `updated_at`: DATETIME(3) DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3)
  - `updated_by`: VARCHAR(32)

- **`rule_field_schema`**（字段元数据表）
  - `id`: BIGINT PK
  - `rule_type`: ENUM(...) NOT NULL
  - `trigger_event`: VARCHAR(128) NOT NULL
  - `field_path`: VARCHAR(128) NOT NULL — 如 `order.address.postcode`
  - `label_ja`: VARCHAR(128) NOT NULL — 日文标签，如「郵便番号」
  - `data_type`: ENUM('string','integer','decimal','boolean','array','datetime') NOT NULL
  - `is_localized`: BOOLEAN DEFAULT FALSE — 是否需本地化（如都道府県名映射）
  - `supported_operators`: JSON — 如 `["=", "in", "is_jp_postcode_valid"]`
  - `example_value`: VARCHAR(128) — 示例值，用于前端提示

#### 5. 业务规则
1. **日本地址强校验**：当条件中使用 `address.postcode` 字段时，`value` 必须为7位纯数字或以 `〒` 开头的8位字符串（`〒123-4567`），否则校验失败；`address.prefecture` 字段值必须为JIS X 0401标准都道府県名（如「東京都」「大阪府」），禁止使用英文缩写。
2. **消费税合规联动**：若规则中配置了 `order.tax_class = tax_exempt` 条件，则必须同时启用 `finance` 模块权限，且动作中若含 `send_notification`，其模板必须包含インボイス登録番号查询链接（由系统自动注入 `{{invoice_reg_url}}` 变量）。
3. **敏感词实时生效**：当规则类型为 `compliance` 且条件含 `review.content contains ...` 时，所用敏感词库必须为租户最新版（`tenant_sensitive_word_list` 表中 `status=active` 的最新记录），且词库更新后，所有关联规则自动触发重新编译（通过监听 `SensitiveWordListUpdatedEvent`）。
4. **动作幂等性保障**：同一订单在相同事件下被同一规则多次触发时（如网络重试），`add_tag` 动作自动去重（检查标签是否已存在），`send_notification` 动作依据 `notification_key = rule_code + event_id + order_id` 去重，24小时内不重复发送。
5. **租户隔离硬约束**：所有规则数据（含 `rule_definition`, `rule_field_schema` 查询结果）强制绑定 `tenant_id`，数据库查询WHERE条件必须包含 `tenant_id = ?`，API层通过JWT解析 `tenant_id` 并透传至DAO层。
...
| 编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 | 优先级 |
|------|----------|----------|----------|----------|--------|
| TC01 | 正常创建东京即日发货规则 | 租户已开通「注文」模块权限；存在日文标签「即日発送」；已配置工作微信通知模板 | 1. 选择 rule_type=order, trigger_event=order.created<br>2. 添加条件：`order.platform = rakuten` AND `address.prefecture in [東京都,神奈川県]`<br>3. 添加动作：`add_tag`（tag_code=immediate_ship）+ `send_notification`（channel=work_wechat）<br>4. 点击「プレビューと検証」→「保存して有効化」 | 返回 status=validated，rule_id 生成，Redis中存在对应AST缓存，Kafka发出 RuleDeployedEvent | P0 |
| TC02 | 邮编格式校验失败 | 同TC01前置条件 | 在条件中设置 `address.postcode = "123456"`（仅6位） | 「プレビューと検証」返回 validation_errors 包含 `"郵便番号は7桁の数字である必要があります"`，状态为 invalid | P0 |
| TC03 | 敏感词规则与自定义词库联动 | 租户已在「日本本地化与合规」模块上传自定义词表（含词「偽物」），状态 active | 创建 compliance 类型规则，条件：`review.content contains "偽物"`，动作：`create_ticket` | 规则保存成功；当真实Review含「偽物」时，规则引擎准确触发工单创建 | P0 |
| TC04 | 规则冲突检测 | 已存在规则A（code=r1, priority=80, condition=`platform=rakuten AND prefecture=東京都`） | 创建新规则B（code=r2, priority=80, condition=`platform=rakuten AND postcode starts with "100"`） | 「保存して有効化」前，冲突检测接口返回 `[{"conflict_rule_code":"r1","reason":"条件の重複領域が存在します"}]` | P1 |
| TC05 | 多币种金额条件比对 | 租户启用多币种（JPY/CNY/USD），订单事件中 `order.total_amount_jpy` 字段存在 | 创建规则：`order.total_amount_jpy > 10000` | 规则引擎执行时，正确提取日元金额字段值（非其他币种），比对成功 | P1 |

##### 6.2 边界测试
- 输入 `rule_name` 为64个全角字符（如「あいうえおかきくけこさしすせそたちつてとなにぬねのはひふへほまみむめもやいゆえよらりるれろわをん」）—— 应保存成功；
- `conditions` 数组长度为50（超常规上限）—— 系统应拒绝，返回 `validation_errors` 提示“条件数量不得超过30个”；
- `actions` 中 `send_notification` 的 `template_id` 传入不存在ID（如 `wx_templ_999`）—— 验证失败并提示“通知模板不存在”。

##### 6.3 异常场景
| 异常场景 | 触发条件 | 系统处理方式 | 用户提示 |
|----------|----------|-------------|----------|
| 租户权限不足 | 用户无「订单中心-规则引擎配置」权限，访问 `/rules/engine/configure` 页面 | 后端返回 403 Forbidden，前端跳转至权限不足页 | 「この機能を利用する権限がありません。管理者にご確認ください。」 |
| Redis缓存写入失败 | 规则保存时，Redis连接超时或OOM | 回滚数据库事务，记录ERROR日志，返回 `status=invalid` + `validation_errors=["ルールASTのキャッシュ保存に失敗しました"]` | 「ルールの有効化に失敗しました。システム管理者にお問い合わせください。」 |
| Kafka事件发布失败 | `RuleDeployedEvent` 发送失败（如Broker宕机） | 启动补偿任务（Quartz定时扫描 `status=created` 的规则），每5分钟重试，最多3次 | 无前端提示，后台自动恢复；若3次失败，向租户管理员发送企业微信告警 |
...
- **用户价值**：
  - 运营人员可5秒内定位某笔订单未被“自动合单规则”捕获的具体原因（如：`order_amount < 3000 JPY` 不满足阈值、`shipping_country != "JP"` 导致跳过）；
  - 技术支持/客户成功团队可导出带完整上下文的日志包（含原始事件JSON、规则快照、执行耗时、租户ID、店铺ID），用于向乐天/亚马逊平台方举证非系统侧故障；
  - 合规审计人员可按《日本个人信息保护法（APPI）》第26条要求，导出含操作时间戳、执行主体（子账号）、规则ID、脱敏事件ID的日志摘要，满足3年日志留存与访问留痕要求。
- **使用角色**：  
  - 主账号管理员（查看全租户日志、导出审计包）  
  - 子账号运营专员（仅查看所属店铺/业务线触发的日志）  
  - 客服主管（关联工单ID反查规则执行结果）  
  - 系统运维工程师（排查规则引擎性能瓶颈与异常中断）
- **使用频率**：中频（日常运营监控约3–5次/日；大促期间（如乐天Super Sale）达20+次/日；故障排查属低频但高优先级）

#### 2. 输入参数
| 参数名 | 类型 | 必填 | 校验规则 | 说明 |
|--------|------|------|----------|------|
| `tenant_id` | string | 是 | 非空、长度≤32、匹配正则 `^[a-z0-9]{8}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{12}$` | 租户唯一标识（UUIDv4），用于多租户数据隔离与合规审计溯源 |
| `rule_id` | string | 否 | 若填写，需存在且状态为 `enabled` 或 `disabled` | 规则唯一ID（如 `RULE_ORD_AUTO_SPLIT_JP_2024Q3`），支持精确过滤 |
| `event_type` | enum | 否 | 取值范围：`order_created`, `inventory_updated`, `ad_budget_exhausted`, `review_posted`, `shipment_confirmed` | 事件类型，对应规则引擎监听的12类核心业务事件（详见`rule_event_type_enum`表） |
| `start_time` | datetime | 是 | ISO8601格式，且 ≥ `tenant.created_at`，≤ 当前时间-1s | 查询起始时间（含），支持日本标准时间（JST, UTC+9）输入，后端自动转为UTC存储 |
| `end_time` | datetime | 是 | ISO8601格式，且 > `start_time`，≤ 当前时间 | 查询截止时间（含），最大跨度为30天（防全表扫描） |
| `status` | enum | 否 | 取值范围：`success`, `failed`, `skipped`, `timeout` | 执行结果状态，`skipped` 表示条件不满足未执行；`timeout` 指规则脚本执行超500ms强制终止 |
| `page_no` | integer | 否 | ≥1，≤10000 | 分页页码，默认为1 |
| `page_size` | integer | 否 | ∈ [10, 100]，默认50 | 每页条数，防止前端OOM |

#### 3. 输出结果
| 输出项 | 类型 | 说明 |
|--------|------|------|
| `total_count` | integer | 符合条件的日志总条数（用于分页计算） |
| `logs` | array of object | 日志列表，每条包含以下字段（见4.4数据模型） |
| `logs[].id` | string | 日志唯一ID（UUIDv4） |
| `logs[].rule_id` | string | 关联规则ID |
| `logs[].rule_name_ja` | string | 规则日文名称（如「注文金額3,000円以上で自動同梱」），用于运营人员快速识别 |
| `logs[].event_id` | string | 原始业务事件ID（如乐天订单号 `RKT20240512-987654321`，已脱敏处理） |
| `logs[].event_type` | string | 事件类型（同输入参数） |
| `logs[].trigger_time` | datetime | 事件触发时间（JST，ISO8601） |
| `logs[].execute_start_time` | datetime | 规则引擎开始执行时间（UTC） |
| `logs[].execute_end_time` | datetime | 规则引擎执行完成时间（UTC） |
| `logs[].duration_ms` | integer | 执行耗时（毫秒），精度±1ms |
| `logs[].status` | string | 执行状态（`success`/`failed`/`skipped`/`timeout`） |
| `logs[].error_message` | string | 仅当 `status=failed` 或 `timeout` 时存在，含日文错误提示（如「在庫数が不足しています（SKU: ABC-123-JP）」）及错误码（`ERR_RULE_EVAL_007`） |
| `logs[].matched_conditions` | array of string | 条件匹配详情（如 `["order_amount >= 3000", "shipping_prefecture IN ['Tokyo','Osaka']"]`），空数组表示`skipped` |
| `logs[].executed_actions` | array of string | 实际执行的动作（如 `["set_order_tag: auto-merged", "send_slack_alert: #ops-jp"]`） |
| `logs[].operator_account_id` | string | 执行该规则的子账号ID（用于权限审计） |
| `logs[].shop_id` | string | 关联店铺ID（如 `rakuten_jp_main`），支持乐天/亚马逊/雅虎多平台标识 |
| `logs[].tenant_id` | string | 租户ID（冗余字段，保障日志独立可查） |

#### 4. 功能设计

##### 4.1 交互流程
1. 用户登录ERP系统，进入【规则引擎】→【执行日志】菜单（路径：`/rules/logs`）；  
2. 系统默认加载当前租户最近7天、状态为 `success` 的日志（分页显示前50条），顶部展示统计卡片（今日执行总数/失败率/平均耗时）；  
3. 用户通过筛选器选择：店铺（支持多选）、规则ID（模糊搜索）、事件类型、状态、时间范围（支持预设选项：今日/近3天/近7天/自定义）；  
4. 点击【搜索】，前端发送GET请求至 `/api/v1/rules/logs`，携带校验后的参数；  
5. 后端返回日志列表，前端渲染表格，每行末尾提供【详情】按钮；  
6. 点击【详情】弹出抽屉式面板，展示该条日志的完整JSON结构，包括：原始事件载荷（脱敏后）、规则DSL快照（含版本号）、执行堆栈（仅对`failed`日志显示前10行）、关联工单ID（如有）；  
7. 用户可点击【导出CSV】生成符合日本《会计法》第20条要求的审计文件（含列：执行时间JST、规则名（日）、店铺、事件ID、状态、耗时ms、操作人姓名（日文））；  
8. 管理员点击【导出全量审计包】生成ZIP，内含：`log_summary.csv`、`rule_snapshot_v20240512.json`、`system_env_report.txt`（含JVM版本、规则引擎版本、时区配置）。

##### 4.2 核心逻辑
- **日志写入时机**：规则引擎在`RuleExecutor.execute()`方法的`finally`块中，无论成功/失败/跳过，均调用`LogService.appendExecutionLog()`异步落库（避免阻塞主流程）；  
- **事件脱敏策略**：对`event_payload`中的敏感字段（`customer_phone`, `customer_email`, `billing_address.full_address`）执行AES-256-GCM加密（密钥由KMS托管），仅保留`customer_id_hash`（SHA256+盐值）供关联查询；  
- **性能保障**：  
  - 写入采用Kafka异步队列（Topic: `erp-rule-exec-log`），消费者批量写入ClickHouse（分区键：`toMonday(trigger_time)`）；  
  - 查询走物化视图 `mv_rule_log_search`，预聚合`tenant_id`, `rule_id`, `event_type`, `status`, `toDate(trigger_time)` 字段；  
- **规则快照绑定**：每次规则保存时，生成不可变快照存入`rule_snapshot`表（含`rule_content_json`, `version`, `created_by`, `created_at`），日志中`rule_snapshot_id`指向该快照，确保历史日志可复现；  
- **JST时间处理**：所有前端传入的`start_time`/`end_time`经Nginx层转换为UTC（`set $time_jst $arg_start_time; set $time_utc ${time_jst}+0900;`），后端存储与查询均基于UTC，展示时按`tenant.timezone='Asia/Tokyo'`转回JST。

##### 4.3 接口设计
| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| `GET /api/v1/rules/logs` | GET | `/api/v1/rules/logs?tenant_id=xxx&start_time=2024-05-01T00:00:00%2B09:00&end_time=2024-05-07T23:59:59%2B09:00&page_no=1&page_size=50` | 查询规则执行日志列表，返回分页结果 |
| `GET /api/v1/rules/logs/{log_id}` | GET | `/api/v1/rules/logs/8a1b2c3d-4e5f-6g7h-8i9j-0k1l2m3n4o5p` | 获取单条日志详情（含脱敏事件载荷与规则快照） |
| `POST /api/v1/rules/logs/export/csv` | POST | `/api/v1/rules/logs/export/csv` | 导出CSV审计文件（响应为`application/octet-stream`） |
| `POST /api/v1/rules/logs/export/audit-zip` | POST | `/api/v1/rules/logs/export/audit-zip` | 导出全量审计ZIP包（需`tenant_admin`权限） |
...
```json
{
  ...
  "start_time": "2024-05-01T00:00:00+09:00",
  "end_time": "2024-05-07T23:59:59+09:00",
  "status": "failed",
  "page_no": 1,
  "page_size": 50
}
    ...
    "logs": [
      {
        ...
        "rule_id": "RULE_INV_LOW_STOCK_JP",
        "rule_name_ja": "在庫数が5個未満の際、楽天店舗にアラート通知",
        "event_id": "RKT20240505-1122334455",
        "event_type": "inventory_updated",
        "trigger_time": "2024-05-05T14:22:33+09:00",
        "execute_start_time": "2024-05-05T05:22:33Z",
        "execute_end_time": "2024-05-05T05:22:33Z",
        "duration_ms": 127,
        "status": "failed",
        "error_message": "Slack Webhook接続失敗（エラー番号：ERR_SLACK_403）",
        "matched_conditions": ["inventory_quantity < 5"],
        "executed_actions": [],
        "operator_account_id": "acc-op-jp-001",
        "shop_id": "rakuten_jp_main",
        "tenant_id": "a1b2c3d4-e5f6-7890-g1h2-i3j4k5l6m7n8"
      }
    ]
  }
...
##### 4.4 数据模型
- **表名**：`rule_execution_log`（ClickHouse引擎：ReplacingMergeTree）  
  | 字段名 | 类型 | 说明 |
  |--------|------|------|
  | `id` | UUID | 日志唯一ID |
  | `tenant_id` | String | 租户ID（分区键） |
  | `rule_id` | String | 规则ID |
  | `rule_snapshot_id` | String | 关联规则快照ID（外键至`rule_snapshot.id`） |
  | `event_id` | String | 业务事件ID（已脱敏） |
  | `event_type` | Enum8 | `('order_created', 'inventory_updated', ...)` |
  | `trigger_time` | DateTime('Asia/Tokyo') | 事件触发时间（JST） |
  | `execute_start_time` | DateTime | 执行开始时间（UTC） |
  | `execute_end_time` | DateTime | 执行结束时间（UTC） |
  | `duration_ms` | UInt32 | 执行耗时（毫秒） |
  | `status` | Enum8 | `('success', 'failed', 'skipped', 'timeout')` |
  | `error_code` | String | 错误码（如`ERR_RULE_EVAL_007`） |
  | `error_message_ja` | String | 日文错误消息（UTF-8） |
  | `matched_conditions` | Array(String) | 匹配的条件表达式数组 |
  | `executed_actions` | Array(String) | 执行的动作数组 |
  | `operator_account_id` | String | 操作人子账号ID |
  | `shop_id` | String | 店铺ID |
  | `created_at` | DateTime | 日志创建时间（UTC） |

- **关联表**：`rule_snapshot`（MySQL）  
  | 字段名 | 类型 | 说明 |
  |--------|------|------|
  | `id` | VARCHAR(36) | 快照ID（PK） |
  | `rule_id` | VARCHAR(64) | 规则ID |
  | `version` | VARCHAR(20) | 版本号（如`v2.3.1`） |
  | `rule_content_json` | JSON | 规则DSL内容（含条件、动作、优先级） |
  | `created_by` | VARCHAR(64) | 创建人账号ID |
  | `created_at` | DATETIME | 创建时间（JST） |

#### 5. 业务规则
1. **日志留存周期**：所有日志在ClickHouse中保留**365天**，到期后由定时任务（每日02:00 JST）归档至AWS S3 Glacier Deep Archive（符合日本《会計帳簿等の保存に関する法律》第20条）；  
2. **敏感信息强制脱敏**：任何含`phone`/`email`/`address`字段的事件载荷，在写入日志前必须经`DataMaskingService.maskEventPayload()`处理，未脱敏日志禁止写入（熔断机制）；  
3. **执行耗时告警**：单条规则执行>500ms触发企业微信告警（发送至「ERP-规则引擎运维群」），告警内容含`rule_id`、`avg_duration_24h`、`p95_duration_24h`；  
4. **租户级隔离**：日志查询接口强制校验`X-Tenant-ID` Header与参数`tenant_id`一致性，不一致则返回`403 Forbidden`；  
5. **日本本地化输出**：所有日志中的`rule_name_ja`、`error_message_ja`字段必须非空，若规则未配置日文名称，则fallback为`rule_name_en`并添加`[英]`标记（如「Auto-Merge Orders [英]」）。
...
| 编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 | 优先级 |
|------|----------|----------|----------|----------|--------|
| TC01 | 正常查询最近7天成功日志 | 租户`a1b2c3d4...`下存在120条`status=success`日志 | 1. 访问`/api/v1/rules/logs`，传参`tenant_id=a1b2c3d4..., start_time=2024-05-01T00:00:00+09:00, end_time=2024-05-07T23:59:59+09:00`<br>2. 不填`status` | 返回`total_count=120`，`logs`数组含50条`status=success`日志，`rule_name_ja`为日文 | P0 |
| TC02 | 精确匹配失败日志并查看详情 | 存在1条`status=failed`日志，`error_message_ja="在庫数が不足しています"` | 1. 查询时指定`status=failed`<br>2. 调用`GET /api/v1/rules/logs/{id}` | 响应中`error_message_ja`字段值为"在庫数が不足しています"，且`matched_conditions`非空 | P0 |
| TC03 | 跨店铺日志隔离验证 | 租户下有`rakuten_jp_main`和`amazon_jp_pro`两个店铺，各触发50条日志 | 查询时指定`shop_id=rakuten_jp_main` | 返回日志`shop_id`全部为`rakuten_jp_main`，总数为50 | P0 |
| TC04 | 时间范围超限拦截 | 尝试查询`start_time=2024-01-01T00:00:00+09:00`到`end_time=2024-06-01T00:00:00+09:00`（跨度32天） | 发送GET请求 | 返回`400 Bad Request`，`message="time range must not exceed 30 days"` | P1 |
| TC05 | 导出CSV含日文与合规字段 | 租户配置时区为`Asia/Tokyo`，存在含日文规则名的日志 | 调用`POST /api/v1/rules/logs/export/csv` | 下载CSV文件，首行为`実行時間(JST),ルール名,店舗ID,ステータス,処理時間(ms),操作者名`，时间列为JST格式 | P0 |

##### 6.2 边界测试
- 测试`page_size=100`时返回100条日志且`total_count`准确（验证分页上限）；  
- 测试`start_time=end_time`（同秒级）时返回空数组（验证时间边界容错）；  
- 测试`tenant_id`为非法UUID（如`abc`）时返回`400`且不泄露内部表结构（验证安全防护）；  
- 测试高并发查询（100 QPS持续5分钟）下ClickHouse CPU使用率≤70%，P99延迟≤800ms（验证性能基线）；  
- 测试`event_id`含日文字符（如`楽天注文20240505-123`）时日志正常写入与查询（验证UTF-8兼容性）。

##### 6.3 异常场景
| 异常场景 | 触发条件 | 系统处理方式 | 用户提示 |
|----------|----------|-------------|----------|
| Kafka写入失败 | `erp-rule-exec-log` Topic不可用超过30秒 | 启用本地磁盘缓冲（`/var/log/erp/rule-log-buffer/`），每5秒重试，缓冲区满（1GB）则触发企业微信告警 | 页面顶部Toast："日志暂未同步，请稍后刷新（错误码：LOG_KAFKA_503）" |
| 规则快照丢失 | 日志中`rule_snapshot_id`在`rule_snapshot`表中不存在 | 自动填充`rule_name_ja="[快照缺失]"`，`rule_content_json=null`，记录审计日志`ALERT_MISSING_SNAPSHOT` | 在日志详情页显示红色警示："关联规则快照已删除，无法复现执行逻辑" |
| 时区解析失败 | 前端传入`start_time="2024-05-01T00:00:00+09:99"`（非法偏移） | 拒绝请求，不写入任何日志 | 返回`400`，`message="Invalid timezone offset in start_time"` |
| 权限越界访问 | 子账号`acc-op-jp-001`尝试查询`tenant_id=other-tenant-id` | 请求被网关拦截，不进入业务逻辑 | 返回`403 Forbidden`，`message="Access denied to tenant"` |
